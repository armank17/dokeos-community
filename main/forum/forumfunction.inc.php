...
function handle_forum_and_forumcategories() {
    ...
    if (( isset($_GET['action']) && $_GET['action'] == 'delete') && isset($_GET['content']) && $get_id) {
        ...
        Display :: display_confirmation_message2($return_message, false, true);
...
function show_add_forumcategory_form($inputvalues = array()) {    
    ...
    // initiate the object
    $form = new FormValidator('forumcategory', 'post', 'index.php?' . api_get_cidreq().'&origin=addCategory');
    ...
    $form->addElement('textarea', 'forum_category_comment', get_lang('Comment'), array('rows' => '5', 'cols' => '50','style'=>'width:365px;resize: none;'));
    ...
    if ($form->validate()) {                 
        ...
        if ($check) {             
            ...
            $value = store_forumcategory($values);             
            //$_SESSION["display_confirmation_message"] = ($value=='insert') ? get_lang('ForumCategoryAdded') : get_lang('ForumCategoryEdited');            
            $url = api_get_path(WEB_CODE_PATH).'forum/index.php?'.api_get_cidreq();           
            header("Location: " . $url );
        }
...
function show_add_forum_form($inputvalues = array()) {
    ...
    if ($inputvalues == ''){
        ...
        $actionSent = 'editForum';
    
    $form = new FormValidator('forumcategory', 'post', 'index.php?' . api_get_cidreq() . '&amp;gradebook=' . $add_params_for_lp . '&origin='.$actionSent);
    ...
    $form->addElement('text', 'forum_title', get_lang('Title'), 'class="focus" style="width:330px;"');
    ...
    if ($form->validate()) {
        ...
        if ($check) {
            ...
            //$_SESSION["display_confirmation_message"] = ($value=='insert') ? get_lang('ForumAdded') : get_lang('ForumEdited');
            //display::display_confirmation_message2($return_message, false, true);
            $url = api_get_path(WEB_CODE_PATH).'forum/index.php?'.api_get_cidreq();
...
function show_edit_forumcategory_form($inputvalues = array()) {
    // initiate the object
    $gradebook = Security::remove_XSS($_GET['gradebook']);
    $form = new FormValidator('forumcategory', 'post',  api_get_path(WEB_CODE_PATH).'forum/index.php?'.api_get_cidreq().'&origin=editCategory');
...
function store_forumcategory($values) {
    ...
    if (isset($values['forum_category_id'])) { // storing an edit
        ...
        $return_message = '<div style="width:100%;padding: 10px;">' . get_lang('ForumCategoryEdited');
        $return_message.= ' :' . '<a href="index.php?' . api_get_cidreq() . '">  aaaa   ' . Display::return_icon('pixel.gif', get_lang('Back'), array('class' => 'prev_document')) . ' ' . get_lang('BackToForum') . '</a></div>';
...
function display_visible_invisible_icon($content, $id, $current_visibility_status, $additional_url_parameters = '', $return = false) {
    ...
    if ($current_visibility_status == '1') {
        ...
        $link .= 'action=invisible&amp;content=' . $content . '&amp;id=' . $id . '&amp;gradebook=' . $gradebook . '&amp;origin=' . $origin . (isset($_GET['forumcategory']) ? '&amp;forumcategory=' . intval($_GET['forumcategory']) : '') . '">' . Display::return_icon('pixel.gif', get_lang('MakeInvisible'), array('class' => 'actionplaceholdericon actionvisible')) . '</a>';
        $output = $link;
    }
    if ($current_visibility_status == '0') {
        ...
        $link .= 'action=visible&amp;content=' . $content . '&amp;id=' . $id . '&amp;gradebook=' . $gradebook . '&amp;origin=' . $origin . (isset($_GET['forumcategory']) ? '&amp;forumcategory=' . intval($_GET['forumcategory']) : '') . '">' . Display::return_icon('pixel.gif', get_lang('MakeVisible'), array('class' => 'actionplaceholdericon actionvisible invisible')) . '</a>';
        $output = $link;
    }
    if ($return) {
        return $output;
...
function display_lock_unlock_icon($content, $id, $current_lock_status, $additional_url_parameters = '', $return = false) {
    ...
    if ($current_lock_status == '1') {
        ...
        $link .= 'action=unlock&amp;content=' . $content . '&amp;id=' . $id . (isset($_GET['forumcategory']) ? '&amp;forumcategory=' . intval($_GET['forumcategory']) : '') . '">' . Display::return_icon('pixel.gif', get_lang('Unlock'), array('class' => 'actionplaceholdericon actionlock')) . '</a>';
        $output = $link;
    }
    if ($current_lock_status == '0') {
        ...
        $link .= 'action=lock&amp;content=' . $content . '&amp;id=' . $id . (isset($_GET['forumcategory']) ? '&amp;forumcategory=' . intval($_GET['forumcategory']) : '') . '">' . Display::return_icon('pixel.gif', get_lang('Lock'), array('class' => 'actionplaceholdericon actionunlock')) . '</a>';
        $output = $link;
    }
    if ($return) {
        return $output;
...
function class_visible_invisible($current_visibility_status) {
    if ($current_visibility_status == '0') {
        return "class='breadcrumb_invisible'";
    }
}
...
function get_forums($id = '') {
    ...
    if ($id == '') {
        ...
        $sql = "SELECT * FROM " . $table_forums . " forum , " . $table_item_property . " item_properties 
		WHERE forum.forum_id=item_properties.ref AND item_properties.visibility=1 
		AND item_properties.tool='" . TOOL_FORUM . "' $condition_session
		ORDER BY forum.forum_order ASC";
        // select the number of threads of the forums (only the threads that are visible)
        $sql2 = "SELECT count(*) AS number_of_threads, threads.forum_id FROM $table_threads threads, " . $table_item_property .
                " item_properties WHERE threads.thread_id=item_properties.ref AND item_properties.visibility=1
		 AND item_properties.tool='" . TOOL_FORUM_THREAD . "' GROUP BY threads.forum_id";
        // select the number of posts of the forum (post that are visible and that are in a thread that is visible)
        $sql3 = "SELECT count(*) AS number_of_posts, posts.forum_id FROM $table_posts posts, $table_threads threads, "
                . $table_item_property . " item_properties WHERE posts.visible=1 AND posts.thread_id=threads.thread_id 
		 AND threads.thread_id=item_properties.ref AND item_properties.visibility=1 
		 AND item_properties.tool='" . TOOL_FORUM_THREAD . "' GROUP BY threads.forum_id";
        ...
        if (is_allowed_to_edit()) {
            ...
            $sql = "SELECT * FROM " . $table_forums . " forum , " . $table_item_property . " item_properties 
                    WHERE forum.forum_id=item_properties.ref AND item_properties.visibility<>2 
                    AND item_properties.tool='" . TOOL_FORUM . "' $condition_session
                    ORDER BY forum_order ASC";
            //echo $sql.'<hr>';
            ...
            $sql2 = "SELECT count(*) AS number_of_threads, threads.forum_id FROM $table_threads threads, " . $table_item_property . " item_properties 
		     WHERE threads.thread_id=item_properties.ref AND item_properties.visibility<>2 AND item_properties.tool='" . TOOL_FORUM_THREAD . "' 
            ...
            $sql3 = "SELECT count(*) AS number_of_posts, posts.forum_id FROM $table_posts posts, $table_threads threads, " . $table_item_property . " item_properties 
		     WHERE posts.thread_id=threads.thread_id AND threads.thread_id=item_properties.ref AND item_properties.visibility=1 
		     AND item_properties.tool='" . TOOL_FORUM_THREAD . "' GROUP BY threads.forum_id";
            //echo $sql3.'<hr>';
        }
    }
    ...
    // when editing a forum (and for the moment it is the only place where we use this part of the function)
        ...
        $sql = "SELECT * FROM " . $table_forums . " forum , " . $table_item_property . " item_properties 
		WHERE forum.forum_id=item_properties.ref AND forum_id='" . Database::escape_string($id) . "' 
		AND item_properties.visibility<>2 AND item_properties.tool='" . TOOL_FORUM . "' $condition_session 
		ORDER BY forum_order ASC";
        ...
        $sql4 = "SELECT  post.post_id, post.forum_id, post.poster_id, post.poster_name, post.post_date, users.lastname, users.firstname
                 FROM $table_posts post, $table_users users WHERE forum_id=" . Database::escape_string($id) . " 
		 AND post.poster_id=users.user_id GROUP BY post.forum_id ORDER BY post.post_id ASC";
    }
...
//        }
        ...
        $rows_post = 0;
        $rs_count  = Database::query("SELECT COUNT(post_id) AS count_post FROM {$table_posts} WHERE session_id IN (0, {$id_session}) AND forum_id={$row3['forum_id']}");
        $rs_count  = Database::fetch_array($rs_count);
        $rows_post = (int) $rs_count['count_post'];

        $rows_thread = 0;
        $rs_count    = Database::query("SELECT COUNT(thread_id) AS count_thread FROM {$table_threads} WHERE session_id IN (0, {$id_session}) AND forum_id={$row3['forum_id']}");
        $rs_count    = Database::fetch_array($rs_count);
        $rows_thread = (int) $rs_count['count_thread'];
                
        $forum_list[$row3['forum_id']]['number_of_posts']   = $rows_post;
        $forum_list[$row3['forum_id']]['number_of_threads'] = $rows_thread;
    }
...
function store_thread($values) {
    ...
    if ($upload_ok) {
        ...
        $sql = "INSERT INTO $table_posts (post_title, post_text, thread_id, forum_id, poster_id, poster_name, post_date, post_notification, post_parent_id, visible, session_id) 
		VALUES ('" . $clean_post_title . "',
		'" . Database::escape_string(Security::remove_XSS(stripslashes(api_html_entity_decode($values['document_content'])), COURSEMANAGERLOWSECURITY)) . "',
		...
		'" . Database::escape_string($visible) . "', '" . $_SESSION['id_session'] . "')";
        Database::query($sql, __FILE__, __LINE__);
...
//        $sql = "UPDATE $table_threads SET thread_last_post='" . api_get_user_id() . "'  WHERE thread_id='" . $last_thread_id . "'";
        ...
        if ($current_forum['approval_direct_post'] == '1' AND !api_is_allowed_to_edit(null, true)) {
            ...
            $urlForumView = api_get_path(WEB_PATH) . 'main/forum/viewforum.php?' . api_get_cidreq() . '&forum=' . $values['forum_id'] . '&gidReq=' . $_SESSION['toolgroup'] . '&origin=' . $origin;
        } else {
            $message.=get_lang('ReturnTo') . ' <a href="viewforum.php?' . api_get_cidreq() . '&amp;forum=' . $values['forum_id'] . '&amp;gidReq=' . $_SESSION['toolgroup'] . '&amp;origin=' . $origin . '">' . get_lang('Forum') . '</a><br />';
        ...
        $returnArray['response'] = '<div class="confirmation-message rounded">' . $message . '</div>';
        $returnArray['url'] = $urlForumView;
    } else {
        //echo get_lang('UplNoFileUploaded');
        $returnArray['response'] = get_lang('UplNoFileUploaded');
        $returnArray['url'] = $urlForumView;
    }
    return $returnArray;
}
...
function show_add_post_form($action = '', $id = '', $form_values = '') {
    ...
    $form = new FormValidator('thread', 'post', api_get_self() . '?' . api_get_cidreq() . '&amp;forum=' . Security::remove_XSS($my_forum) . '&amp;gradebook=' . $gradebook . '&thread=' . Security::remove_XSS($my_thread) . '&post=' . Security::remove_XSS($my_post) . '&amp;action=' . Security::remove_XSS($my_action) . '&amp;origin=' . $origin);
    ...
    $renderer->setElementTemplate('<div class="row"><div style="width:100%;float:right;">{element}</div></div>', 'document_content');
    $form->addElement('html_editor', 'document_content', get_lang('Text'), null, api_is_allowed_to_edit(null, true) ? array('ToolbarSet' => 'ForumStudent', 'Width' => '100%', 'Height' => '300') : array('ToolbarSet' => 'ForumStudent', 'Width' => '100%', 'Height' => '300', 'UserStatus' => 'student')
    );
...
//        $form->addElement('checkbox', 'thread_qualify_gradebook', '', get_lang('QualifyThreadGradebook'), 'onclick="javascript:if(this.checked==true){document.getElementById(\'options_field\').style.display = \'block\';}else{document.getElementById(\'options_field\').style.display = \'none\';}"');
    ...
    $form->addElement('file', 'user_upload', get_lang('FileName'), 'id=fileattachforum');
    $form->addElement('textarea', 'file_comment', get_lang('FileComment'), array('rows' => 4, 'cols' => 34, 'style'=> 'width:315px'));
    ...
    if (!empty($form_values)) {
        ...
        $defaults['document_content'] = prepare4display(Security::remove_XSS($form_values['document_content']));
    ...
    if ($form->validate()) {
        ...
        if ($forum_setting['show_thread_iframe_on_reply'] and $action <> 'newthread') {
            echo '<div class="row">
					...
					<div class="formw">';
            echo "<iframe style=\"border: 1px solid black\" src=\"iframe_thread.php?forum=" . Security::remove_XSS($my_forum) . "&id_session=" . Security::remove_XSS($_GET['id_session']) . "&thread=" . Security::remove_XSS($my_thread) . "#" . Security::remove_XSS($my_post) . "\" width=\"100%\"></iframe>";
            echo '	</div>
				</div>';
        }
    }
    ...
    if ($upload_ok) {
        ...
        $sql = "INSERT INTO $table_posts (post_title, post_text, thread_id, forum_id, poster_id, poster_name, post_date, post_notification, post_parent_id, visible, session_id) 
				VALUES ('" . Database::escape_string(Security::remove_XSS($values['post_title'])) . "',
						'" . Database::escape_string(isset($values['document_content']) ? Security::remove_XSS(stripslashes(api_html_entity_decode($values['document_content'])), COURSEMANAGERLOWSECURITY) : null) . "',
        ...
        $urlForumView = api_get_path(WEB_PATH) . 'main/forum/viewthread.php?' . api_get_cidreq() . '&forum=' . $values['forum_id'] . '&thread=' . $values['thread_id'] . '&origin=' . $origin;
        ...
        $returnArray['response'] = get_lang('UplNoFileUploaded') . " " . get_lang('UplSelectFileFirst');
        $returnArray['url'] = $urlForumView;
    }
    return $returnArray;
}
...
function show_edit_post_form($current_post, $current_thread, $current_forum, $form_values = '', $id_attach = 0) {
    ...
    $form->applyFilter('post_title', 'html_filter');
    $renderer->setElementTemplate('<div class="row"><div style="width:100%;float:right;">{element}</div></div>', 'document_content');
    $form->addElement('html_editor', 'document_content', get_lang('Text'), null, api_is_allowed_to_edit(null, true) ? array('ToolbarSet' => 'ForumStudent', 'Width' => '100%', 'Height' => '400') : array('ToolbarSet' => 'ForumStudent', 'Width' => '100%', 'Height' => '400', 'UserStatus' => 'student')
    );
    //$form->applyFilter('post_text', 'html_filter');
    ...
    $defaults['document_content'] = prepare4display($current_post['post_text']);
...
function store_edit_post($values) {
    ...
    $sql = "UPDATE $table_posts SET post_title='" . Database::escape_string(Security::remove_XSS($values['post_title'])) . "',
				post_text='" . Database::escape_string(Security::remove_XSS(stripslashes(api_html_entity_decode($values['document_content'])), COURSEMANAGERLOWSECURITY)) . "',
    ...
    $urlForumView = api_get_path(WEB_PATH) . 'main/forum/viewthread.php?' . api_get_cidreq() . '&forum=' . Security::remove_XSS($_GET['forum']) . '&gidReq=' . $_SESSION['toolgroup'] . '&origin=' . $origin . '&gradebook=' . $gradebook . '&thread=' . $values['thread_id'];
    api_session_unregister('formelements');
    api_session_unregister('origin');
    api_session_unregister('breadcrumbs');
    api_session_unregister('addedresource');
 ...
 * @version february 2006, dokeos 1.8
 */
function display_user_link($user_id, $name, $origin = '', $type = 'page') {
    if ($type == 'popup') {
        if ($user_id <> 0) {
            return '<a href="#" onclick="showUserInfo(' . $user_id . ')">' . $name . '</a>';
        } else {
            return $name . ' (' . get_lang('Anonymous') . ')';
        }
    } else {
        if ($user_id <> 0) {
            return '<a href="../user/userInfo.php?uInfo=' . $user_id . '&' . api_get_cidreq() . '&origin=forum   " ' . (!empty($origin) ? 'target="_self"' : '') . '>' . $name . '</a>';
        } else {
            return $name . ' (' . get_lang('Anonymous') . ')';
        }
    }
 ...
 */
function display_user_image($user_id, $name, $origin = '', $type = 'page') {
    if ($type == 'popup') {        //api_is_anonymous();
        if (api_is_anonymous()) {
            $link = '<a href="#">';
        } else {
            $link = '<a onclick="showUserInfo(' . $user_id . ')" href="#">';
        }
    } else {
...
function get_whats_new() {
    ...
    //api_session_unregister('last_forum_access');
...
function move_thread_form() {
    ...
    $htmlcontent .= '	<div class="row">
		<div class="label">' . get_lang('MoveTo') . '
		</div>
		<div class="formw">';
 ...
 */
function forum_search($page, $search) {
    global $origin;
    // initiate the object
    ...
    $form->addElement('text', 'search_term', get_lang('SearchTerm'), 'class="input_titles wdt1"', '');
    ...
    if ($search <> "") {
        ...
        display_forum_search_results($values['search_term'], $page, $search);
    } else {
        $form->display();
    }
}
 ...
 * @version march 2008, dokeos 1.8.5
 */
function display_forum_search_results($search_term, $page, $search) {
    ...
    $result = Database::query($sql, __FILE__, __LINE__);
    $total_num_items = mysql_num_rows($result);
    while ($row = Database::fetch_array($result, 'ASSOC')) {
    ...
    if ($page > 1) {
        echo "<a href='forumsearch.php?" . api_get_cidreq() . "&page=1&search=$search_term'> << " . get_lang('FirstPage') . "&nbsp;&nbsp;</a>";
    }
    if ($page > 1) {
        echo "<a href='forumsearch.php?" . api_get_cidreq() . "&page=" . ($page - 1) . "&search=$search_term'> < " . get_lang('PreviousPage') . "&nbsp;&nbsp;</a>";
    }
    if ($previous) {
        echo "&nbsp...&nbsp";
    }
    for ($first_page; $first_page <= (int) $last_page; $first_page++) {
        if ($first_page == $page) {
            echo "<b>&nbsp$page&nbsp</b>";
        } else {
            echo "<a href='forumsearch.php?" . api_get_cidreq() . "&page=$first_page&search=$search_term'> &nbsp$first_page&nbsp </a>";
        }
    }
    if ($next) {
        echo "&nbsp...&nbsp";
    ...
    if ($page < $total_page) {
        echo "<a href='forumsearch.php?" . api_get_cidreq() . "&page=" . ($page + 1) . "&search=$search_term'>&nbsp;&nbsp;" . get_lang('NextPage') . " > </a>";
    }
    if ($page < $total_page) {
        echo "<a href='forumsearch.php?" . api_get_cidreq() . "&page=$total_page&search=$search_term'>&nbsp;&nbsp;" . get_lang('LastPage') . " >> </a>";
    }
    echo "</div>";
    echo '<ul>';
    $count = 1;
...
function paginator_forum($search) {
    global $table_categories, $table_forums, $table_threads, $table_posts;
    global $origin;
    $sql = "SELECT cat.cat_id,cat.cat_title,cat.cat_comment,forum.forum_id,forum.forum_title,forum.forum_comment,posts.post_id,posts.post_title,posts.post_text,posts.thread_id,item_properties.visibility,posts.visible FROM $table_categories cat LEFT JOIN $table_forums forum ON cat.cat_id=forum.forum_category LEFT JOIN  $table_posts posts ON forum.forum_id = posts.forum_id
			INNER JOIN $tbl_item_property item_properties ON item_properties.ref=forum.forum_id	WHERE (" . implode(' AND ', $search_restriction) . " AND item_properties.visibility=1 AND item_properties.tool='" . TOOL_FORUM . "') ";
...
function search_link() {
    ...
    if ($origin != 'learnpath') {
        $return = '<a href="forumsearch.php?' . str_replace('&', '&amp;', api_get_cidreq()) . '&amp;action=search&amp;origin=' . $origin . '"> ' . Display::return_icon('pixel.gif', get_lang('Search'), array('class' => 'toolactionplaceholdericon toolactionsearch')) . ' ' . get_lang('Search') . '</a>';
        if (!empty($_GET['search'])) {
            $returnx = Security::remove_XSS($_GET['search']) . ' ';
            $url = api_get_self() . '?';
            ...
            $return .= '<a href="' . $url . '">' . Display::return_icon('pixel.gif', get_lang('RemoveSearchResults'), array('class' => 'toolactionplaceholdericon toolactiondelete_32')) . $returnx . '</a>';
        }
    }
    return $return;
}
...
function show_columns_titles() {
    ...
    if (api_is_allowed_to_edit())
        $output .= "\t\t<th width='5%'>" . get_lang('Move') . "</th>\n";
...
function get_threads_data($from, $number_of_items, $column, $direction) {
    ...
    $session_id = api_get_session_id();
    $session_condition = 'AND t.session_id= '.$session_id;
    if($session_id > 0){
        $session_condition = 'AND (t.session_id= '.$session_id.' OR t.session_id = 0)';
    }
    ...
    $data = array();
    $visibility = is_allowed_to_edit() ? ' AND ip.visibility <> 2' : ' AND ip.visibility = 1';
    $sql = "SELECT t.thread_id, t.thread_title as col1, t.thread_replies as col2, t.thread_views as col3,
            t.thread_sticky, t.thread_poster_name, t.thread_date, ip.visibility, ip.id_session,
            concat(u.firstname,' ',u.lastname) as col4, u.user_id, post.post_id, post.post_title, 
            post.post_text, post.forum_id, post.poster_id, post.poster_name, post.post_date, 
            post.post_notification, post.post_parent_id, post.visible, last_poster_users.firstname as last_poster_firstname , 
            last_poster_users.lastname as last_poster_lastname, last_poster_users.user_id as last_poster_user_id, 
            t.locked as locked FROM $table_threads t INNER JOIN $table_item_property ip ON t.thread_id=ip.ref 
            $visibility AND ip.tool='" . TABLE_FORUM_THREAD . "' LEFT JOIN $table_users u ON t.thread_poster_id=u.user_id 
            LEFT JOIN $table_posts post ON t.thread_last_post = post.post_id LEFT JOIN $table_users last_poster_users 
            ON post.poster_id= last_poster_users.user_id WHERE t.forum_id='" . $forum_id . "' $session_condition";
    ...
    if (Database::num_rows($rs) > 0) {
        ...
        while ($row = Database::fetch_array($rs, 'ASSOC')) {
            ...
            $result = Database::query("SELECT COUNT(post_id) AS count_post FROM " . $table_posts . " WHERE session_id=" . $id_session . " 
                               AND forum_id=" . $forum_id . " AND thread_id=" . $row['thread_id'] . "");
            $result = Database::fetch_array($result);
            $rows_post = $result['count_post'];
            if ($id_session == 0)
                $rows_post--;
            ...
            if (api_is_allowed_to_edit(false, true) OR !($row['thread_replies'] == '0' && $row['visible'] == '0')) {
                ...
                if (is_array($my_whatsnew_post_info) and !empty($my_whatsnew_post_info)) {
                    ...
                    // $col[0] = icon('../img/forumthread_new_22.png');
                    $col[0] = Display::return_icon('pixel.gif', '', array('class' => 'toolactionplaceholdericon toolactionpost'));
                }

                $col[1] = "<a href=\"viewthread.php?" . api_get_cidreq() . "&gradebook=" . Security::remove_XSS($_GET['gradebook']) . "&forum=" . Security::remove_XSS($my_forum) . "&origin=" . $origin . "&thread=" . $row['thread_id'] . $origin_string . "&views=" . $row['col3'] . "&search=" . Security::remove_XSS(urlencode($my_search)) . "\" " . class_visible_invisible($row['visibility']) . ">" . prepare4display($row['col1']) . "</a>";
                ...
                if ($origin != 'learnpath') {
                    $col[4] = CourseManager::is_user_subscribed_in_course($row['last_poster_user_id'], api_get_course_id()) ? sprintf("<a id='user_id_%s' class='user_info' href='javascript:void(0);'>%s</a>", $row['last_poster_user_id'], $name) : $name; //display_user_link($row['user_id'], $row['col4']);
                } else {
                    $col[4] = $row['col4'];
                }
                ...
                if (($row['visible'] == '1' OR api_is_allowed_to_edit(false, true)) && $origin != 'learnpath') {
                    $date = TimeZone::ConvertTimeFromServerToUser(api_get_user_id(), strtotime($row['thread_date'])); //. display_user_link($row['last_poster_user_id'], $name);
                    $last_post = date("d-m-Y H:i:s", strtotime($date)) . " " . get_lang('By') . ' ';
                    $last_post.= CourseManager::is_user_subscribed_in_course($row['last_poster_user_id'], api_get_course_id()) ? sprintf("<a id='user_id_%s' class='user_info' href='javascript:void(0);'>%s</a>", $row['last_poster_user_id'], $name) : $name;
                    ...
                    $date = TimeZone::ConvertTimeFromServerToUser(api_get_user_id(), $last_post_row['post_date']); //. display_user_link($last_post_row['poster_id'], $name);
                    $last_post = date("d-m-Y H:i:s", strtotime($date)) . " " . get_lang('By') . ' ';
                    $last_post.= CourseManager::is_user_subscribed_in_course($row['last_poster_user_id'], api_get_course_id()) ? sprintf("<a id='user_id_%s' class='user_info' href='javascript:void(0);'>%s</a>", $row['last_poster_user_id'], $name) : $name;
                } else {
                    $last_post_sql = "SELECT post.*, user.firstname, user.lastname FROM $table_posts post, $table_users user WHERE post.poster_id=user.user_id AND visible='1' AND thread_id='" . $row['thread_id'] . "' ORDER BY post_id DESC";
                    ...
                    $last_post_row = Database::fetch_array($last_post_result);
                    $name = api_get_person_name($last_post_row['firstname'], $last_post_row['lastname']);
                    $last_post = TimeZone::ConvertTimeFromServerToUser(api_get_user_id(), strtotime($last_post_row['post_date'])) . " " . get_lang('By') . ' ' . $name;
                }
                ...
                if ($origin != 'learnpath') {
                    ...
                    if (api_is_allowed_to_edit(false, true) && !(api_is_course_coach() && $current_forum['session_id'] != $_SESSION['id_session']) AND !($row['id_session'] == 0 && intval($_GET['id_session']) != 0)) {
                        $actions .= "<center>";
                        ...
                        $link = api_get_self() . "?" . api_get_cidreq() . "&amp;forum=" . Security::remove_XSS($forum_id) . "&amp;action=delete&amp;content=thread&amp;gidReq=" . $_SESSION['toolgroup'] . "&amp;id=" . $row['thread_id'] . $origin_string;
                        $title = get_lang("ConfirmationDialog");
                        $text = get_lang("ConfirmYourChoice");
                        $actions .= "<a href='javascript:void(0);' onclick='Alert_Confim_Delete(\"" . $link . "\",\"" . $title . "\",\"" . $text . "\");'>" . Display::return_icon('pixel.gif', get_lang('Delete'), array('class' => 'actionplaceholdericon actiondelete')) . "</a>&nbsp;&nbsp;&nbsp;\n";
