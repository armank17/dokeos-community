...
foreach ($rows as $post)
{
	...
	else
	{
		if ($post['visible']=='0')
		{
			$class=' class="breadcrumb_invisible"';
		}
		else
		{
			$class='';
...
{
	echo '<b><span class="breadcrumb_invisible">'.$first_img.' '.$first_message.'</b></span>';
	echo '<b><span class="breadcrumb_invisible">'.$prev_img.' '.$prev_message.'</b></span>';
}
...
{
	echo '<b><span class="breadcrumb_invisible">'.$next_message.' '.$next_img.'</b></span>';
	echo '<b><span class="breadcrumb_invisible">'.$last_message.' '.$last_img.'</b></span>';
}
echo '</center>';
...
if (api_get_course_setting('allow_user_image_forum')) {echo '<br />'.display_user_image($rows[$display_post_id]['user_id'],$name, $origin,'popup').'<br />';	}
echo display_user_link($rows[$display_post_id]['user_id'], $name, $origin,'popup').'<br />';
echo TimeZone::ConvertTimeFromServerToUser(api_get_user_id(), strtotime($rows[$display_post_id]['post_date'])) .'<br /><br />';
// get attach id
