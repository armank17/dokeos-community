...
$current_thread = get_thread_information($thread_id); // note: this has to be validated that it is an existing thread
...
if (!empty($_SESSION['toolgroup'])) {
    ...
    $interbreadcrumb[] = array("url" => "viewthread.php?forum=" . Security::remove_XSS($_GET['forum']) . "&gradebook=" . $gradebook . "&thread=" . Security::remove_XSS($thread_id), "name" => prepare4display($current_thread['thread_title']));
...
if ($my_message) {
    ...
    $_SESSION["display_confirmation_message"] = get_lang($my_message);
}
if ($message == 'PostDeletedSpecial') {
    echo'<script>
            location.href="' . api_get_path(WEB_CODE_PATH) . 'forum/viewforum.php?cidReq=' . $_course[id] . '&forum=' . Security::remove_XSS($_GET['forum']) . '"
         </script>';
}
...
if ($my_message <> 'PostDeletedSpecial') {
    ...
    $result = Database::query("SELECT thread_views FROM " . Database::get_course_table(TABLE_FORUM_THREAD) . " 
                               WHERE thread_id=" . $thread_id . "");
    $result = Database::fetch_array($result);
    if ($result['thread_views'] == $_REQUEST['views'])
    ...
    if ($origin != 'learnpath') {
        if ($_REQUEST['id_session'] != '') {
            echo '<a href="index.php?cidReq=' . $_course[id] . '&id_session=' . Security::remove_XSS($_REQUEST['id_session']) . '&gradebook=' . $gradebook . '">' . Display::return_icon('pixel.gif', get_lang('BackToForumOverview'), array('class' => 'toolactionplaceholdericon toolactionback')) . get_lang('BackToForumOverview') . '</a>';
            echo '<a href="viewforum.php?id_session=' . Security::remove_XSS($_REQUEST['id_session']) . '&forum=' . Security::remove_XSS($_GET['forum']) . '&gidReq=' . $_SESSION['toolgroup'] . '">' . Display::return_icon('pixel.gif', get_lang('BackToForum'), array('class' => 'toolactionplaceholdericon toolactionforum')) . get_lang('BackToForum') . '</a>';
        } else {
            echo '<a href="index.php?cidReq=' . $_course[id] . '&gradebook=' . $gradebook . '">' . Display::return_icon('pixel.gif', get_lang('BackToForumOverview'), array('class' => 'toolactionplaceholdericon toolactionback')) . get_lang('BackToForumOverview') . '</a>';
    ...
    if ($current_forum_category['locked'] == 0 AND $current_forum['locked'] == 0 AND $current_thread['locked'] == 0 OR api_is_allowed_to_edit(false, true)) {
        ...
        if ($_user['user_id'] OR ($current_forum['allow_anonymous'] == 1 AND !$_user['user_id'])) {
            ...
            if (!api_is_anonymous() && api_is_allowed_to_session_edit(false, true)) {
                echo '<a href="reply.php?' . str_replace('&', '&', api_get_cidreq()) . '&forum=' . Security::remove_XSS($_GET['forum']) . '&thread=' . Security::remove_XSS($thread_id) . '&action=replythread&origin=' . $origin . '">' . Display::return_icon('pixel.gif', get_lang('ReplyToThread'), array('class' => 'toolactionplaceholdericon toolactionsinvite')) . get_lang('ReplyToThread') . '</a>';
            }
            //new thread link
            if ((api_is_allowed_to_edit(false, true) && !(api_is_course_coach() && $current_forum['session_id'] != $_SESSION['id_session'])) OR ($current_forum['allow_new_threads'] == 1 AND isset($_user['user_id'])) OR ($current_forum['allow_new_threads'] == 1 AND !isset($_user['user_id']) AND $current_forum['allow_anonymous'] == 1)) {
                if ($current_forum['locked'] <> 1 AND $current_forum['locked'] <> 1) {
                    ...
                    /* 					if ( isset($_GET['gradebook']) && $_GET['gradebook']!=""){
                      $info_thread=get_thread_information($thread_id);
    ...
    /* 	if ($origin != 'learnpath') {
      $my_url = '<a href="viewthread.php?'.api_get_cidreq().'&forum='.Security::remove_XSS($_GET['forum']).'&origin='.$origin.'&gradebook='.$gradebook.'&thread='.Security::remove_XSS($thread_id).'&search='.Security::remove_XSS(urlencode($my_search));
    ...
    echo'<script type="text/javascript">
        ...
        $.ajax({
            url: "' . api_get_path(WEB_PATH) . 'main/inc/ajax/user_manager.ajax.php?a=get_user_info&user_id="+user_id,
    ...
    if ($origin != 'learnpath') {
        if($current_forum_category['cat_id']==1)
            echo '<span class="forum_low_description">' . prepare4display(get_lang("ExampleForumCategory")) . ' - ';
        else
        echo '<span class="forum_low_description">' . prepare4display($current_forum_category['cat_title']) . ' - ';
    }
    if($current_forum['forum_id']==1)
        echo prepare4display(get_lang('ExampleForum')) . '<br />';
    else
