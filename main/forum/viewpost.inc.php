...
if(isset($rows)){
	foreach ($rows as $row) {
		if ($row['status']=='0') {
			...
			$url_post = '<a href="javascript:void(0);" onclick="javascript:hidecontent(\'#post'.$row['post_id'].'\')"> '.get_lang('ViewComentPost').'</a> ';
		}
		...
		// The post message
		echo "\t<tr >\n";
		echo "\t\t<td class=\"$messageclass\" id=\"$messageid\">".prepare4display($row['post_text'])."</td>\n";
		echo "\t</tr>\n";
