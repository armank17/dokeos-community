...
if ($origin != 'learnpath') {
    ...
    if ($_REQUEST['id_session'] != '') {
        ...
        echo '<a href="viewthread.php?id_session=' . Security::remove_XSS($_REQUEST['id_session']) . '&forum=' . Security::remove_XSS($_GET['forum']) . '&gradebook=' . $gradebook . '&thread=' . Security::remove_XSS($_GET['thread']) . '">' . Display::return_icon('pixel.gif', get_lang('BackToThread'), array('class' => 'toolactionplaceholdericon toolactionsinvite')) . get_lang('BackToThread') . '</a>';
    } else {
...
if (!empty($current_forum_category['cat_title'])) {
    if($current_forum_category['cat_id']==1)
        echo '<span class="forum_low_description">' . prepare4display(get_lang("ExampleForumCategory")) . " - </span>";
    else
    echo '<span class="forum_low_description">' . prepare4display($current_forum_category['cat_title']) . " - </span>";
}
if($current_forum['forum_id'])
    echo '<span class="forum_low_description">' . prepare4display(get_lang('ExampleForum')) . '</span>';
else
...
if (!empty($values) AND isset($_POST['SubmitPost'])) {
    $returnValue = store_reply($values);
    $_SESSION["display_confirmation_message"] = get_lang('ReplyAdded');
    header("Location:" . $returnValue['url']);
    ob_end_flush();
}
