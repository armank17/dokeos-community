...
echo '<a href="index.php?'.api_get_cidreq().'&amp;gradebook='.$gradebook.'">'.Display::return_icon('pixel.gif',get_lang('BackToForumOverview'),array('class' => 'toolactionplaceholdericon toolactionback')).get_lang('BackToForumOverview').'</a>';
echo '<a href="viewforum.php?cidReq='.$_course[id].'&amp;forum='.Security::remove_XSS($_GET['forum']).'&amp;gidReq='.$_SESSION['toolgroup'].'">'.Display::return_icon('pixel.gif',get_lang('BackToForum'),array('class' => 'toolactionplaceholdericon toolactionforum')).get_lang('BackToForum').'</a>';
echo '</div>';
...
if (!empty($values) and isset($values['SubmitPost'])) {
	...
	$returnValue = store_thread($values);
        $_SESSION["display_confirmation_message"] = get_lang('ForumThreadAdded');          
