...
foreach ($rows as $post) {
	...
	if (api_get_course_setting('allow_user_image_forum')) {
		echo '<br />'.display_user_image($post['user_id'],$name,$origin,'popup').'<br />';
	}
	echo display_user_link($post['user_id'], $name, $origin,'popup').'<br />';
	echo TimeZone::ConvertTimeFromServerToUser(api_get_user_id(), strtotime($post['post_date'])) .'<br /><br />';
	// get attach id
	...
	if ($post['post_notification']=='1' AND $post['poster_id']==$_user['user_id']) {
		$post_image.=icon('../img/forum_na.gif',get_lang('YouWillBeNotified'));
	}
	// The post title
