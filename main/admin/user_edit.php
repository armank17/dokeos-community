...
$htmlHeadXtra[] = '
<script type="text/javascript" src="' . api_get_path(WEB_LIBRARY_PATH) . 'javascript/jquery.validate.js"></script>    
<script type="text/javascript">
...
//$form->addElement('html', '<div class="row"><div class="label"></div><div class="formw" style="max-width: 50%;"><div id="progress_user" style="height:30px; margin-bottom:10px; border-radius:5px; text-align:center; line-height:28px; font-weight:bold; font-size:14px;"></div></div></div>');
...
//check to delete anonymous of list for status of learner
foreach ($status as $id => $value) {
    if ($value == get_lang('Anonymous')) {
        unset($status[$id]);
    }
}
...
if ($user_id != $_SESSION['_uid']) {
    ...
    $display = ($user_data['status'] == STUDENT || !isset($user_data['status'])) ? 'none' : 'block';
...
if (!$user_data['platform_admin']) {
    ...
    $form->addElement('radio', 'active', get_lang('ActiveAccount'), get_lang('Active'), 1, 'class='.(intval($user_data['active']) == 0 && !is_portal_attribute_valid('actived_users')?'sas-attribute-blocked':''));
...
$expiration_date = date("d-m-Y h:i:00 a", strtotime($user_data['expiration_date']));
if ($user_data['expiration_date'] == '0000-00-00 00:00:00') {
...
if ($form->validate()) {
    ...
    $platform_admin = ($status == STUDENT) ? 0 : intval($user['platform_admin']);
    ...
    if ($expiration_date != '0000-00-00 00:00:00') {
        $expiration_date_e = date("Y-m-d H:i:m", strtotime($expiration_date));
    } else {
        $expiration_date_e = $expiration_date;
    }

    UserManager::update_user($user_id, $firstname, $lastname, $username, $password, $auth_source, $email, $status, $official_code, $phone, $picture_uri, $expiration_date_e, $active, null, $hr_dept_id, null, $language, null, null, $timezone);
