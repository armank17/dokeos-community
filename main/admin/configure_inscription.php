...
$language_id = (api_get_language_id($lang) == NULL)? 0 : api_get_language_id($lang);
...
Display::display_header_admin_of_portal(7,$lang,$nodeId);
//echo '<div class="actions">';
//echo '<div class="float_l">';
//
////echo '<a href="'.api_get_self().'?action=edit_top">'.Display::return_icon('pixel.gif', get_lang("EditMessage"), array('class' => 'toolactionplaceholdericon tooledithome')).get_lang('EditMessage').'</a>';
...
// start the content div
echo '<div id="content">';
echo '<div class="title-dokeos-register">'.get_lang('ConfigureInscription').'</div>';// display the tool title
// api_display_tool_title($tool_name);
...
if (get_setting('allow_registration_as_teacher') <> 'false') {
     $form->addElement('checkbox', 'status', get_lang('Status'), null, array('class'=>'status','disabled' => 'disabled' ));
