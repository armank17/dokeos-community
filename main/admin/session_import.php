		...
		if ($file_type == 'xml') {
					...
					foreach ($root->Session as $node_session) {
						...
						foreach ($node_session->User as $node_user) {
							...
							if ($user_id !== false) {
								...
								$user_counter++;
                                                        if ($send_mail) {
                                                                SessionManager::send_email_notification_to_user_reg_session($user_id, $session_id);
                                                        }
							}
						}
						...
						foreach ($node_session->Course as $node_course) {
							...
							if (CourseManager::course_exists($course_code)) {
								...
								$inserted_in_course[$course_code] = $course_info['title'];
                                                                ...
                                                                // adding tools ---------------------------------------------------------------------------------------------------------------- /
                                                                $sql            = "SELECT db_name FROM $tbl_course WHERE code LIKE '$course_code'";
                                                                $res_course     = Database::query($sql);
                                                                $course_db_name = Database::fetch_array($res_course);                                                                
                                                                $tool_table     = Database::get_course_table(TABLE_TOOL_LIST, $course_db_name['db_name']);
                                                                
                                                                $rs_tool = Database::query("SELECT * FROM $tool_table WHERE session_id = 0");
                                                                while($row_tool = Database::fetch_array($rs_tool)){
                                                                ...
                                                                $names = array();
                                                                foreach($node_course->Tool as $tool){
                                                                    $names[] = "'". $tool ."'";
                                                                }
                                                                $names = implode(',', $names);
                                                                
                                                                $sql   = "UPDATE $tool_table SET visibility = 1 WHERE session_id = $session_id AND name IN (". $names .")";
                                                                Database::query($sql);
							}
			...
			if (!api_strstr($content[0], ';')) {
				...
				foreach ($sessions as $enreg) {
					...
					$session_name = api_eregi_replace('"', '', $enreg['SessionName']);
					$date_start = api_eregi_replace('"', '', $enreg['DateStart']);
					$date_end = api_eregi_replace('"', '', $enreg['DateEnd']);
					...
					if (!$updatesession) {
						...
						$session_counter++;
                                        ...
                                        $enreg['Users'] = api_eregi_replace('"', '', $enreg['Users']);
						...
						foreach ($users as $user) {
							...
							if ($user_id !== false) {
								...
								$user_counter++;
                                        ...
                                        $enreg['Courses'] = api_eregi_replace('"', '', $enreg['Courses']);
						...
						if (CourseManager::course_exists($course_code)) {
							...
							while ($tok !== false) {
							    ...
							    $tok = strtok("[]");
                                                        ...
                                                        preg_match_all('/\[(.*?)\]/', $course, $course_split);                                                        
                                                        $course_split = $course_split[1];
							
							$course_coachs = explode(",", $course_split[0]);
							$course_users  = explode(",", $course_split[1]);
                                                        $course_tools  = explode(",", $course_split[2]);
