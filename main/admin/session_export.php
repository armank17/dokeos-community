		...
		while($row=Database::fetch_array($result))
		{
			...
			if($cvs){
				$add.= $row['name'].';'.$row['username'].';'.$row['date_start'].';'.$row['date_end'].';';
                                //$row['visibility'].';'.$row['session_category'].';';
			}
			else {
				$add = "\t<Session>\n"
			...
			while($rowUsers = Database::fetch_array($rsUsers)){
				if($cvs){
                                        /* Add one by one the usernames of the session */
			...
			$sql = "SELECT DISTINCT 
                                ...
                                $tbl_course.db_name
                                FROM $tbl_course
                                INNER JOIN $tbl_session_course_user ON (
                                        $tbl_course.code = $tbl_session_course_user.course_code
			...
			while($rowCourses = Database::fetch_array($rsCourses)){
                                // get tool course table name 
                                $tbl_course_tool = Database::get_course_table(TABLE_TOOL_LIST, $rowCourses['db_name']);
				...
				while($rowUsersCourse = Database::fetch_array($rsUsersCourse)){
					...
					if($cvs){
						...
						$courses .= "\t\t\t<User>$rowUsersCourse[username]</User>\n";
                                ...
                                // Curses Tools ------------------------------------------------------------------------ /
				$sql = "SELECT tool.name
                                        FROM $tbl_course_tool tool
                                        WHERE
                                          tool.visibility > 0
                                          AND tool.session_id = ". $row['id'];
				$rsToolsCourse = Database::query($sql,__FILE__,__LINE__);
				$toolscourse = '';
                                while($rowToolsCourse = Database::fetch_array($rsToolsCourse)){
                                    if($cvs){
                                            $toolscourse .= str_replace(';',',',$rowToolsCourse['name']).',';
                                    }
                                    else {
                                            $courses .= "\t\t\t<Tool>$rowToolsCourse[name]</Tool>\n";
                                    }
                                }
                                // ------------------------------------------------------------------------------------- /
					...
					$courses .= $userscourse.']';
                                        ...
                                        if(!empty($toolscourse)){
						$toolscourse = api_substr($toolscourse , 0, api_strlen($toolscourse)-1);
                                                $courses .= '['. $toolscourse .']';
                                        }
                                        $courses .= '|';
				}
...
<tr>
  <td>&nbsp;</td>
  <td>
  <div class="pull-bottom"><button class="save" type="submit" name="name" value="<?php echo get_lang('ExportSession') ?>"><?php echo get_lang('ExportSession') ?></button></div>
  </td>
</tr>
</table>
</form>
