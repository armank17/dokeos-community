...
if (isset($_POST['SubmitAddNewLanguage'])) {
	...
	foreach ($check_information as $index_information => $value_information) {
		...
		if ($index_information=='original_name') {
			$message1[] = '"'.get_lang('OriginalName').'" '.'('.$original_name.')';
		}
		if ($index_information=='english_name') {
			$message1[] = '"'.get_lang('EnglishName').'" '.'('.$english_name.')';
		}
		if ($index_information=='isocode') {
			$message1[] = '"'.get_lang('PlatformCharsetTitle').'" '.'('.$isocode.')';
		}
        ...
        if(!empty($message1)){
                echo Display::display_error_message($message1,true,true,null,null, get_lang('AlreadyExists'));
        }
	if (strlen($original_name)>0 && strlen($english_name)>0 && strlen($isocode)>0) {
		if ($allow_insert_info===true && $language_id_exist===true) {
			$english_name=str_replace(' ','_',$english_name);
			$isocode=str_replace(' ','_',$isocode);
                        $str_info = array(get_lang('OriginalName').' : '.$original_name,get_lang('EnglishName').' : '.$english_name,get_lang('PlatformCharsetTitle').' : '.$isocode);
			...
			if ($mkdir_result===true) {
			  	add_sub_language($original_name,$english_name,$isocode,$sublanguage_available,$parent_id);
                                ...
                                echo Display::display_confirmation_message($str_info,true,true,null,null, get_lang('TheNewSubLanguageHasBeenAdd'));
			} else {
                                
                                echo Display::display_confirmation_message($str_info,true,true,null,null,get_lang('LanguageDirectoryNotWriteableContactAdmin'));
				  
			}
		} else {
			if ($language_id_exist===false) {
                                
				$message1 =  get_lang('LanguageParentNotExist');
                                 echo Display::display_confirmation_message($message1,true,true);
			}
		}
	} else {     
			$message1 =  get_lang('FormHasErrorsPleaseComplete');
                         echo Display::display_confirmation_message($message1,true,true);
	}
}
	...
	if (is_dir($path)) {
		...
		if ($rs===true) {
			SubLanguageManager::removed_sub_language($parent_id,$sub_language_id);
                        echo Display::display_confirmation_message(get_lang('TheSubLanguageHasBeenRemoved'),true,true);
