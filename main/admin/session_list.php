...
if ($action == 'delete') {
    ...
    SessionManager::deleteSessionEcommerce($idChecked);
...
//    echo '<h2 style="font-weight:bolder; text-transform:uppercase;">'.get_lang('AdvancedSearch').'</h2>';
    echo '<div class="secondary-actions-extra" style="height:55px;">';
    ...
    echo '<h2 style="font-weight:bolder; text-transform:uppercase; margin-bottom:-5px;">'.get_lang('ListSession').'</h2>';
    ...
    if (count($Sessions) == 0) {
        echo (isset($_GET['keyword_name'])) ? get_lang('NoSearchResults') : get_lang('ThereIsNotStillASession');
    } else {
        if ($num > $limit) {
            if ($page) {
                ?>
        ...
        <!--<tr>
            ...
            <tr>
                ...
                <th align="center"><a href="<?php echo api_get_self(); ?>?sort=date_start&amp;type=<?php echo $sort_link; ?>"><?php echo get_lang('StartDate'); ?></a></th>
          ...
          <!--  <th><a href="<!--?php echo api_get_self(); ?>?sort=visibility<!--?php echo $cond_url; ?>"><!--?php echo get_lang('Visibility'); ?></a></th>-->
            ...
            foreach ($Sessions as $key => $enreg) {
                ...
                <tr class="<?php echo $i ? 'row_odd' : 'row_even'; ?>">
                    ...
                    <td class="data_td"><?php echo api_htmlentities($date_start, ENT_QUOTES, $charset); ?></td>
                    <td class="data_td"><?php echo api_htmlentities($date_end, ENT_QUOTES, $charset); ?></td>
                    <td><?php echo api_htmlentities(api_get_person_name($enreg['firstname'], $enreg['lastname']), ENT_QUOTES, $charset); ?></td>
              <!--  <td><?php
                    switch (intval($enreg['visibility'])) {
                        case SESSION_VISIBLE_READ_ONLY: //1
...
<?php
                    $link = api_get_self().'?sort='.$sort.'&amp;action=delete&amp;idChecked='.$enreg['id'];
                    $title = get_lang("ConfirmationDialog");
                    $text = get_lang("ConfirmYourChoice");
                    ?>
                    <a href="javascript:void(0);" <?php echo "onclick='Alert_Confim_Delete(\"".$link."\",\"".$title."\",\"".$text."\");'"  ?>><?php echo Display::return_icon('pixel.gif', get_lang('Delete'), array('class' => 'actionplaceholdericon actiondelete')); ?></a>
                    </td>
                </tr>
                <?php
                $i = $i ? 0 : 1;
...
function export_csv_data() {
    ...
    $data[] = array(get_lang('NameOfTheSession'), get_lang('NumberOfCourses'), get_lang('StartDate'), get_lang('EndDate'), get_lang('Coach'));
