...
echo '<a href="'.api_get_path(WEB_CODE_PATH).'admin/access_url_edit_sessions_to_url.php">'.Display::return_icon('course_del.gif',get_lang('EditSessionToURL'),'').get_lang('EditSessionToURL').'</a>';
echo '</div>';
	...
	if ($form_sent == 1) {
		if ( count($sessions) == 0 || count($url_list) == 0) {
			//Display :: display_error_message(get_lang('AtLeastOneSessionAndOneURL'));
                        $_SESSION['display_error_message']=get_lang('AtLeastOneSessionAndOneURL');
			...
			Display :: display_confirmation_message(get_lang('SessionBelongURL'));
                        $_SESSION['display_confirmation_message']=get_lang('SessionBelongURL');
