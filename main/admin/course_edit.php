...
$form->addElement('radio', 'payment', get_lang('AttachToCatalogueOfProducts'), get_lang('AllowedToUnsubscribe'), 1);
...
if( $form->validate())
{
	...
	Database::query($sql, __FILE__, __LINE__);
        ...
        if($payment == '1'){
            ...
            //$courseEcommerce = array('wanted_code'=> Database :: escape_string($course_code) ,
             //       'date_end' => "FROM_UNIXTIME(".$expiration_date . ")" );
            ...
            $sql = "UPDATE {$tbl_ecommerce} SET status = {$status['invisible']} WHERE code = '{$course_code}'";
            Database::query($sql,__FILE__,__LINE__);
        }else{
            $tbl_ecommerce = Database::get_main_table( TABLE_MAIN_ECOMMERCE_ITEMS );
            $sql = "UPDATE {$tbl_ecommerce} SET status = {$status['invisible']} WHERE code = '{$course_code}'";
...
echo '<a href="course_list.php">'.Display::return_icon('pixel.gif', get_lang('Back'), array('class' => 'toolactionplaceholdericon toolactionback')).get_lang('Back').'</a>';
echo '</div>';
