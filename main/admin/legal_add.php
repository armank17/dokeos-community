...
if( $form->validate()) {
		...
		if ($check) {
			...
			if (isset($values['language'])){
				if($submit=='back') {
					...
					$defaults['type']=$type;
					$defaults['legal_content']=$content;
	...
	if (!isset($_POST['language'])) {
		...
		$defaults['legal_content']=$term_preview['legal_content'];
...
if (isset($_POST['language'])) {
//$form->addElement('html_editor', 'legal_content', null, null, array('ToolbarSet' => 'Basic', 'Width' => '100%', 'Height' => '250'));
	$form->addElement('static', $_POST['language']);
	$form->addElement('hidden', 'language',$_POST['language']);
	$form->add_html_editor('legal_content', get_lang('Content'), true, false, array('ToolbarSet' => 'terms_and_conditions', 'Width' => '100%', 'Height' => '250'));
	//$form->addElement('textarea', 'legal_content', get_lang('Content'),array('cols'=>'120','rows'=>'10'));
