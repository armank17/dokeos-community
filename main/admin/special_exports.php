...
Display::display_header_admin_of_portal(2);
...
if($export == true && $name != false) {
	Display::display_confirmation_message(get_lang('BackupCreated').'<br /><br /><a class="bottom-link" href="'.api_get_path(WEB_CODE_PATH).'course_info/download.php?archive='.urlencode($name).'&session=true">'.$name.'</a>', false, true);
	...
	if (isset ($_POST['backup_option']) && $_POST['backup_option'] == 'select_items') {
		...
		if($course === false){
			Display::display_error_message(get_lang('ErrorMsgSpecialExport'), true, true);
			form_special_export();
		} else {
			Display::display_normal_message(get_lang('ToExportSpecialSelect'), true, true);
...
function form_special_export(){
	...
	$renderer->setElementTemplate('<div>{element}</div> ');
        $form->addElement('header', '', get_lang('SpecialExports'));
