...
function search_users($needle, $type, $selected){
	...
	if (!empty($needle) && !empty($type)) {
		...
		if ($type == 'single') {
			...
			$sql = 'SELECT user_id, username, lastname, firstname '
                             ...
                             $sql .= (!empty($selected)) ? (' AND user_id NOT IN (' . $selected . ') ') : '';                             
                             $sql .= $order_clause;                           
		}
...
// Function used for validate what the div have content
function xajax_search_users_validate(){
    // Remove the class
    $("#ajax_list_users_single").removeClass("ajax_list_users_single");
    //If the div have content
    if($("#ajax_list_users_single").html()!="" && $("#user_add").val()!=""){
        // Add the class to div
        $("#ajax_list_users_single").addClass("ajax_list_users_single");
    }
}
...
<?php echo get_lang('FirstLetterUser'); ?> :
     <select name="firstLetterUser" onchange = "xajax_search_users(this.value, 'multiple', getSelected())" >
      <option value = "%">--</option>
      <?php
...
<tr>
  ...
  <div id="content_source">
  	  ...
  	  if (!($add_type=='multiple')) {
  	  	?>
		<input type="text" id="user_to_add" onkeyup="xajax_search_users(this.value,'single');xajax_search_users_validate();" />
  	  ...
  	  <div id="ajax_list_users_multiple">
          <!--[if IE 7]>
          <div class="selectt-ajax_list_courses_multiple" style="width:360px;"> 
          <![endif]-->       
	  <select id="origin_users" name="nosessionUsersList[]" multiple="multiple" size="15" style="width:360px;" onDblClick="moveItem(document.getElementById('origin_users'), document.getElementById('destination_users'))" onDblClick="moveItem(document.getElementById('origin_users'), document.getElementById('destination_users'))">
		<?php
  ...
  if ($ajax_search) {
  ?>
  	<button class="button-blue" type="button" onclick="remove_item(document.getElementById('destination_users'))" ><img src="<?php echo api_get_path(WEB_CSS_PATH).api_get_setting('stylesheets')?>/images/action/arrow-white-left.png"/></button>
  <?php
  } else {
  ?>
  	<button class="button-blue" type="button" onclick="moveItem(document.getElementById('origin_users'), document.getElementById('destination_users'))" onclick="moveItem(document.getElementById('origin_users'), document.getElementById('destination_users'))"><img src="<?php echo api_get_path(WEB_CSS_PATH).api_get_setting('stylesheets')?>/images/action/arrow-white-right.png"/></button>
	<br /><br />
	<button class="button-blue" type="button" onclick="moveItem(document.getElementById('destination_users'), document.getElementById('origin_users'))" onclick="moveItem(document.getElementById('destination_users'), document.getElementById('origin_users'))"><img src="<?php echo api_get_path(WEB_CSS_PATH).api_get_setting('stylesheets')?>/images/action/arrow-white-left.png"/></button>
	<?php
  }
  ?>
	<br /><br /><br /><br /><br /><br />
  ...
  <td align="center">
  <select id="destination_users" name="sessionUsersList[]" multiple="multiple" size="15" style="width:360px;" onDblClick="moveItem(document.getElementById('destination_users'), document.getElementById('origin_users'))" onDblClick="moveItem(document.getElementById('destination_users'), document.getElementById('origin_users'))">
...
<tr>
	<td colspan="3" align="right">
		...
		<?php
		if(isset($_GET['add'])) {
			echo '<button style="float:none;" class="save" type="button" value="" onclick="valide()" >'.get_lang('FinishSessionCreation').'</button>';
        } else {
			...
			echo '<button style="float:none;" class="save" type="button" value="" onclick="valide()" >'.get_lang('SubscribeUsersToSession').'</button>';
...
function getSelected() {
    var ret = new Array();
    var options = $('select#destination_users option');
    for (i = 0 ; i<options.length ; i++) {
