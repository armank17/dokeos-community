			...
			if ($num == 0) {
				...
				if (substr($url,strlen($url)-1, strlen($url))=='/') {
					...
					UrlManager::add($url.'/', $description, $active);
                                ...
                                MenuLinkManager::addMenuLinksToAccessUrlId($id);  // Add menulinks
                                TemplateManager::addTemplatesToAccessUrlId($id);  // Add templates
...
$form->addElement('textarea','description',get_lang('Description'),'class="WidthCusMultisite"', get_lang('Description'));
