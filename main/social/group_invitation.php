...
if (is_array($members) && count($members)>0) {
	...
	foreach ($members as &$member) {            
		...
		$member['image'] = '<img src="'.$picture['file'].'"  width="50px" height="50px"  />';                
                if(strlen($member['firstname'])>10){
                    $member['firstname']=substr($member['firstname'],0,7).'...';  
                }
                if(strlen($member['lastname'])>10){
                    $member['lastname']=substr($member['lastname'],0,7).'...';  
                }                                
	}        
	echo '<span class="social-groups-text1"><strong>'.get_lang('UsersAlreadyInvited').'</strong></span>';
	Display::display_sortable_grid('invitation_profile', array(), $members, array('hide_navigation'=>true, 'per_page' => 20), $query_vars, false, array(true, false, true,true));
}
