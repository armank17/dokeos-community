			...
			if (api_get_setting('allow_message_tool')=='true') {	
	            //normal message
		 		...
		 		<input id="txt_subject_id" name="title" type="text" style="width:450px;" value="<?php echo $subject; ?>"><br/>
		   		...
		   		<?php
                                $use_default_editor = api_get_setting('use_default_editor');
                                if($use_default_editor == 'Ckeditor'){
                                ...
                                $oCKeditor->BasePath = '../inc/lib/ckeditor/' ;
                                
                                $oCKeditor->editor('content', $message, $config);
                                }else{
				...
				<span id="filepaths"><div id="filepath_1"><input type="file" name="attach_1" size="20" title="<?php echo get_lang('Upload'); ?>" ></div></span>
                                <div id="link-more-attach"><a href="javascript://" onclick="return add_image_form()"><?php echo utf8_encode(get_lang('AddOneMoreFile')) ?></a>&nbsp;(<?php echo api_xml_http_response_encode(sprintf(get_lang('MaximunFileSizeX'),format_file_size(api_get_setting('message_max_upload_filesize')))) ?>)</div>		   				   				   		
		   		<br />
		   		<button class="save" onclick="if(validate_text_empty(this.form.title.value,'<?php echo get_lang('YouShouldWriteASubject')?>')){return false;}" type="submit" value="<?php echo api_xml_http_response_encode(get_lang('SendMessage')); ?>"><?php echo api_xml_http_response_encode(get_lang('SendMessage')) ?></button>
                                <div><span style="color:red">*</span><?php echo utf8_encode(get_lang('FieldRequired')) ?></div>
	<?php } ?>
	</dl>
</td>
</tr>
