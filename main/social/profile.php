...
if (USE_JQUERY_CORNERS_SCRIPT) {
	echo $s='<script type="text/javascript">$(document).ready( function(){
		  $(".rounded").corners();
		});</script>';
}
...
if (!empty($user_info['firstname']) || !empty($user_info['lastname'])) {
	echo '            <div><h3 class="title-profile">'.api_get_person_name($user_info['firstname'], $user_info['lastname']).'</h3></div>';
} else {
	//--- Basic Information
	echo '            <div><h3 class="title-profile">'.get_lang('Information').'</h3></div>';
}
...
if ($show_full_profile) {
	...
	if (is_array($extra_user_data) && count($extra_user_data)>0 ) {
		...
		$extra_information .= '<div><h3 class="title-profile">'.get_lang('ExtraInformation').'</h3></div>';
...
if ($show_full_profile) {
	...
	if ($number_friends != 0) {
		$friend_html.= '<div><h3 class="title-profile2">'.get_lang('SocialFriend').'</h3></div>';
...
if ($show_full_profile) {
	...
	if (is_array($results) && count($results) > 0) {
		...
		foreach ($results as $result) {
			...
			if ($result['relation_type'] == GROUP_USER_PERMISSION_ADMIN) {
				$icon = Display::return_icon('pixel.gif', get_lang('Admin'), array('style'=>'vertical-align:middle;margin-left: 5px; margin-top: -5px;','class'=>'actionplaceholdericon actionadmin_star'));
			...
			$item_name = '<div class="box_shared_profile_group_title title-group">'.$url_open.'<span class="social-groups-text1">'.api_strtoupper($name).'</span>'. $icon.$url_close.'</div>';
			...
			$result['picture_uri'] = '<div style="height:100px; float:left;"><div class="box_shared_profile_group_image"><img class="social-groups-image" src="'.$picture['file'].'" hspace="4" height="50" border="2" align="left" width="50" /></div></div>';
			$item_actions = '';
			if (api_get_user_id() == $user_id) {
				$item_actions = '<div class="box_shared_profile_group_actions"><a class="link-buttons" href="groups.php?id='.$id.'">'.get_lang('SeeMore').$url_close.'</div>';
			}
    	...
    	echo '<div class="social-box-main1">';
			echo '<div class="quiz_content_actions">';
				echo '<div class="social-box-content2">';
					echo '<div><h3 class="title-profile2">'.get_lang('MyGroups').'</h3></div>';
					$count_groups = 0;
					if (count($results) == 1 ) {
			...
			echo '<div class="quiz_content_actions">';
                ...
                if ($show_full_profile && $user_id == intval(api_get_user_id())) {
                    ...
                    if (!empty($announcements)) {
                        echo '<div align="left" class="social-menu-title" ><h3 class="title-profile2">'.get_lang('ToolAnnouncement').'</h3></div>';
                        echo '<div style="margin-bottom:15px">';
                            foreach ($announcements as $announcement) {
                                echo '<div style="margin-bottom:10px;">'.$announcement.'</div>';
                            }
                        echo '</div>';
                    }
                }
	...
	if ( is_array($list) ) {
		echo '<div class="social-box-main1">';
			echo '<div class=" quiz_content_actions">';
				echo '<div class="social-box-content2">';
					echo '<div><h3 class="title-profile2">'.api_ucfirst(get_lang('MyCourses')).'</h3></div>';
					echo '<div class="social-content-training" style="font-size:10px !important">';
	...
	if (!empty($user_feeds )) {
		echo '<div class="social-box-main1">';
			echo '<div class=" quiz_content_actions">';
				echo '<div class="social-box-content2">';
					echo '<div><h3 class="title-profile2">'.get_lang('RSSFeeds').'</h3></div>';
	    			echo '<div class="social-content-training" style="font-size:10px !important">'.$user_feeds.'</div>';
	    			//echo '<div class="clear"></div>';
				echo '</div>';
			echo '</div>';
		echo '</div>';
	...
	if (!empty($production_list) || !empty($file_list) || $count_pending_invitations > 0) {
		...
		if (!isset($_GET['u']) || (isset($_GET['u']) && $_GET['u']==api_get_user_id())) {
			if ($count_pending_invitations > 0) {
				echo '<div class="social-box-content2">';
				echo '<div><h3 class="title-profile2">'.get_lang('PendingInvitations').'</h3></div>';
		...
		if (!empty($production_list )) {
			echo '<div><h3 class="title-profile2">'.get_lang('MyProductions').'</h3></div>';
		...
		if (!empty($file_list)) {
			echo '<div><h3 class="title-profile2">'.get_lang('ImagesUploaded').'</h3></div>';
	...
	if (!empty($user_info['competences']) || !empty($user_info['diplomas']) || !empty($user_info['openarea']) || !empty($user_info['teach']) ) {
		echo '<div class=" quiz_content_actions" style="margin-top:10px;">';
		echo '<div class="social-box-content1" style="width:95%">';
		echo '<div><h3 class="title-profile2">'.get_lang('MoreInformation').'</h3></div>';
