...
//+----------------------------------------------------------------------+
//
// $Id: sample_ical.php 986 2007-11-09 13:03:13Z matthias $
// setting the global file that gets the general configuration, the databases, the languages, ...
include('../../inc/global.inc.php');
$this_section=SECTION_COURSES;

// include the agenda functions
include ('../agenda2.inc.php'); 

// getting all the agenda items of the user
$agenda_items=get_all_agenda_items();
...
foreach ($agenda_items as $year=>$year_agenda_items)
{
	if ($_GET['reverse']==1)
	{
		$year_agenda_items=array_reverse($year_agenda_items,1);
	}
	foreach($year_agenda_items as $month=>$month_agenda_items)
	{
		if ($_GET['reverse']==1)
		{
			$month_agenda_items=array_reverse($month_agenda_items,1);
		}			
		foreach($month_agenda_items as $day=>$day_agenda_items)
		{	
			if ($_GET['reverse']==1)
			{
			$day_agenda_items=array_reverse($day_agenda_items,1);
			}						
			//echo 'year:'.$year.'<br>month:'.$month.'<br>day:'.$day;
			foreach ($day_agenda_items as $key=>$individual_agenda_item)
			{
				$starttimestamp=mktime($individual_agenda_item['hour'],$individual_agenda_item['minute'],'00',$individual_agenda_item['month'],$individual_agenda_item['day'],$individual_agenda_item['year']);
				$endtimestamp = $starttimestamp+(60*60); 
				$iCal->addEvent(
								'', // Organizer
								$starttimestamp, // Start Time (timestamp; for an allday event the startdate has to start at YYYY-mm-dd 00:00:00)
								$endtimestamp, // End Time (write 'allday' for an allday event instead of a timestamp)
								'', // Location
								0, // Transparancy (0 = OPAQUE | 1 = TRANSPARENT)
								'', // Array with Strings
								$individual_agenda_item['text'], // Description
								$individual_agenda_item['title'], // Title
