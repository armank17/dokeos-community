...
class Dropbox_Work {
	...
	/*	$this->isOldWork = FALSE;
		...
		if ($this->isOldWork) {
			...
			$desired_filename = '/mindmaps/'.addslashes($this->title);
			
			$document_id = add_document($_course, $desired_filename,'file',addslashes($this->filesize),addslashes($this->title));
			if ($document_id)
			{
			//update document item_property
			$to_group_id = 0;
			$to_user_id = 0;
			api_item_property_update($_course,TOOL_DOCUMENT,$document_id,'MindmapCreated',addslashes($this->uploader_id),$to_group_id,$to_user_id,null,null,intval($_SESSION['id_session']));			
			}
			
		}*/

	//	$file_ext = explode(".",$this->title);
		$file_ext = pathinfo($this->title, PATHINFO_EXTENSION);
		$file_parts = pathinfo($this->title);

		if($file_ext == 'xmind')
		{
			$this->title = $file_parts['filename'].'.png';
		}
		$curdirpath = $_SESSION['curdirpath'];
		if(empty($curdirpath))
		{
		$desired_filename = '/mindmaps/'.addslashes($this->title);
		}
		...
		{
		$desired_filename = '/mindmaps/'.$curdirpath.'/'.addslashes($this->title);
		}
		...
		$result = Database::query("SELECT MAX(display_order) FROM  ".$tbl_documents);
		list ($orderMax) = Database::fetch_row($result);
		$order = $orderMax +1;

		$sql = "UPDATE $tbl_documents SET display_order = ".$order." WHERE id = ".$document_id;
	...
	/*	$sql="SELECT uploader_id, filename, filesize, title, description, author, upload_date, last_upload_date, cat_id
		...
		$sql = "SELECT prop.insert_user_id AS uploader_id, doc.path AS filename, doc.title AS title FROM $tbl_documents doc,$propTable prop WHERE doc.id = prop.ref AND prop.tool = '".TOOL_DOCUMENT."' AND doc.filetype = 'file' AND doc.id='".addslashes($id)."'";
...
class Dropbox_SentWork extends Dropbox_Work
{
	...
	function _createNewSentWork ($uploader_id, $title, $description, $author, $filename, $filesize, $recipient_ids) {
		...
		//	api_item_property_update($_course, TOOL_DROPBOX, $this->id, "DropboxFileAdded", $ownerid, NULL, $recipid) ;
			api_item_property_update($_course, TOOL_DOCUMENT, $this->id, "MindmapFileCreated", $ownerid, NULL, $recipid) ;
		}
	}
...
class Dropbox_Person
{
    ...
    * Deletes all the received categories and work of this person
	...
	function deleteReceivedWork ($id) {
		...
		$tbl_documents = Database::get_course_table(TABLE_DOCUMENT);
		Database::query("DELETE FROM ".$tbl_documents." WHERE id=".$id,__FILE__,__LINE__);
	}
