...
$htmlHeadXtra[] = "<script>
    $(function() {
        if (typeof FormData !== 'undefined')
            $('#dragandrophandler').DragandDrop('MINDMAP', 100, '" . api_get_path(WEB_PATH) . "main/document/uploadFiles.php?" . api_get_cidreq() . "&dir=/mindmaps');
        else {
            $('#dragandrophandler').remove();
            $('#dynamicInput').DinamicUpload('MINDMAP');
        }
    });
    </script>";
...
function display_addcategory_form($category_name = '', $id = '', $action) {
    ...
    echo '<div class="row"><div class="label"><span class="form_required">*</span> ' . get_lang('CategoryName') . '</div>
	  <div class="formw">';
    ...
    echo '<input type="text" name="category_name" value="' . Security::remove_XSS($category_name) . '" /></div></div>';

    echo '<div class="row"><div class="label"></div><div class="formw">
	  <button class="' . $class . '" type="submit" name="StoreCategory">' . $text . '</button></div></div>';
    echo '<div class="row"><div class="label"></div><div class="formw">
	  <span class="form_required">*</span> <small>' . get_lang('ThisFieldIsRequired') . '</small></div></div>';
    echo '</form>';
    echo '<div style="clear: both;"></div>';
}
...
function display_add_form() {
    ...
    <form method="post" action="<?php echo api_get_path(WEB_PATH); ?>main/document/uploadFiles.php?<?php echo api_get_cidreq(); ?>&dir=/mindmaps" enctype="multipart/form-data">
        <div class="row">
            <div class="label">
                <span class="form_required">* </span><?php echo get_lang('File') ?>:
            </div>
            <div class="formw">
                <input type="hidden" name="MAX_FILE_SIZE" value='<?php echo dropbox_cnf("maxFilesize") ?>' />
                <input type="file" name="file" style="opacity:0;width:100%;filter:alpha(opacity=0);position:absolute;border:none;margin:0px;padding:0px;top:0px;right:0px;cursor:pointer;height:40px;" <?php if (dropbox_cnf("allowOverwrite")) echo 'onChange="checkfile(this.value)"'; ?> title="<?php echo get_lang('Upload'); ?>" >
        ...
        <div class="row">
            <div class="formw">
                <div id="dynamicInput" style="width:90%; margin-top:-7px;"></div>
            </div>
        </div>

        <div class="row">
            <div style="margin-left:18%;width:80%;padding-top:1px;">
                <div id="dragandrophandler"><?php echo get_lang('DragDrop'); ?></div>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="formw">
                ...
                <select id="users_maps" name="recipients[]" size="
                <?php
                ...
                ?>" multiple style="width: 350px;">
                    ...
                    foreach ($complete_user_list_for_dropbox as $current_user) {
                        if (($dropbox_person->isCourseTutor || $dropbox_person->isCourseAdmin || dropbox_cnf("allowStudentToStudent") // RH: also if option is set
                            ...
                            echo '<option value="' . $current_user_id . '">' . $full_name . '</option>';
                        }
                    }
                    ...
                    echo '</select>
		<img id="mindmap_preview" style="vertical-align:top;" src="../img/mindmap_128.png">
		</div>
	</div>';
		...
		<div class="row">
			...
			WHERE p.file_id = '" . $file_id . "'";
                ...
                function store_add_dropbox() {
                    ...
                    //echo $dropbox_filename ,$dropbox_filesize,$dropbox_filetmpname,  exit();
                    ...
                    $dropbox_filename = replace_accents($dropbox_filename);
                    ...
                    //remove empty spaces
                    $dropbox_filename = str_replace(" ", "_", $dropbox_filename);
                    ...
                    list($course_id, $session_id) = explode("&", api_get_cidReq());

                    $course_name = explode("=", $course_id);
                ...
                function display_user_link($user_id, $name = '') {
                    ...
                    if ($user_id <> 0) {
                        if ($name == '') {
                            ...
                            $result = Database::query($sql, __FILE__, __LINE__);
                            $row = Database::fetch_array($result);
                            return sprintf("<a id='user_id_%s' class='user_info' href='javascript:void(0);'>%s</a>", $row['user_id'], "" . api_get_person_name($row['firstname'], $row['lastname']));
                        } else {
