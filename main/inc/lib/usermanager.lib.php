...
// $Id: usermanager.lib.php 22378 2009-07-26 19:58:38Z yannoo $
...
class UserManager {
     ...
     */
    public static function create_user($firstName, $lastName, $status, $email, $loginName, $password, $official_code = '', $language = '', $phone = '', $picture_uri = '', $auth_source = PLATFORM_AUTH_SOURCE, $expiration_date = '0000-00-00 00:00:00', $active = 1, $hr_dept_id = 0, $extra = null, $country_code = '', $civility = '', $admin = false, $timezone) {
        ...
        $timezone = Security::remove_XSS($timezone);
        ...
        if (is_array($extra) && count($extra) > 0) {
        	...
        	foreach ($extra as $fname => $fvalue) {
            	...
            	$res = self::update_extra_field_value($return, $fname, $fvalue);
            }
        }

        if ($status == COURSEMANAGER && ($admin && $return)) {
            $tbl_admin = Database::get_main_table(TABLE_MAIN_ADMIN);
            $check = Database::query("SELECT user_id FROM $tbl_admin WHERE user_id = $return");
            if (Database::num_rows($check) == 0) {
                Database::query("INSERT INTO $tbl_admin SET user_id = $return");
            }
        }
        return $return;
    }
     ...
     * @return boolean true if the user information was updated
     */
    public static function update_user($user_id, $firstname, $lastname, $username, $password = null, $auth_source = null, $email = '', $status = STUDENT, $official_code = '', $phone = '', $picture_uri = '', $expiration_date = '', $active = 1, $creator_id = null, $hr_dept_id = 0, $extra = null, $language = 'english', $country_code = '', $civility = '', $timezone, $theme=null) {
        ...
        $sql = "UPDATE $table_user SET
				...
				username='" . Database::escape_string($username) . "',
                                timezone='" . Database::escape_string($timezone) . "',
				language='" . Database::escape_string($language) . "',";
        if (!is_null($password)) {
        ...
        if (!is_null($theme)){
            $sql .= " theme ='".Database::escape_string($theme)."'," ;
        }
        ...
        if (is_array($extra) && count($extra) > 0) {
            ...
            foreach ($extra as $fname => $fvalue) {
                // Save new fieldlabel into user_field table
                $field_id = self::create_extra_field($fname, 1, $fname, '');
            	// save the external system's id into user_field_value table
            	$res = self::update_extra_field_value($return, $fname, $fvalue);
            }
        }
    ...
    public static function get_user_info_by_email($user_email) {
        $user_table = Database :: get_main_table(TABLE_MAIN_USER);
    ...
    public static function update_user_picture($user_id, $file = null, $source_file = null) {
        ...
        $type = $picture_info[2];
        $small = self::resize_picture($source_file, 22,$path . 'small_' . $filename);
        $medium = self::resize_picture($source_file, 85,$path . 'medium_' . $filename);
        $normal = self::resize_picture($source_file, 175,$path . '' . $filename);
        $big = self::resize_picture($source_file, $picture_info[0]-1,$path . 'big_' . $filename); // This is the original picture.
        //$big = new image($source_file); // This is the original picture.

        $ok = false;
        $detected = array(1 => 'GIF', 2 => 'JPG', 3 => 'PNG');
        if (in_array($type, array_keys($detected))) {
            //$ok = $small->send_image($detected[$type], $path . 'small_' . $filename) && $medium->send_image($detected[$type], $path . 'medium_' . $filename) && $normal->send_image($detected[$type], $path . $filename) && $big->send_image($detected[$type], $path . 'big_' . $filename);
			$ok = true;
        }
     ...
     */
    public static function get_extra_fields($from = 0, $number_of_items = 0, $column = 5, $direction = 'ASC', $all_visibility = true, $header_export = false) {
        ...
        $sqlf = "SELECT * FROM $t_uf WHERE access_url_id = $access_url_id ";
        if ($all_visibility == false && $header_export == false) {
            $sqlf .= " AND field_visible = 1 and field_registration = 1 ";
        }
     ...
     */
    public static function create_extra_field($fieldvarname, $fieldtype, $fieldtitle, $fielddefault, $fieldoptions = '', $fieldregistration = 0) {
        global $_configuration;
                
        $access_url_id = $_configuration['multiple_access_urls'] == true?api_get_current_access_url_id():1;
        ...
        $sql = "SELECT MAX(field_order) FROM $table_field WHERE access_url_id = $access_url_id";
        $res = Database::query($sql, __FILE__, __LINE__);
        $order = 0;
        if (Database::num_rows($res) > 0) {
        ...
        $sql = "INSERT INTO $table_field
				...
				field_order = '$order',
                                field_registration = '" . intval($fieldregistration) . "',
        ...
        if (!empty($fieldoptions) && in_array($fieldtype, array(USER_FIELD_TYPE_RADIO, USER_FIELD_TYPE_SELECT, USER_FIELD_TYPE_SELECT_MULTIPLE, USER_FIELD_TYPE_DOUBLE_SELECT))) {
            ...
            foreach ($list as $option) {
                ...
                if (Database::num_rows($res) > 0) {
                    ...
                    $time = time();
                    $sql = "INSERT INTO $table_field_options (field_id,option_value,option_display_text,option_order,tms,field_registration) VALUES ($return,'$option','$option',$max,FROM_UNIXTIME($time),'" . intval($fieldregistration) . "')";
                    $res = Database::query($sql, __FILE__, __LINE__);
                    if ($res == false) {
                        $return = false;
                    }
     ...
     * @since Dokeos 1.8.6
     */
    public static function save_extra_field_changes($fieldid, $fieldvarname, $fieldtype, $fieldtitle, $fielddefault, $fieldoptions = '', $fieldregistration = 0) {
        // database table definition
        $table_field = Database::get_main_table(TABLE_MAIN_USER_FIELD);
        $table_field_options = Database::get_main_table(TABLE_MAIN_USER_FIELD_OPTIONS);
        ...
        */
        ...
        Database::query("DELETE FROM $table_field_options WHERE field_id = ".intval($fieldid));
        ...
        foreach ($list as $key => $option) {
            ...
            $time = time();
            $sql = "INSERT INTO $table_field_options (field_id,option_value,option_display_text,option_order,tms,field_registration) VALUES ('" . Database::escape_string($fieldid) . "','" . Database::escape_string($option) . "','" . Database::escape_string($option) . "',$max,FROM_UNIXTIME($time), '" . intval($fieldregistration) . "')";
            $result = Database::query($sql, __FILE__, __LINE__);
        }
        return true;
    }
    ...
    /**
     * Check if a user is connected to chat in footer to receive notification
     * @param   int   User id 
     * @return  bool  
     */
    public static function is_user_chat_connected($user_id = null) {
        $connected = false;
        if (!isset($user_id)) {
            $user_id = api_get_user_id();
        }
        $user_data = self::get_extra_user_data($user_id);
        $connected = isset($user_data['chat_connected']) && $user_data['chat_connected'] == 'true' ? true : false;
        return $connected;
    }
    ...
    public static function get_sessions_by_category($user_id, $fill_first = false, $is_time_over = false) {
        ...
        $condition_date_end = $condition_cat_date_end = "";
        if ($is_time_over) {
            $condition_date_end = " AND date_end < CURDATE() AND date_end != '0000-00-00' ";
            $condition_cat_date_end = " AND s.date_end < CURDATE() AND s.date_end != '0000-00-00' ";
        } else {
            $condition_date_end = " AND (date_end >= CURDATE() OR date_end = '0000-00-00') ";
            $condition_cat_date_end = " AND (s.date_end >= CURDATE() OR s.date_end = '0000-00-00') ";
        }
        ...
        // We get sessions by session category tutor
        $sessions_sql = "SELECT DISTINCT s.id, s.session_category_id FROM $tbl_session s 
                         INNER JOIN $tbl_session_category sc ON sc.id = s.session_category_id
                         INNER JOIN $tbl_session_category_rel_tutor sct ON sct.session_category_id = sc.id
                         WHERE sct.tutor_id = $user_id $condition_cat_date_end
                         ORDER BY s.session_category_id, s.date_start, s.date_end
                        ";
        $result = Database::query($sessions_sql);
        if (Database::num_rows($result) > 0) {
            while ($row = Database::fetch_array($result)) {
    ...
    public static function get_courses_list_by_session($user_id, $session_id) {
        ...
        if ($_configuration['multiple_access_urls'] == true) {
            ...
            if ($access_url_id != -1) {
                ...
                $join_access_url = "INNER JOIN $tbl_url_course url_rel_course ON url_rel_course.course_code = scu.course_code ";
                $where_access_url = " AND access_url_id = $access_url_id ";
            }
        }
        ...
        $personal_course_list_sql = "SELECT scu.course_code as code
                                                  FROM $tbl_session_course_user as scu
                                                  INNER JOIN $tbl_session s ON s.id=scu.id_session
                                                  INNER JOIN $tbl_session_course src ON src.course_code = scu.course_code AND src.id_session = $session_id
                                                  $join_access_url
                                                  WHERE s.id = $session_id AND scu.id_user=$user_id
                                                  $where_access_url
        ...
        if (api_is_allowed_to_create_course()) {
            ...
            // We get category session
            $rs = Database::query("SELECT session_category_id FROM $tbl_session_category_rel_tutor WHERE tutor_id = $user_id");
            if (Database::num_rows($rs) > 0) {
                $categories = array();
                while ($row = Database::fetch_object($rs)) {
                    $categories[] = $row->session_category_id;
                }                
                $sql = "SELECT scu.course_code as code
                        FROM $tbl_session_course_user as scu
                        INNER JOIN $tbl_session s ON s.id=scu.id_session
                        INNER JOIN $tbl_session_course src ON src.course_code = scu.course_code AND src.id_session = $session_id
              $join_access_url
                        WHERE s.id = $session_id AND s.session_category_id IN(".implode(',', $categories).")
              $where_access_url
                       ORDER BY src.position";
            ...
            $personal_course_list_sql = "SELECT scu.course_code as code
                                                  ...
                                                  INNER JOIN $tbl_session_course src ON src.course_code = scu.course_code AND src.id_session = $session_id
                                                  $join_access_url
                                                  WHERE s.id = $session_id AND ((scu.id_user=$user_id AND scu.status=2) OR s.id_coach=$user_id)
                                                  $where_access_url
                                                 ORDER BY src.position";
            $course_list_sql_result = Database::query($personal_course_list_sql, __FILE__, __LINE__);
            if (Database::num_rows($course_list_sql_result) > 0) {
     ...
     */
    public static function resize_picture($file, $max_size_for_picture,$output) {
            ...
            if (file_exists($file) && !is_dir($file)) {
                ...
                switch ($size_picture) {
                    case 'big_' :
                        ...
                        break;
                    case 'medium_' :
                        $picture['file'] = api_get_path(WEB_CSS_PATH) . api_get_setting('stylesheets') . '/images/unknown_50_50.jpg';
                        break;
                    case 'small_' :
                        $picture['file'] = api_get_path(WEB_CODE_PATH) . 'img/unknown.jpg';
                        break;
    ...
    public static function get_search_form($query) {
        echo'<form id="socialsearch" method="GET" action="' . api_get_path(WEB_PATH) . 'main/social/search.php">
		...
		<td align="right" width="200px">              
                ...
                <td align="left">
                <button style="margin:2px;float:none;height:28px;" class="search" type="submit" value="search">' . get_lang('Search') . '</button>
                </td>
		</tr>
		</table></form>';
    }
    ...
    public function get_public_users($keyword, $from = 0, $number_of_items = 20, $column = 2, $direction = 'ASC') {
        ...
        $sql = "SELECT
						 ...
						 " . (api_is_western_name_order() ? "u.firstname 			AS col2,
    ...
    public static function get_user_in_course($user_id, $course_code) {
        $tbl_course_user = Database :: get_main_table(TABLE_MAIN_COURSE_USER);
        $sql = "SELECT user_id FROM $tbl_course_user WHERE course_code = '$course_code' AND user_id = '$user_id'  ";
        $result_course_user = Database::query($sql, __FILE__, __LINE__);
        if (Database::num_rows($result_course_user) > 0) {
            return get_lang('Registered');
        } else {
            return get_lang('NoRegistered');
        }
    }
    ...
    public static function register_shopp_user_course($user_id) {
        $courses_code = array_keys($_SESSION['shopping_cart']['items']);
        if (!empty($courses_code)) {
            foreach ($courses_code as $course_code) {
                //Verify course if already purchased
                $resultPayment = Database::query("SELECT date_end FROM " . Database::get_main_table(TABLE_MAIN_PAYMENT_COURSE_REL_USER) . " WHERE user_id='" . $user_id . "' AND course_code='" . $course_code . "'");
                $course_payment = Database::fetch_array($resultPayment);
                $date_end_payment = trim($course_payment['date_end']); //last date of purchase
                if ($date_end_payment == '') {
                    $date_end_payment = '0000-00-00 00:00:00'; //DATETIME default
                }
                //gets the duration of the course
                date_default_timezone_get(); //Configure TimeZone
                $date_today = date("Y-m-d H:i:s"); //get DATETIME NOW
                $resultEcommerce = Database::query("SELECT duration,duration_type FROM " . Database::get_main_table(TABLE_MAIN_ECOMMERCE_ITEMS) . " WHERE code='" . $course_code . "'");
                while ($row = Database::fetch_array($resultEcommerce)) {
                    ...
                    $date_end = trim($row_date_end['date']); //last date of closing
                    if ($date_end > $date_end_payment && $date_end_payment != '0000-00-00 00:00:00') {//course was purchased
                        $sql = "UPDATE " . Database::get_main_table(TABLE_MAIN_PAYMENT_COURSE_REL_USER) . " SET date_start='" . $date_today . "',date_end='" . $date_end . "' WHERE user_id='" . $user_id . "' AND course_code='" . $course_code . "'";
                    } else {
                        $sql = "INSERT INTO " . Database::get_main_table(TABLE_MAIN_PAYMENT_COURSE_REL_USER) . " (user_id,course_code,date_start,date_end) 
                                VALUES('" . $user_id . "','" . $course_code . "','" . $date_today . "','" . $date_end . "')";
                    }
                    Database::query($sql, __FILE__, __LINE__);
                }
            }
     ...
     * @param   string  Language interface
     * @return  void
     */
    public static function send_mail_to_new_user($recipient_name, $email, $subject, $username, $password, $sender_name, $email_admin, $language = '') {
        ...
        $check_email_template = api_get_setting('show_emailtemplates');
        ...
        $language = (empty($language)) ? $language_interface : $language;
        ...
        $result = Database::query("SELECT content FROM $table_emailtemplate WHERE description = 'Userregistration' AND language= '" . $language . "' AND access_url = ".$access_url_id);
        $message = '';
        $domain_server = api_get_path(WEB_SERVER_ROOT_PATH) . "main/default_course_document";
        if (Database::num_rows($result) > 0 AND $check_email_template == 'true') {
            $row = Database::fetch_array($result);
            $message .= $row['content'];
            ...
            $message .= get_lang('YouAreReg') . " " . api_get_setting('siteName') . " " . get_lang('WithTheFollowingSettings') . "\n\n";
            $message .= get_lang('Username') . ": " . $username . " \n";
            $message .= get_lang('Pass') . " :" . stripslashes($password) . " \n\n";
            $message .= get_lang('Address') . " " . api_get_setting('siteName') . " " . get_lang('Is') . " - {url} \n\n";
            $message .= get_lang('Problem') . "\n\n" . get_lang('Formula') . ",\n";
            $message .= api_get_person_name(api_get_setting('administratorName'), api_get_setting('administratorSurname')) . "\n";
            $message .= get_lang('Manager') . "\n";
            $message .= api_get_setting('administratorTelephone') . " \n";
            $message .= get_lang('Email') . " : {emailAdministrator}";
    ...
    public static function send_mail_to_new_user_for_cheque($recipient_name, $email, $subject, $firstname, $lastname, $username, $password, $programme, $sender_name, $email_admin) {
        ...
        $siteUrl = api_get_setting('InstitutionUrl');
        $table_emailtemplate = Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);
        $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'EmailsInCaseOfChequePayment' AND language= '" . $language_interface . "' AND access_url = ".$access_url_id;
    ...
    public static function send_mail_to_new_user_for_cheque_to_admin($recipient_name, $email, $subject, $firstname, $lastname, $username, $password, $programme, $sender_name, $email_admin) {
        ...
        $siteUrl = api_get_setting('InstitutionUrl');
        $table_emailtemplate = Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);
        $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'EmailsInCaseOfChequePaymentToAdmin' AND language= '" . $language_interface . "' AND access_url = ".$access_url_id;
    ...
    public static function send_mail_add_programmes_for_credit_card_or_installment($recipient_name, $email, $subject, $firstname, $lastname, $programme, $sender_name, $email_admin) {
        ...
        $siteUrl = api_get_setting('InstitutionUrl');
        $table_emailtemplate = Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);
        $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'RegisterUserToProgrammes' AND language= '" . $language_interface . "' AND access_url = ".$access_url_id;
//                $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'RegisterUserToProgrammes' AND language= 'english'";
        ...
        if (api_get_setting('enable_invoice') == 'true') {
            ...
            $header = array();
            api_mail_html($recipient_name, $email, $subject, $message, $sender_name, $email_admin, $header, $data_file);
        }
        else
            api_mail_html($recipient_name, $email, $subject, $message, $sender_name, $email_admin);
    }
    ...
    public static function send_mail_add_programmes_for_cheque($recipient_name, $email, $subject, $firstname, $lastname, $username, $password, $programme, $sender_name, $email_admin) {
        ...
        $siteUrl = api_get_setting('InstitutionUrl');
        $table_emailtemplate = Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);
        $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'RegisterUserToProgrammesforChequePayment' AND language= '" . $language_interface . "' AND access_url = ".$access_url_id;

        $result = api_sql_query($sql, __FILE__, __LINE__);

        $_SESSION['test']['sql'] = $sql;
        while ($row = Database::fetch_array($result)) {
            $mail_content = $row['content'];
        }
        $domain_server = api_get_path(WEB_SERVER_ROOT_PATH) . "main/default_course_document";
    ...
    public static function send_mail_add_programmes_for_cheque_to_admin($recipient_name, $email, $subject, $firstname, $lastname, $username, $password, $programme, $sender_name, $email_admin) {
        ...
        $siteUrl = api_get_setting('InstitutionUrl');
        $table_emailtemplate = Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);
        $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'RegisterUsertoProgrammesforChequePaymentToAdmin' AND language= '" . $language_interface . "' AND access_url = ".$access_url_id;
    ...
    public function get_all_user_info($user_id) {
        $tbl = Database::get_main_table(TABLE_MAIN_USER);
        $user_id = intval($user_id);
    ...
    public function get_user_course_modules_progress($courseCode, $sessionId, $userId) {
        
        $courseInfo = api_get_course_info($courseCode);
        
        $tblLp = Database :: get_course_table(TABLE_LP_MAIN, $courseInfo['dbName']);
	$tblLpView = Database :: get_course_table(TABLE_LP_VIEW, $courseInfo['dbName']);
	$tblLpItemView = Database :: get_course_table(TABLE_LP_ITEM_VIEW, $courseInfo['dbName']);
        $tblLpItem = Database :: get_course_table(TABLE_LP_ITEM, $courseInfo['dbName']);
        ...
        /**
         * Get number of users actived in the platform
         */
        public static function get_number_of_actived_users() {
            $tbl_user = Database::get_main_table(TABLE_MAIN_USER);            
            $rs = Database::query("SELECT user_id FROM $tbl_user WHERE active = 1 AND official_code <> 'anonymous'");
