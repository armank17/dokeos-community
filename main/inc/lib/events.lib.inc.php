...
function check_attempt($quesId,$exeId)
{	
	...
	$res = Database::query("SELECT * FROM $TBL_TRACK_ATTEMPT WHERE exe_id = $exeId AND user_id = ".$userId." AND question_id = $quesId AND course_code = '".$courseCode."'");	
	if(Database::num_rows($res) > 0) {
		Database::query("DELETE FROM $TBL_TRACK_ATTEMPT WHERE exe_id = $exeId AND user_id = ".$userId." AND question_id = $quesId AND course_code = '".$courseCode."'");
	}	
}
 ...
 * @return	boolean	Result of the insert query
 */
function exercise_exam_attempt($score,$answer,$quesId,$exeId,$j)
{
	$score = Database::escape_string($score);
//	$answer = Database::escape_string($answer);
	$quesId = Database::escape_string($quesId);
