...
class SortableTable extends HTML_Table {
    ...
    /**
     * Additional attribute to show custom message
     */
        ...
        if (!$empty_table) {
            ...
            if (count($this->form_actions) > 0) {
                ...
                $html .= '<script type="text/javascript">
                            $(function(){
                                    ...
                                    $(".execute-action").click(function(){
                                            ...
                                            var _self = $(this),
                                                title = _self.attr("dk:title"),
                                                text  = _self.attr("dk:text");
                ...
                switch ($action) {
                   // case 'delete_courses':
                    case 'delete':
                        $html .= '<div class="btn-quiz-submit-bottom"></div>';
                        $html .= '<div class="pull-bottom"><button type="submit" class="save multiaction execute-action" dk:title="'. get_lang("ConfirmationDialog") .'" dk:text="'.get_lang('AreYouSureToDelete').'" >' . $button_text . '</button></div>';                        

                        //$html .= '<div class="pull-bottom"><button type="submit" class="display:none" id="button_submit" class="save multiaction">' . $button_text . '</button></div>';
                        //$html .= '<div class="pull-bottom"><button type="button" class="save multiaction" onclick="$.confirm(\'.Content.\',\'.Title.\', function() {});">' . $button_text . '</button></div>';
                        break;
                    case 'delete_dropbox':
                        $html .= '<div class="btn-quiz-submit-bottom"></div>';
                        $html .= '<div class="pull-bottom"><button type="button" class="save multiaction" id="deleteDropbox">' . $button_text . '</button></div>';
                        break;
                    case 'deleteOnJquery':
                        $html .= '<div class="btn-quiz-submit-bottom"></div>';
                        $html .= '<div class="pull-bottom"><button type="button" class="cancel multiaction" id="deleteOnJquery">' . $button_text . '</button></div>';
                        break;
                    default: 
                        $html .= '<div class="btn-quiz-submit-bottom"></div>';
                        
                        //$html .= '<div class="pull-bottom"><button type="submit" class="save multiaction" onclick="javascript:if(!confirm(' . "'" . addslashes(api_htmlentities(get_lang("ConfirmYourChoice"), ENT_QUOTES, $charset)) . "'" . ')) return false;">' . $button_text . '</button></div>';
                        $html .= '<div class="pull-bottom"><button type="submit" class="save multiaction execute-action" dk:title="'. $button_text .'" dk:text="'.get_lang("ConfirmYourChoice").'">' . $button_text . '</button></div>';
//                      
//                          $html .= '<div class="pull-bottom"><button type="submit" class="display:none" id="button_submit" class="save multiaction">' . $button_text . '</button></div>';
//                        $html .= '<div class="pull-bottom"><button type="button" class="save multiaction" onclick="  
...
//			</style>';
	 ...
	 * Should not be use to edit information (edit/delete rows) only.
	 * */
	public function display_ajax_grid () {
		global $charset;		
		$empty_table = false;
		$html = '';
                           ...
                           $(document).ready(function() {
                                $(".grid_nav").each(function() {
                                    $(this).find("a").each(function(i) {
                                        ...
                                        $(this).attr("id","ajax-chat-page-"+page_num.pop());
                                        $(this).attr("class","chat-user-page");
                                    });                                
                                });
                                
                           });
                           
                            $(".chat-user-page").click(function(e){
                              var page_id = $(this).attr("id");
                                  page_id = page_id.replace("ajax-chat-page-","");
                                  $.ajax({
                                          contentType: "application/x-www-form-urlencoded",
                                          type: "GET",
                                          url: "'.api_get_path(WEB_AJAX_PATH).'social.ajax.php",
                                          data: {a: "chat_user_list", page_id: page_id, quantity : 25},  
                                          success: function(datos) {
                                              $("#ajax-chat-dialog").html();
                                              $("#ajax-chat-dialog").html(datos);
            ...
            foreach ($items as $row) {
                ...
                foreach ($row as $element) {
                    if ($filter === true || $vibility_options[$i] == true) {
                        $html.='<div class="' . $this->table_name . '_grid_element_' . $i . ' grid-aling_' . $i . ' ">' . $element . '</div>';
                    }
                    $i++;
                }
                $html.='</div>';
    ...
    public function set_header($column, $label, $sortable = true, $th_attributes = null, $td_attributes = null) {
        ...
        if ($sortable) {
            $cireq = api_get_cidreq();
            $link = '<a href="' . api_get_self() . '?origin=' . $origin . '&amp;';
            if (!empty($cireq)) {
    ...
    public function set_form_actions($actions, $checkbox_name = 'id') {
        ...
        if (count($this->form_actions) > 0) {
            $this->set_header(0, '<center><input type="checkbox" value="select_all_none" name="select_all_none" onclick="javascript:setCheckbox();" /></center>', false);
        }
    ...
    public function filter_data($row) {
        ...
        if (count($this->form_actions) > 0) {
            if (strlen($row[0]) > 0) {
                $row[0] = '<center><input type="checkbox" name="' . $this->checkbox_name . '[]" value="' . $row[0] . '"';
