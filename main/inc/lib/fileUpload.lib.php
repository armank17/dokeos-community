...
function process_uploaded_file($uploaded_file) {
    ...
    switch ($uploaded_file['error']) {
        case 1:
            ...
            Display::display_error_message(get_lang('UplExceedMaxServerUpload') . ini_get('upload_max_filesize'), false, true);
            return false;
        case 2:
            ...
            Display::display_error_message(get_lang('UplExceedMaxPostSize') . round($_POST['MAX_FILE_SIZE'] / 1024) . " KB", false, true);
            return false;
        case 3:
            // The uploaded file was only partially uploaded.
            Display::display_error_message(get_lang('$UplPartialUpload') . " " . get_lang('PleaseTryAgain'), false, true);
            return false;
        case 4:
            // No file was uploaded.
            Display::display_error_message(get_lang('UplNoFileUploaded') . " " . get_lang('UplSelectFileFirst'), false, true);
            return false;
    }
            ...
            switch ($what_if_file_exists) {
                ...
                case 'overwrite':
                    ...
                    if (@move_uploaded_file($uploaded_file['tmp_name'][$file_no], $store_path)) {
                        ...
                        if ($file_exists) {
                            ...
                            item_property_update_on_folder($_course, $upload_path, $user_id);
                            //display success message with extra info to user
                            return array('path' => $file_path, 'error' => 4);
                        } else {
                            //put the document data in the database
                            $document_id = add_document($_course, $file_path, 'file', $file_size, $document_name);
                            if ($document_id) {
                            ...
                            //if the file is in a folder, we need to update all parent folders
                            item_property_update_on_folder($_course, $upload_path, $user_id);
                            return array('path' => $file_path, 'error' => 9);
                        }
                    } else {
                        return array('path' => $file_path, 'error' => 5);
                    }
                    break;
                ...
                case 'rename':
                    ...
                    if (@move_uploaded_file($uploaded_file['tmp_name'][$file_no], $store_path)) {
                        ...
                        item_property_update_on_folder($_course, $upload_path, $user_id);
                        //display success message to user
                        return array('path' => $new_file_path, 'error' => 6);
                    } else {
                        return array('path' => $document_name, 'error' => 7);
                    }
                    break;
                ...
                default:
                    if (file_exists($store_path)) {
                        return array('path' => $file_path, 'error' => 8);
                    } else {
                        if (@move_uploaded_file($uploaded_file['tmp_name'][$file_no], $store_path)) {
                            chmod($store_path, $files_perm);
...
function documents_total_space($to_group_id = '0') {
    ...
    $sql = "SELECT SUM(size) FROM " . $TABLE_ITEMPROPERTY . " props, " . $TABLE_DOCUMENT . " docs 
            WHERE docs.id=props.ref AND props.tool='" . TOOL_DOCUMENT . "' AND 
            props.to_group_id='" . $to_group_id . "' AND props.visibility <> 2";
        ...
        if ($dir = @opendir($baseWorkDir . $uploadPath)) {
            while ($file = readdir($dir)) {
                if ($file != '.' && $file != '..') {
                    ...
                    if (is_dir($baseWorkDir . $uploadPath . '/' . $file))
                        $filetype = "folder";
...
function filter_extension(&$filename) {
    ...
    if ($blacklist != 'whitelist') {//if = blacklist
        ...
        if (empty($ext)) {
            return 1;
        }//we're in blacklist mode, so accept empty extensions
 ...
 */
function update_existing_document($_course, $document_id, $filesize, $readonly = 0, $doctitle = null) {
    global $charset;
    ...
    if (Database::query($sql, __FILE__, __LINE__)) {
        // update title in lp
        if (!empty($doctitle)) {
            $lp_item_table = Database::get_course_table(TABLE_LP_ITEM, $_course['dbName']);
            $lp_table = Database::get_course_table(TABLE_LP_MAIN, $_course['dbName']);
            $rs = Database::query("SELECT lpi.id as item_id, lpi.lp_id, lpi.path, lp.default_encoding 
										FROM $lp_item_table lpi
										INNER JOIN $lp_table lp ON lp.id = lpi.lp_id
										WHERE lpi.item_type='document' AND lpi.path='" . intval($document_id) . "'");
            if (Database::num_rows($rs) > 0) {
                while ($row = Database::fetch_object($rs)) {
                    $lp_encoding = $row->default_encoding;
                    $item_id = $row->item_id;
                    $lp_title = api_convert_encoding($doctitle, $lp_encoding, $charset);
                    Database::query("UPDATE $lp_item_table SET title='" . Database::escape_string($lp_title) . "' WHERE id=$item_id");
                }
            }
        }
        return true;
