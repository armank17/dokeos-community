...
function get_unicode_html($content = "") {
...
function api_mail_html($recipient_name, $recipient_email, $subject, $message, $sender_name = "", $sender_email = "", $extra_headers = null, $data_file = array()) {
    ...
    //Characters especials Subject for announcement
    $subject = get_unicode_html($subject);
...
//    $mail->AltBody = strip_tags(str_replace('<br />',"\n", api_html_entity_decode($message)));
    ...
    // attachment ...
    if (!empty($data_file)) {
        if (isset($data_file['path'])) {
            $mail->AddAttachment($data_file['path'], $data_file['filename']);
        } elseif (isset($data_file['string'])) {
            $mail->AddStringAttachment($data_file['string'], $data_file['filename'], $data_file['encoding'], $data_file['type']);
        }
    }
