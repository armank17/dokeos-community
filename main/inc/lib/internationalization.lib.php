...
function api_format_date($date_format, $time_stamp = -1, $language = null) {
		...
		switch ($date_format) {
			...
			case DATE_TIME_FORMAT_LONG:
				$date_format = get_lang('dateTimeFormatLong', '', $language);
                                $date_format = str_replace('%M', '%M:%S', $date_format);                                                        
				break;
 ...
 function api_format_time($seconds) {
...
/**
 * Converts a given array into UTF-8 encoded array.
 * @param array $array                      The array being converted.
 * @param string $from_encoding (optional)  The encoding that $string is being converted from. If it is omited, the platform character set is assumed.
 * @return array array                      Returns the converted array.
 */
function api_utf8_encode_array($array, $from_encoding = null) {
    $return = array();    
    foreach($array as $key => $value) {
        switch(gettype($value)){
            case 'array':
                $return[$key] = api_utf8_encode_array($value, $from_encoding);
                break;
            case 'string':
                $return[$key] = api_utf8_encode($value, $from_encoding);
                break;
            default:
                $return[$key] = $value;
        }
...
function api_time_format($seconds) {
