...
class SystemAnnouncementManager
{
	...
	function display_announcements($visible, $id = -1)
	{
		...
		if (Database::num_rows($announcements))
		{
			...
			echo '<a class="btn-announcement" href="news_list.php">'.get_lang("More").'</a>';
			echo '</td></tr>';
			echo '</table>';
			echo '</div>';
		}
	...
	function count_announcements($visible, $id = -1)
	{
        ...
        function display_all_node_news() {
            ...
            $sqlNews = "SELECT  node.id,
                        ...
                        WHERE node.active = 1  
                          ...
                          AND node.enabled = 1
                          AND CURRENT_TIMESTAMP BETWEEN news.start_date AND news.end_date
            ...
            if ($num>0){
                ...
                while ($row = Database::fetch_array($resultsNews)) {
                    ...
                    echo '<a name="'.$row['id'].'"></a>
                                    ...
                                    <div class="system_announcement_title">'
                                            .$row['start_date'].' <strong>'.$row['title'].'</strong>
                                    </div>
                                    <br />
                                    <div class="system_announcement_content">'
                                                    .$row['content'].'
	...
	function send_system_announcement_by_email($title,$content,$teacher, $student)
	{
		...
		while($row = Database::fetch_array($result,'ASSOC'))
		{
			...
			$content = str_replace('\n','',$content);
			api_mail_html(api_get_person_name($row['firstname'], $row['lastname'], null, PERSON_NAME_EMAIL_ADDRESS), $row['email'], api_html_entity_decode($title, ENT_QUOTES, $charset), $content, api_get_person_name($_user['firstName'], $_user['lastName'], null, PERSON_NAME_EMAIL_ADDRESS), api_get_setting('emailAdministrator'), api_get_setting('emailAdministrator'));
		}
	}
}
?>
