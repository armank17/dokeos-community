    ...
    setTimeout(function() {
        ...
        $.each(browser.files, function(i, file) {
            ...
            if (_.kuki.get('view') == 'list') {
                ...
                if (browser.type == document.location.hostname) {
                    icon = '/main/webtv/thumbnail.php?file=' + encodeURIComponent(file.name) + '.png&size=18x18';
                }
                html += '<tr class="file">' +
                ...
                if (browser.type == document.location.hostname) {
                    icon = '/main/webtv/thumbnail.php?file=' + encodeURIComponent(file.name) + '.png&size=100x100';
                }
                html += '<div class="file">' +
        ...
        $.each(browser.files, function(i, file) {
            ...
            $(item).data(file);
            if (_.inArray(file.name, selected) || ((typeof selected != 'undefined') && !selected.push && (file.name == selected)) || file.selected) {
...
browser.getUrlParam = function(paramName) {
...
browser.returnFile = function(file) {
    ...
    if (this.opener.CKEditor) {        
    ...
    else if (window.parent.CKEDITOR) {       
      ...
      CurrObj.getContentElement(tab, 'txtWidth').setValue(file.data('widthImage'));
      CurrObj.getContentElement(tab, 'txtHeight').setValue(file.data('heightImage'));
    } 
