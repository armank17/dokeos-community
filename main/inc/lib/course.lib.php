...
class CourseManager {
    ...
    public static function subscribe_user($user_id, $course_code, $status = STUDENT) {
        ...
        if (empty($_SESSION['id_session'])) {
            ...
            $emailsubject = get_lang('YouHaveBeenRegisteredInTheCourse') . ': ' . $name_course;
            ...
            $result = Database::query("SELECT content FROM $table_emailtemplate WHERE description = 'MessageToUserInUserRegistrationToCourse' AND language= '" . $language_interface . "' AND access_url = ".$access_url_id);
            $message = '';
            $num_rows = Database::num_rows($result);
            $row = Database::fetch_array($result);
            $message = trim(strip_tags($row['content']));

            if ($num_rows == 0 || empty($message)) {
                $message .= get_lang('Dear') . ': ' . api_get_person_name($student['firstname'], $student['lastname']) . "<br>";
                $message .= get_lang('HaveBeenRegisteredInTheCourse') . ': ' . $name_course . "<br>";
                $message .= get_lang('UserName') . ': ' . $student['username'] . "<br>";
                if (api_is_western_name_order()) {
                    $message .= get_lang('FirstName') . ': ' . $student['firstname'] . "<br>";
                    $message .= get_lang('LastName') . ': ' . $student['lastname'] . "<br>";
                } else {
                    $message .= get_lang('LastName') . ': ' . $student['lastname'] . "<br>";
                    $message .= get_lang('FirstName') . ': ' . $student['firstname'] . "<br>";
                }
                $message .= get_lang('Email') . ': ' . $student['mail'] . "<br>";
            } else {
                ...
                $message = str_replace('{nameCourse}', $name_course, $message);
            ...
            $notification = api_get_course_setting('email_alert_to_user_subscribe_in_course', $course_code);
            if ($notification == 1) {
                $sent = api_mail_html($recipient_name, $emailto, $emailsubject, $message, $sender_name, $email_admin);
            }
            //send email to tutor
            ...
            $result = @Database::query(
                            "INSERT INTO " . Database::get_main_table(TABLE_MAIN_COURSE_USER) . "
            ...
            if (api_get_setting('automatic_group_filling') == 'true' ) {
                SessionManager::automatically_add_user_to_group($course_code,$user_id);
            }
    ...
    public static function add_user_to_course($user_id, $course_code, $status = STUDENT) {
        ...
        $ecommerce_items_table = Database::get_main_table(TABLE_MAIN_ECOMMERCE_ITEMS);
        ...
        $sql = "SELECT e.duration, e.duration_type FROM {$ecommerce_items_table} AS e WHERE code = '{$course_code}'";
        $row = Database::fetch_row(Database::query($sql));
        $todayDate = date("Y-m-d");
        $endDate = date("Y-m-d", strtotime(date("Y-m-d", strtotime($todayDate)) . "+{$row[0]} {$row[1]}"));
        ...
        return (bool) Database::query("INSERT INTO " . $course_user_table . "
                ...
                date_end = '{$endDate}'", __FILE__, __LINE__);
    }
    ...
    public static function get_user_list_from_course_code($course_code, $with_session = true, $session_id = 0, $limit = '', $order_by = '') {
        ...
        $sql = $session_id == 0 ? 'SELECT DISTINCT course_rel_user.status, user.user_id, course_rel_user.role, course_rel_user.tutor_id, user.*  ' : 'SELECT DISTINCT user.user_id, user.status, user.*  ';
    ...
    public static function email_to_tutor($user_id, $course_code, $send_to_tutor_also = false) {
        ...
        if ($send_to_tutor_also) {
            $sql = "SELECT user_id FROM " . Database::get_main_table(TABLE_MAIN_COURSE_USER) . " WHERE course_code='" . $course_code . "' AND status=1 OR course_code='" . $course_code . "' AND tutor_id=1";
        } else {
            $sql = "SELECT user_id FROM " . Database::get_main_table(TABLE_MAIN_COURSE_USER) . " WHERE course_code='" . $course_code . "' AND status=1";
        }
        ...
        foreach ($send_id as $id => $id_user) {

            $tutor = Database::fetch_array(Database::query("SELECT * FROM " . Database::get_main_table(TABLE_MAIN_USER) . " WHERE user_id='" . $id_user . "'", __FILE__, __LINE__));
            $emailto = $tutor['email'];
            ...
            $table_emailtemplate = Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);
            $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'UserRegistrationToCourse' AND language= '" . $language . "' AND access_url = ".$access_url_id;
            $result = api_sql_query($sql, __FILE__, __LINE__);
            $row = Database::fetch_array($result);
            if (Database::num_rows() == 0 || empty($row['content'])) {
                $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'UserRegistrationToCourse' AND language= 'english' AND access_url = ".$access_url_id;
                $result = api_sql_query($sql, __FILE__, __LINE__);
            }
            ...
            $message = str_replace("/main/default_course_document", "tmp_file", $message);
            $message = str_replace("{tutorFirstNameTutorLastName}", api_get_person_name($tutor['firstname'], $tutor['lastname']), $message);
            $message = str_replace('{nameCourse}', $name_course, $message);
            $message = str_replace('{userName}', $student['username'], $message);
            $message = str_replace('{userFirstName}', $student['firstname'], $message);
            $message = str_replace('{userLastName}', $student['lastname'], $message);
            $message = str_replace('{userEmail}', $student['email'], $message);
            $message = str_replace("tmp_file", $domain_server, $message);
            ...
            //@api_mail($recipient_name, $emailto, $emailsubject, $message, $sender_name,$email_admin);
            api_mail_html($recipient_name, $emailto, $emailsubject, $message, $sender_name, $email_admin);
        }
    }
    ...
    public static function get_courses_list_by_user_id($user_id, $include_sessions = false) {
        ...
        $result = Database::query('SELECT c.code,c.db_name,c.title,cru.user_course_cat as category_id
				...
				WHERE cru.user_id=' . $user_id . ' ' . $where_access_url . ' AND cru.date_end <= "{$curr_date}" ORDER BY cru.sort, c.title', __FILE__, __LINE__);
    ...
    public static function get_emails_of_tutors_to_course($code) {
        ...
        while ($list_users = Database::fetch_array($res)) {
            ...
            while ($row_user = Database::fetch_array($result)) {
                ...
                $list[$row_user['user_id']] = array($row_user['email'] => $name_teacher);
            }
        }
        return $list;
    }
    ...
    public static function get_email_of_tutor_to_session($session_id, $course_code) {
        ...
        if (Database::num_rows($rs) > 0) {
            ...
            $sql = "SELECT user_id,firstname,lastname,email FROM $tbl_user WHERE user_id IN (" . implode(",", $user_ids) . ")";
            ...
            while ($row_emails = Database::fetch_array($rs_user)) {
                ...
                $coachs_emails[$row_emails['user_id']] = array($row_emails['email'] => $name_tutor);
            }
        }
    ...
    public static function get_session_category_id_by_session_id($session_id) {
        return Database::result(Database::query('SELECT  sc.id session_category
				...
				ON sc.id=s.session_category_id WHERE s.id="' . Database::escape_string($session_id) . '"', __FILE__, __LINE__), 0, 'session_category');
    }
    ...
    /**
     * get the courses coincidences in an array 
     * @param type $haystack array
     * @param type $match s
     * @return type
     */
     
    public static function get_courses_list_in_Array($haystack,$subject){
            $matches = implode(",",preg_grep ('/^'.preg_quote($subject,'/').'.*?$/i', $haystack));
        return $matches;
        
    }
    public static function paginationInCourseList($limit,$numRows,$page){
        $allPages   = ceil($numRows / $limit);
                   
                    $next = $page+1;
                    echo '<input id="nextPg" type=hidden value='.$next.'>';
                    echo '<input id="rowLimit" type=hidden value='.$limit.'>';
                    $paginHTML .= '<div style="margin-top:10px;">';
                    if($page>1){
                        $paginHTML .= '<a class="previewPage" style="color:red;position:absolute;bottom:26px;"href="javascript:void(0)">'.api_utf8_encode(get_lang('Preview')).'</a>';
                    }
                    if($allPages!=$page){
                        $paginHTML .= '&nbsp;<a class="nextPage" style="color:blue;float:right;" href="javascript:void(0)">'.get_lang('Next').'</a>';
                    }
                    $paginHTML .= '</div>';
    ...
    /*     * When delete a course fromecommerce_courses.php it update thye status to 0
     *
     * @param type $code 
     */

    function delete_from_payment($code) {
        $course_table = Database::get_main_table(TABLE_MAIN_COURSE);
        $tbl_ecommerce = Database::get_main_table(TABLE_MAIN_ECOMMERCE_ITEMS);
        $result = @Database::query("UPDATE " . $course_table . " SET payment =0  WHERE code = '" . $code . "'", __FILE__, __LINE__);
        $sql1 = "UPDATE {$tbl_ecommerce} SET status = 0 WHERE code = '$code'";
        Database::query($sql1, __FILE__, __LINE__);
    }

    function show_menu_course_admin($ref = 'list') {
        ...
        echo '<a href="' . api_get_path(WEB_CODE_PATH) . 'admin/course_user_import.php">' . Display::return_icon('pixel.gif', get_lang('ImportUsersToACourse'), array('class' => 'toolactionplaceholdericon toolactionimportcourse')) . get_lang('ImportUsersToACourse') . '</a>';
         if ($ref == 'list') {
            echo '<a href="javascript:void(0)" id="btn-search">' . Display::return_icon('pixel.gif', get_lang('AdvancedSearch'), array('class' => 'toolactionplaceholdericon toolactionsearch')) . get_lang('Search') . '</a>';
        }
