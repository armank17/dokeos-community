...
# $Id: fileManage.lib.php 21531 2009-06-20 16:03:29Z ivantcholakov $
 ...
 *
 */
function update_db_info($action, $oldPath, $newPath = "", $course_db = '') {
    global $dbTable; // table 'document'  // RH: see below

    $dbTable = Database::get_course_table(TABLE_DOCUMENT, $course_db);
    ...
    if ($action == "update") {
		...
		SET path = CONCAT('" . $newPath . "', SUBSTRING(path, LENGTH('" . $oldPath . "')+1) ), title = '" . basename($newPath) . "'
...
function my_rename($filePath, $newFileName) {
    ...
    if ((strpos($newFileName, '.') === FALSE) && ($dotpos = strrpos($oldFileName, '.'))) {
    ...
    if (strtolower($newFileName) != strtolower($oldFileName) && check_name_exist($path . "/" . $newFileName))
        return false;
