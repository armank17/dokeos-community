 ...
 */
jQuery.fn.highlight = function(searchString, lenient, real_code) {
  ...
  function setHighlight(node, pattern) {                  
    ...
    if (node.nodeType == 3) {
      ...
      if (lenient) {
       ...
       var SearchRegExp = new RegExp("(\\x08|\\b|^)"+pattern+"(\\x08|\\b|$)", "g");        
       index = textToTest.search(SearchRegExp);
      }
      
      //var theText = node.data;      
      if (index >= 0) {
        var spanNode = document.createElement("a");
        spanNode.className = "glossary-ajax highlight";
  ...
  function createAccentRegexp(characters) {
    ...
    //return new RegExp("(\\w|^)"+regexp+"(\\w|$)", "g");
    return new RegExp("^.*\\b("+regexp+")\\b.*$", "g");       
  }
