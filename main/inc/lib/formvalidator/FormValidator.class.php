...
class FormValidator extends HTML_QuickForm
{
	...
	function FormValidator($form_name, $method = 'post', $action = '', $target = '', $attributes = null, $trackSubmit = true)
	{
		...
		$this->registerRule('username',null,'HTML_QuickForm_Rule_Username',$dir.'Rule/Username.php');
                $this->registerRule('captcha',null,'HTML_QuickForm_Rule_ValidateCaptcha',$dir.'Rule/ValidateCaptcha.php');
...
EOT;
        ...
        <div class="row">
                <div class="label">
                        <div class="label1 cuslabel-content">
...
EOT;
	...
	{
               //$this->addElement('html', '<div class="ck-loading" style="position:relative;clear:both;">');
               $this->addElement('html_editor', $name, $label, 'style="height:'.$config['Height'].'px;width:'.$config['Width'].'"', $config);
               $this->applyFilter($name,'trim');
               //$this->addElement('html', '<div class="ck-icon-loading" style="width:210px;height:80px;text-align:center;margin:auto;position:absolute;top:20%;left:38%;font-size:10px;line-height: 24px;">'.get_lang('LoadingEditor').'<br />'.Display::return_icon('mozilla_blu.gif').'</div>');
	...
	function add_real_progress_bar($upload_id, $elementAfter, $delay=2,$waitAfterUpload=false)
	{
		...
		<div id="dynamic_div_container" style="display:none; clear:both; margin-top:10px;">
...
//			$return_value .= '<div id="dynamic_div" style="display:block;margin-left:40%;margin-top:10px;height:50px;"></div>';
			$return_value .= '<div id="dynamic_div" style="display:block;margin-left:40%;"></div>';
		}
		return $return_value;
	}
}
