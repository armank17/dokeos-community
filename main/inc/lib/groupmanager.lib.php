...
class GroupManager {
	...
	public static function get_group_list ($category = null, $course_code = null) {
		...
		$my_user_id=api_get_user_id();
                $my_course_code = $course_code;
                if (is_null($my_course_code)) {
	...
	*	GROUP USERS FUNCTIONS
	...
	public static function is_subscribed ($user_id, $group_id) {
		...
		return Database::num_rows($db_result) > 0;
         ...
         public static function get_user_not_subscribed($group_id, $courseCode){             
             
             
             $course_rel_user = Database::get_main_table(TABLE_MAIN_COURSE_USER);
             $users = Database::get_main_table(TABLE_MAIN_USER);
             $group_rel_user= Database::get_course_table(TABLE_MAIN_USER_REL_GROUP);             
             $sql = "SELECT DISTINCT(users.user_id),CONCAT_WS(' ',users.lastname, users.firstname ) as fullname 
                    FROM $users users INNER JOIN $course_rel_user cru  
                    ON (users.user_id = cru.user_id)
                    WHERE cru.course_code = '$courseCode' AND users.user_id <> 2  AND users.user_id 
                    NOT IN (
                            SELECT gru.user_id FROM  $group_rel_user gru WHERE gru.group_id = $group_id)";
           ...
           // echo $sql;
            ...
            while ($row = Database::fetch_array($result)){
                  $userList[] = $row;
                if(self :: number_of_students($group_id) >= self :: maximum_number_of_students($group_id)){
