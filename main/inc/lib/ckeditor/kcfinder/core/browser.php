...
class browser extends uploader {
    ...
    public function action() {
        ...
        if (isset($_SESSION['dir'])) {
            $this->session['dir'] = $this->clear_session_dir($_SESSION['dir']);
        }
    ...
    protected function clear_session_dir($dir) {
        $dir = urldecode($dir);
        $new_dir = $this->type;
        if (strpos($dir, $this->type) !== FALSE) {
            $new_dir = strstr($dir, $this->type);
        }
        return dirname($new_dir);
    }
    ...
    protected function act_newDir() {
        ...
        $path = '/'.strstr($dir, $this->type).$newDir;        
        $doc_id = add_document($_course, $path, 'folder', 0, $newDir);
    ...
    protected function act_renameDir() {
        ...
        $newFile = '/'.strstr(dirname($dir) . "/$newName", $this->type);
        $oldFile = '/'.strstr($this->normalizeFilename(trim($dir)), $this->type);
        update_db_info('update', $oldFile, $newFile, $_course['dbName']);
    ...
    protected function act_deleteDir() {
        ...
        if (isset($_course['dbName']) && $_course<>'-1') {
            $document_path = '/'.strstr($dir, $this->type); //strstr($file, '/document/'); //substr($fullpath, strpos($fullpath,'/document/')+9, strlen($fullpath)); //   /shared_folder/4/name
            $fullpath = str_replace($document_path, '/', $dir);            
    ...
    protected function act_rename() {
        ...
        $newFile = '/'.strstr($newName, $this->type);
        $oldFile = '/'.strstr($this->normalizeFilename(trim($file)), $this->type);
        update_db_info('update', $oldFile, $newFile, $_course['dbName']);
    ...
    protected function act_delete() {
        ...
        global $_course;
        if (isset($_course['dbName']) && $_course<>'-1') {
            $document_path = '/'.strstr($file, $this->type);
            $fullpath = str_replace($document_path, '/', $file);            
            DocumentManager::delete_document($_course,$document_path,$fullpath);
        }
    ...
    protected function resizeImage($img, $set_width, $set_height, $width_img, $height_img) {
        $scale_image = imagecreatetruecolor($set_width, $set_height);
        $white = imagecolorallocate($scale_image, 255, 255, 255);
        imagefill($scale_image, 0, 0, $white);
        imagecopyresampled($scale_image, $img, 0, 0, 0, 0, $set_width, $set_height, $width_img, $height_img);
        imagedestroy($img);
        return $scale_image;
    }
    
    protected function saveImage($name_img, $path_img, $path_save_img, $max_width, $max_height) {
        ...
        switch ($ext) {
            ...
            case "jpg":
                ...
                break;
            case "gif":
                $image = imagecreatefromgif($path_img);
                break;
            default:
                $image = imagecreatefromjpeg($path_img);
                break;
        }
        if ($width > $max_width || $height > $max_height) { //resize
            $set_width = min($width, $max_width);
            $set_height = ($height * $set_width) / $width;
            $image = self::resizeImage($image, $set_width, $set_height, $width, $height);
            if ($set_height > $max_height) { //height is bigger
                $width = $set_width;
                $height = $set_height;
        ...
        switch ($ext) {
            case "png":
                $succes = imagepng($image, $path_save_img);
                break;
            case "jpg":
                $succes = imagejpeg($image, $path_save_img, 100);
                break;
            case "gif":
                $succes = imagegif($image, $path_save_img);
                break;
            default:
                $succes = imagejpeg($image, $path_save_img, 100);
                break;
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        $succes = self::saveImage($file['name'], $file['tmp_name'], $target, 900, 700);
        if ($succes != true) {
            @unlink($file['tmp_name']);
...
//            chmod($target, $this->config['filePerms']);
        ...
        $stat = stat($target);
        $path = '/'.strstr($dir, $this->type).'/'.$file['name'];        
        $doc_id = add_document($_course, $path, 'file', $stat['size'] , $file['name']); //get Dokeos
        $current_session_id = api_get_session_id();
        api_item_property_update($_course, TOOL_DOCUMENT, $doc_id, 'DocumentAdded', api_get_user_id(), $_SESSION['_gid'], null, null, null, $current_session_id);//get Dokeos
    ...
    protected function getFiles($dir) {
        ...
        foreach ($files as $file) {
            ...
            $selected = false;
            if (isset($_SESSION['dir']) && $name == basename($_SESSION['dir'])) {
                $selected = true;
            }
    ...
    protected function getDirs($dir) {
        ...
        if (is_array($dirs)) {
            ...
            foreach ($dirs as $cdir) {
               $pos = strpos(basename($cdir), '_DELETED_');
				if($pos === false) {
