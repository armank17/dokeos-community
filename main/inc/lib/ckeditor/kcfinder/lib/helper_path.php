  ...
  /** Get the absolute URL path of the given one. Returns FALSE if the URL
    ...
    * @return string */
    ...
    static function rel2abs_url($path) {
        ...
        $dir = @getcwd();
        $dokeos_root_sys = $_configuration['root_sys'];
        if (!isset($dokeos_root_sys) || ($dir === false))
            return false;

        if (!empty($_configuration['url_append'])) {
            if (strpos($dir, $_configuration['url_append']) === 0) {
                $dir = str_replace($_configuration['url_append'], '', $dir);
            }
        }
        
        $dir = self::normalize($dir);
        $doc_root = self::normalize($dokeos_root_sys);
  ...
  /** Resolve full filesystem path of given URL. Returns FALSE if the URL
    ...
    * @return string */
    ...
    static function url2fullPath($url) {
        ...
        if (!empty($_configuration['url_append'])) {
            if (strpos($url, $_configuration['url_append']) === 0) {
                $url = str_replace($_configuration['url_append'], '', $url);
            }
        }

        if (isset($dokeos_root_sys)) {
            return self::normalize($dokeos_root_sys . "/$url");
