...
(function()
{
	var imageDialog = function( editor, dialogType )
	{
		...
		var resetSize = function( dialog )
		{
			...
			if ( oImageOriginal.data )
			{
                ...
                var setupBrowseIframe = function(type, element) {
                    ...
                    if (element && element.is('img')) {                                               
                        ...
                        if (src) {
                            ...
                            url = getBaseUrl() + editor.config.filebrowserImagemanagerBrowseUrl+'&path='+encodeURIComponent(src)+'&ts='+ts;                            
                        }   
                    }
                    else {
                        var ts = new Date().getTime();
                        url = getBaseUrl() + editor.config.filebrowserImagemanagerBrowseUrl+'&ts='+ts;
                    }
                    if ($("#kcfinder_image_iframe").length > 0) {   
                        $("#kcfinder_image_iframe").attr("src", url); 
                    }
                };
                ...
                {                   
                    var url = getBaseUrl() +editor.config.filebrowserImagemanagerBrowseUrl;
                    return '<div style="width:100%;height:342px;"><iframe name="kcfinder_image_iframe" src="'+url+'" id="kcfinder_image_iframe" frameborder="0" width="100%" height="342px" style="width:100%;height:342px;" marginwidth="0" marginheight="0" scrolling="no"></iframe></div>';
                }
		...
		return {
			title : editor.lang.image[ dialogType == 'imagemanager' ? 'title' : 'titleButton' ],
			minWidth : 850,
			minHeight : 450,
			onShow : function()
			{
			...
			onOk : function()
			{
				...
				if ( this.imageEditMode )
				{
					...
					if ( dialogType == 'imagemanager' && imgTagName == 'input' && confirm( editor.lang.image.button2Img ) )
					{
						// Replace INPUT-> IMG
						imgTagName = 'img';
						this.imageElement = editor.document.createElement( 'img' );
					...
					else if ( dialogType != 'imagemanager' && imgTagName == 'img' && confirm( editor.lang.image.img2Button ))
					{
						// Replace IMG -> INPUT
						imgTagName = 'input';
			...
			onLoad : function()
			{
				...
				this.commitContent = commitContent;
                                ...
                                var ts = new Date().getTime();
                                var src = getBaseUrl() + editor.config.filebrowserImagemanagerBrowseUrl+'&ts='+ts;
                                if ($("#kcfinder_image_iframe").length > 0) {
                                    $("#kcfinder_image_iframe").attr("src", src);
                                }
                                            ...
                                            [
                                                    {
                                                            ...
                                                            [
                                                                    ...
                                                                    },                                                                   
                                                                    {
                                                                        id : 'txtTitImg',
                                                                        type : 'text',
                                                                        label : editor.lang.image.titImg || 'Image Title',
                                                                        width : '100px',
                                                                        accessKey : 'T',
                                                                        'default' : '',
                                                                        onChange : function()
	...
	CKEDITOR.dialog.add( 'imagemanager', function( editor )
		{
			return imageDialog( editor, 'imagemanager' );
		});
