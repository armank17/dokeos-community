...
var CK = oEditor.currentInstance;
var CKConfig = CK.config ;
var CKLang = CK.lang.imgmap ;
...
window.onload = function()
{
    var btn;
    btn = $('imgpointer') ;
    btn.alt = btn.title = CKLang.imgmapPointer;
    btn = $('imgrectangle') ;
    btn.alt = btn.title = CKLang.imgmapRectangle;
    btn = $('imgcircle') ;
    btn.alt = btn.title = CKLang.imgmapCircle;
    btn = $('imgpolygon') ;
    btn.alt = btn.title = CKLang.imgmapPolygon;


    img_obj = CK.getSelection().getSelectedElement();
    ...
    if (!img_obj) {
        alert(CKLang.msgImageNotSelected);
        getElementsByClassName(window.parent.document, "cke_dialog_ui_button_cancel")[0].click();
        return false;
    } else {
        img_obj = img_obj.$;
    }

    // Autoselect the language based on the current CKeditor language
    // Check if the plugin has the language file for the active language.
    var sLang ;
    var AvailableLangs = CK.plugins.imgmap.lang.split(",");
    if (AvailableLangs.indexOf(CK.langCode) >= 0)
        sLang = CK.langCode;
    else
        sLang = "en"; // Load the english language file if the prefered by the user is not available.
    ...
    if ( typeof mapname == 'string' && mapname !== '') {
        ...
        var maps = CK.document.$.getElementsByTagName('MAP');
    ...
    $('btnBrowse').style.display = CKConfig.LinkBrowser ? '' : 'none' ;
...
function Ok()
{
    ...
    if (img_obj !== null && img_obj.nodeName == "IMG") {
        ...
        if (typeof map_obj == 'undefined' || map_obj === null) {
            map_obj = CK.document.$.createElement("map");
...
// An area has been removed in the image
...
{
    OpenFileBrowser(
        CKConfig.LinkBrowserURL,
        CKConfig.LinkBrowserWindowWidth,
        CKConfig.LinkBrowserWindowHeight ) ;
}