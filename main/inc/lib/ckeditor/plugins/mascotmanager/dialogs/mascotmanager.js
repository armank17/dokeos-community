...
(function()
{
	var mascotDialog = function( editor, dialogType )
	{
		// Load image preview.
		var IMAGE = 1,
			LINK = 2,
		...
		var resetSize = function( dialog )
		{
			...
			if ( oImageOriginal.data )
			{
                ...
                var setupBrowseIframe = function(type, element) {
                    ...
                    if (element && element.is('img')) {                                               
                        ...
                        if (src) {
                            ...
                            url = getBaseUrl() + editor.config.filebrowserMascotmanagerBrowseUrl+'&path='+encodeURIComponent(src)+'&ts='+ts;                            
                        }   
                    }
                    else {
                        var ts = new Date().getTime();
                        url = getBaseUrl() + editor.config.filebrowserMascotmanagerBrowseUrl+'&ts='+ts;
                    }
                    if ($("#kcfinder_mascot_iframe").length > 0) {   
                        $("#kcfinder_mascot_iframe").attr("src", url); 
                    }
                };
                ...
                {                   
                    var url = getBaseUrl() +editor.config.filebrowserMascotmanagerBrowseUrl;
                    return '<div style="width:100%;height:342px;"><iframe name="kcfinder_mascot_iframe" src="'+url+'" id="kcfinder_mascot_iframe" frameborder="0" width="100%" height="342px" style="width:100%;height:342px;" marginwidth="0" marginheight="0" scrolling="no"></iframe></div>';
                }
		...
		return {
			title : editor.lang.image[ dialogType == 'mascotmanager' ? 'title' : 'titleButton' ],
			minWidth : 850,
			minHeight : 450,
			onShow : function()
			{
			...
			onLoad : function()
			{
				...
				this.commitContent = commitContent;
                                ...
                                var ts = new Date().getTime();
                                var src = getBaseUrl() + editor.config.filebrowserMascotmanagerBrowseUrl+'&ts='+ts;
                                if ($("#kcfinder_mascot_iframe").length > 0) {
                                    $("#kcfinder_mascot_iframe").attr("src", src);
                                }
	...
	CKEDITOR.dialog.add( 'mascotmanager', function( editor )
		{
			return mascotDialog( editor, 'mascotmanager' );
		});

	CKEDITOR.dialog.add( 'imagebutton', function( editor )
		{
			return mascotDialog( editor, 'imagebutton' );
		});
})();
