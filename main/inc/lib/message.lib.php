	...
	*/
		...
		if ($unread) {
			...
			$condition_msg_status = ' msg_status IN('.MESSAGE_STATUS_NEW.','.MESSAGE_STATUS_UNREAD.') ';
                ...
                if (isset($_GET['group_id']) && $_GET['group_id'] != '') {
                    $and_group = " AND group_id = '" . $_GET['group_id'] . "' ";
                }
		if (api_is_allowed_to_edit()) {
                    $sql_query = "SELECT COUNT(*) as number_messages FROM $table_message WHERE user_receiver_id='".api_get_user_id()."' AND " . $condition_msg_status . $and_group;
		} else {
                    $sql_query = "SELECT COUNT(*) as number_messages FROM $table_message WHERE user_receiver_id=" . api_get_user_id() ." AND ".$condition_msg_status . $and_group;
		}
	...
	public static function show_message_box($message_id, $source = 'inbox') {
		...
		<table class="message_view_table actions">
				...
				*/
				if ($source == 'outbox') {
					//$message_content .='<td>'.get_lang('From').' '.$user_image.'<a href="'.api_get_path(WEB_PATH).'main/social/profile.php?u='.$row[1].'">'.GetFullUserName($row[1]).'</a> '.api_strtolower(get_lang('To')).'&nbsp;<b>'.GetFullUserName($row[2]).'</b> </TD>';
                                        $message_content .='<td>'.get_lang('From').' '.$user_image.sprintf('<a id="user_id_%s" class="user_info" href="javascript:void(0);">', $row[1]).GetFullUserName($row[1]).'</a> '.api_strtolower(get_lang('To')).'&nbsp;<b>'.GetFullUserName($row[2]).'</b> </TD>';
				} else {
					$message_content .='<td>'.get_lang('From').' '.$user_image.sprintf('<a id="user_id_%s" class="user_info" href="javascript:void(0);">',$row[1]).GetFullUserName($row[1]).'</a> '.api_strtolower(get_lang('To')).'&nbsp;<b>'.get_lang('Me').'</b> </TD>';
		 ...
		 $message_content .='</tr>
		              <tr>
		              <td>'.get_lang('Date').'&nbsp; '. TimeZone::ConvertTimeFromUserToServer(api_get_user_id(), $row[4]) .'</TD>
		              </tr>
		            </tr>
		        </table>
		        <br />
		...
		if (is_array($rows) && count($rows)> 0) {
				...
				foreach ($topics as $index => $topic) {
					...
					$html .= '<div class="social-box-container2 " >';
						$html .= '<div class="social-box-content2 quiz_content_actions" style="padding:10px">';
							$html .= '<a href="javascript:void(0);" class="head" id="head_'.$topic['id'].'">';
							$html .= '<span class="message-group-title-topic" id="'.$topic['id'].'" data-display="true" data-images=' . "'{\"hide\": {\"alt\": \"".get_lang("show")."\", \"title\": \"".get_lang("show")."\", \"src\": \"".api_get_path(WEB_PATH)."main/img/div_show.gif\"}, \"show\": {\"alt\": \"".get_lang("hide")."\", \"title\": \"".get_lang("hide")."\", \"src\": \"".api_get_path(WEB_PATH)."main/img/div_hide.gif\"}}'". '>'.(((isset($_GET['anchor_topic']) && $_GET['anchor_topic'] == 'topic_'.$topic['id']) || in_array('items_'.$topic['id'].'_page_nr',$param_names))?Display::return_icon('div_show.gif',get_lang("show"),array('style'=>'vertical-align: middle')):
										Display::return_icon("div_hide.gif",get_lang("hide"),array('style'=>'vertical-align: middle'))).'
							...
							$html .= '<div id="topic_'.$topic['id'].'">';
								...
								$html.= '<div style="margin-bottom:10px">';
									$html.= '<div id="message-reply-link" style="margin-right:10px">
											<a href="'.api_get_path(WEB_CODE_PATH).'social/topic_add.php?view_panel=1&amp;height=390&amp;width=610&amp;user_friend='.$current_user_id.'&amp;group_id='.$group_id.'&amp;message_id='.$topic['id'].'&amp;action=reply_message_group&amp;anchor_topic=topic_'.$topic['id'].'&amp;topics_page_nr='.intval($_GET['topics_page_nr']).'&amp;items_page_nr='.intval($_GET['items_page_nr']).'"  title="'.get_lang('Reply').'">'.Display :: return_icon('pixel.gif', get_lang('Reply'),array('class'=>'actionplaceholdericon actionforum')).'</a>';
									...
									if (($my_group_role == GROUP_USER_PERMISSION_ADMIN || $my_group_role == GROUP_USER_PERMISSION_MODERATOR) || $topic['user_sender_id'] == $current_user_id) {
										$html.= '&nbsp;&nbsp;<a href="'.api_get_path(WEB_CODE_PATH).'social/topic_add.php?view_panel=1&amp;height=390&amp;width=610&amp;user_friend='.$current_user_id.'&amp;group_id='.$group_id.'&amp;message_id='.$topic['id'].'&amp;action=edit_message_group&amp;anchor_topic=topic_'.$topic['id'].'&amp;topics_page_nr='.intval($_GET['topics_page_nr']).'&amp;items_page_nr='.intval($_GET['items_page_nr']).'"  title="'.get_lang('Edit').'">'.Display :: return_icon('pixel.gif', get_lang('Edit'),array('class'=>'actionplaceholdericon actionedit')).'</a>';
									}
					...
					if (is_array($topic['items'])) {
						...
						foreach ($topic['items'] as $item) {
							...
							$html_items.= '<div class="social-box-container3" >';
								...
								$html_items.=	'<a href="'.api_get_path(WEB_CODE_PATH).'social/topic_add.php?view_panel=1&amp;height=390&amp;width=610&amp;user_friend='.api_get_user_id().'&amp;group_id='.$group_id.'&amp;message_id='.$item['id'].'&amp;action=reply_message_group&amp;anchor_topic=topic_'.$topic['id'].'&amp;topics_page_nr='.intval($_GET['topics_page_nr']).'&amp;items_page_nr='.intval($items_page_nr).'&topic_id='.$topic['id'].'"  title="'.get_lang('Reply').'">'.Display :: return_icon('pixel.gif', get_lang('Reply'),array('class'=>'actionplaceholdericon actionforum')).'</a>';
								...
								if (($my_group_role == GROUP_USER_PERMISSION_ADMIN || $my_group_role == GROUP_USER_PERMISSION_MODERATOR) || $item['user_sender_id'] == $current_user_id) {
									$html_items.= '&nbsp;&nbsp;<a href="'.api_get_path(WEB_CODE_PATH).'social/topic_add.php?view_panel=1&amp;height=390&amp;width=610&amp;user_friend='.$current_user_id.'&amp;group_id='.$group_id.'&amp;message_id='.$item['id'].'&amp;action=edit_message_group&amp;anchor_topic=topic_'.$topic['id'].'&amp;topics_page_nr='.intval($_GET['topics_page_nr']).'&amp;items_page_nr='.intval($items_page_nr).'&topic_id='.$topic['id'].'"  title="'.get_lang('Edit').'">'.Display :: return_icon('pixel.gif', get_lang('Edit'),array('class'=>'actionplaceholdericon actionedit')).'</a>';
								}
								$html_items.= '</div>';
                                                                $item1 = explode(':', $item['title']);
								$html_items.= '<div class="message-group-title">'.  get_lang("Reply").Security::remove_XSS($item1[1]).'&nbsp;</div>';
...
//	$charset = api_get_setting('platform_charset');
    ...
    if (isset($_GET['group_id']) || isset($_REQUEST['f'])) {
        if ($_GET['group_id'] != '') {
            $parameters['group_id'] = $_GET['group_id'];
        }
...
function outbox_display() {
	...
	$table = new SortableTable('message_outbox', array('MessageManager','get_number_of_messages_sent'), array('MessageManager','get_message_data_sent'),3,20,'DESC');
        if (isset($_GET['group_id']) || isset($_GET['f'])) {
            if ($_GET['group_id'] != '') {
                $parameters['group_id'] = $_GET['group_id'];
            }
            if ($_GET['f'] != '') {
