 ...
 */
class WorkManager {
    private $id,$url,$title,$description,$author,$active;
    ...
    /**
     * set the data of WorkManager
     * @param type $id id of work
     */
    ...
    function search_engine_delete() {
        ...
        if (api_get_setting('search_enabled') == 'true' && extension_loaded('xapian') && is_writable($search_db_path)) {
            ...
            $sql = sprintf($sql, $tbl_se_ref, $course_id, TOOL_STUDENTPUBLICATION, $this->id);
    ...
    function search_engine_edit() {
        ...
        if (api_get_setting('search_enabled') == 'true' && extension_loaded('xapian') && is_writable($search_db_path)) {
            ...
            if (Database::num_rows($res) > 0) {
                 ...
                 $name = substr($this->url, 1, strlen($this->url));
                $ic_slide->addValue('title',$name);
                $ic_slide->addCourseId($course_id);
                $ic_slide->addToolId(TOOL_STUDENTPUBLICATION);
                $xapian_data = array(
                    SE_COURSE_ID => $course_id,
                    SE_TOOL_ID => TOOL_STUDENTPUBLICATION,
                    SE_DATA => array('type' => SE_DOCTYPE_STUDENT_PUBLICATION, 'student_publication_id' => (int) $this->id),
                    SE_USER => (int) api_get_user_id(),
                );
                $ic_slide->xapian_data = serialize($xapian_data);
                $std_pbl_description = !empty($this->description) ? $this->description : $name;
                
                //get the keyword
                $searchkey = new SearchEngineManager();
                $add_extra_terms = $searchkey->getKeyWord(TOOL_STUDENTPUBLICATION, $this->id);

                $file_content = $add_extra_terms .' '. $std_pbl_description.' ';
                ...
                $ic_slide->addValue("content", $file_content);
                 ...
                 if ($did) {
                  ...
                  $sql = 'INSERT INTO %s (id, course_code, tool_id, ref_id_high_level, search_did)
                                    VALUES (NULL , \'%s\', \'%s\', %s, %s)';
                  $sql = sprintf($sql, $tbl_se_ref, $course_id, TOOL_STUDENTPUBLICATION, $this->id, $did);
                  api_sql_query($sql, __FILE__, __LINE__);
                  return true;
                 }
            }
