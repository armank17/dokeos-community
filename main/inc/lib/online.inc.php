...
function WhoIsOnline($uid=0,$statistics_database='',$valid)
{
	...
	$query = "SELECT login_user_id,login_date FROM ".$track_online_table ." WHERE DATE_ADD(login_date,INTERVAL $valid MINUTE) >= '".$current_date."' AND login_user_id<>2 ";
	...
	if ($_configuration['multiple_access_urls']==true) {
		...
		if ($access_url_id != -1){
			...
			WHERE access_url_id =  $access_url_id AND DATE_ADD(login_date,INTERVAL $valid MINUTE) >= '".$current_date."' AND login_user_id<>2 ";
		}
	}
...
/**
 * Make a popup sortable table with the list of tutors to course.
 * The users array use: CourseManager::get_emails_of_tutors_to_course($course_id)
 * 
 * @author Ricardo Garcia Rodriguez <master.ojitos@gmail.com>
 * 
 * @param array $users Course teachers array
 * @param string $type User type: coachs | teachers
 * 
 * @return string Contains the html popup
 */
function display_tutors_list($users, $type) {
    if (!is_array($users)) return;
    $table_data = array();
    foreach ($users as $user_id => $user) {
        foreach ($user as $email => $name) {
            $image_array = UserManager::get_user_picture_path_by_id($user_id, 'system', false, true);
            $friends_profile = SocialManager::get_picture_user($user_id, $image_array['file'], 80, USER_IMAGE_SIZE_ORIGINAL);
            $link = Display::encrypted_mailto_link($email, ':person_name');
            $table_row = array();
            $table_row[] = str_replace(':person_name', '<img width="64" height="64" src="' . $friends_profile['file'] . '" alt="' . $name . '" title = "' . $name . '" />', $link);
            $table_row[] = '<div style="font-size:11px;">' . str_replace(':person_name', $name, $link) . '</div>';
            $table_data[] = $table_row;
        }
    }
    echo '<div id="view_' . $type . '_dialog" style="display:none;">';
    Display::display_sortable_table('', $table_data, array(), array('per_page' => 25), '', array(), 'ajax_grid');
    echo '</div>
    <script type="text/javascript">
        $(document).ready(function() {
            $("#view_' . $type . '").click(function(){
                $("#view_' . $type . '_dialog").dialog({modal: true, title: "'.get_lang('Course' . ucwords($type)).'",width: "600px"});
            });
         });
    </script>';
}