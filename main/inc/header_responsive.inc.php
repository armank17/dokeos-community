...
/**
 * ==============================================================================
 * This script displays the Dokeos header with responsive design.
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<?php echo $document_language; ?>" lang="<?php echo $document_language; ?>">
    ...
    ?>
        ...
        <link rel="stylesheet" href="<?php echo api_get_path(WEB_CODE_PATH); ?>css/dokeos2_black_tablet/templates.css" type="text/css" />
        <!--[if IE 7]>
    ...
    if (defined('DOKEOS_HOMEPAGE') && DOKEOS_HOMEPAGE)
        $onload = ' onload="javascript:if(document.formLogin) { try{ document.formLogin.login.focus(); }catch(e){} }"';
    else if (defined('DOKEOS_EXERCISE') && DOKEOS_EXERCISE)
        $onload = ' onload="javascript:if(document.exercise_admin) { try{ document.exercise_admin.exerciseTitle.focus(); }catch(e){} }"';
    else if (defined('DOKEOS_QUIZGALLERY') && DOKEOS_QUIZGALLERY)
        $onload = ' onload="javascript:if(document.question_admin_form) { try{ document.question_admin_form.questionName.focus(); }catch(e){} }"';
    else if (defined('DOKEOS_GLOSSARY') && DOKEOS_GLOSSARY)
        $onload = ' onload="javascript:showGlossary(\'A - Z\',\'0\')"';
    else
        $onload = '';
    ?>
    </head>
    <body class="back_main"<?php echo (isset($text_dir) && !empty($text_dir)) ? ' dir="' . $text_dir . '"' : ''; echo $onload; ?>>
