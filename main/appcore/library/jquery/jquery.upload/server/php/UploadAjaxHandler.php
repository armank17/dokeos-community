...
class UploadAjaxHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        if (isset($options['accept_file_types'])) {
            $accept_file_types = $options['accept_file_types'];
        }
        if (isset($options['thumbnail']['max_width'])) {
            $max_width = $options['thumbnail']['max_width'];
        }
        if (isset($options['thumbnail']['max_height'])) {
            $max_height = $options['thumbnail']['max_height'];
        }
        $this->options = array(
    ...
    protected function save($filename, $origin_name) {
        if (isset($this->options['course_code'])) {                                    
            $courseInfo = api_get_course_info($this->options['course_code']);
            $imagePath = $this->options['upload_dir'].$filename;
            $fileSize = filesize($imagePath);
            $saveFilePath = str_replace(api_get_path(SYS_COURSE_PATH).$courseInfo['path'].'/document', '', $imagePath);
    ...
    protected function clean_filename($string) {        
        $string = trim($string);
        $string = str_replace(array('á', 'à', 'ä', 'â', 'ª', 'Á', 'À', 'Â', 'Ä', 'æ'), array('a', 'a', 'a', 'a', 'a', 'A', 'A', 'A', 'A', 'a'), $string);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        $origin_name = api_utf8_decode($file->name);
        $file->name = $this->clean_filename($origin_name);
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    if (move_uploaded_file($uploaded_file, $file_path)) {
                        $this->save($file->name, $origin_name);
                    }
                }
            } else {
