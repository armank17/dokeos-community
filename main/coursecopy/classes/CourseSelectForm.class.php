...
class CourseSelectForm {
     ...
     */
    public function display_form($course, $hidden_fields = null, $avoid_serialize = false) {
        ...
        $resource_titles[RESOURCE_EVALUATION] = get_lang('Exams');
        ...
        $resource_titles[RESOURCE_CERTIFICATE] = get_lang('Certificate');
        ?>
        <script language="JavaScript" type="text/javascript">
            ...
            function checkLearnPath(message){
                ...
                for (i = 0; i < d.elements.length; i++) {
                    if (d.elements[i].type == "checkbox") {
                        var name = d.elements[i].attributes.getNamedItem('name').nodeValue;
                        if(name.indexOf('exam') > 0){
							setCheckbox('quiz',true);
							setCheckbox('certificate',true);
						}
    ...
    public function display_hidden_quiz_questions($course) {
    ...
    public function display_hidden_scorm_directories($course) {
        if (is_array($course->resources)) {
            foreach ($course->resources as $type => $resources) {
                if (count($resources) > 0) {
                    switch ($type) {
     ...
     */
    public function get_posted_course($from = '', $session_id = 0, $course_code = '') {
        ...
        if (is_array($course->resources)) {
            foreach ($course->resources as $type => $resources) {
                switch ($type) {
                    ...
                    case RESOURCE_LINK :
                        foreach ($resources as $id => $obj) {
                            if (!in_array($obj->source_id, array_keys($_POST['resource'][RESOURCE_LINK]))) {
                                unset($course->resources[$type][$id]);
                            }
                        }
                        break;
                    ...
                    case RESOURCE_FORUM :
                        foreach ($resources as $id => $obj) {
                            if (!in_array($obj->source_id, array_keys($_POST['resource'][RESOURCE_FORUM]))) {
                                unset($course->resources[$type][$id]);
                            }
                        }
                        break;
     ...
     * @param boolean the document array will be serialize. This is used in the course_copy.php file
     */
    public function display_form_session_export($list_course, $hidden_fields = null, $avoid_serialize = false) {
        ?>
        <script language="JavaScript" type="text/javascript">
            function exp(item) {
                el = document.getElementById('div_'+item);
