...
function display_form() {
	...
	$html .= '<div class="normal-message" style="padding:10px;clear:both;">'.$introduction.'</div>';
	...
	$html .= '<div class="pull-bottom "><button class="save" type="submit" onclick="javascript:if(!confirm('."'".addslashes(api_htmlentities(get_lang("ConfirmYourChoice"),ENT_QUOTES,$charset))."'".')) return false;">'.get_lang('CopyCourse').'</button></div></div>';
...
if ((isset ($_POST['action']) && $_POST['action'] == 'course_select_form') || (isset ($_POST['copy_option']) && $_POST['copy_option'] == 'full_copy')) {
	...
	if (isset ($_POST['action']) && $_POST['action'] == 'course_select_form') {
		...
		Display::display_normal_message(get_lang('CopyFinished'),false,true);
		display_form();
	} else {
		...
		if ((is_array($arr_course_origin) && count($arr_course_origin) > 0)  && !empty($destination_session)) {
			...
			Display::display_error_message(get_lang('YouMustSelectACourseFromOriginalSession'),false,true);
			display_form();
		}
	...
	if (api_get_setting('show_glossary_in_documents') != 'none' && !((is_array($arr_course_origin) && count($arr_course_origin) > 0) && !empty($destination_session))) {
		Display::display_normal_message(get_lang('ToExportDocumentsWithGlossaryYouHaveToSelectGlossary'),false,true);
	}

	if ((is_array($arr_course_origin) && count($arr_course_origin) > 0) && !empty($destination_session)) {
                if (api_get_setting('show_glossary_in_documents') != 'none') {
                    $add_message = ' ' .get_lang('ToExportDocumentsWithGlossaryYouHaveToSelectGlossary');
                }
		Display::display_normal_message(get_lang('ToExportLearnpathWithQuizYouHaveToSelectQuiz').$add_message,false,true);
		$cb = new CourseBuilder();
