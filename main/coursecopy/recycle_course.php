...
if(  (isset($_POST['action']) && $_POST['action'] == 'course_select_form' ) || (isset($_POST['recycle_option']) && $_POST['recycle_option'] == 'full_backup' ) ) {
	...
	$cr->recycle();
	Display::display_confirmation_message('<a href="'.api_get_path(WEB_COURSE_PATH).$_course['official_code'].'/index.php'.$param.'" target="'.$target.'">'.get_lang('RecycleFinished').'</a>', false,true);
	...
	if( ! $course->has_resources()) {
		Display::display_warning_message( get_lang('NoResourcesToRecycle'),false, true);
       
	} else {
		Display::display_warning_message(get_lang('RecycleWarning'),false, true);
