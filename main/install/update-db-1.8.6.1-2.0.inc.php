              ...
              else
              {
                ...
                if($log)
                {
                  error_log("In ".$row_course['db_name'].", executed: $query",0);
					...
					mysql_query("INSERT INTO ".($singleDbForm?"$prefix{$row_course['db_name']}_document":"document")." (path,title,filetype,size) VALUES ('/mindmaps','mindmaps','folder','0')");
					mysql_query("INSERT INTO ".($singleDbForm?"$prefix{$row_course['db_name']}_item_property":"item_property")." (tool,insert_user_id,insert_date,lastedit_date,ref,lastedit_type,lastedit_user_id,to_group_id,to_user_id,visibility) VALUES ('document',1,NOW(),NOW(),".mysql_insert_id().",'DocumentAdded',1,0,NULL,0)");
										
					// fix a default order for quiz (new field)
					$sql = 'SELECT id FROM '.($singleDbForm?"$prefix{$row_course['db_name']}_quiz":"quiz").' ORDER BY title ASC';
					$rsQuiz = mysql_query($sql);
					$i = 0;
					while($rsQuiz = mysql_fetch_array($rs)){
						$i++;
						$sql = 'UPDATE '.($singleDbForm?"$prefix{$row_course['db_name']}_quiz":"quiz").' SET position='.$i.' WHERE id = '.$quiz['id'];
						mysql_query($sql);
					}
