<div id="announcement-content">
    ...
    <div id="announcement-content-right">
        ...
        echo '<div class="announcement_title "><span style="font-size:14px;font-weight:bold; padding-left: 5px;    padding-right: 5px;">' . $ann_title . '</span></div> ';
       
        if (ereg("MSIE", $_SERVER["HTTP_USER_AGENT"])) {
            echo '	<br/><div class="custom-" style="min-height: 30px;    overflow: auto;     padding: 5px; ">' . $ann_content . '</div>';
        } else {
            echo '	<br/><div class="custom-" style="min-height: 30px;    overflow: auto;     padding: 5px; ">' . $ann_content . '</div>';
        }
 ...
 echo '	<div class="announcement_metadata">';
        echo '		<div class="announcement_date"style="padding-right:7px; padding-top:5px;" >' . TimeZone::ConvertTimeFromServerToUser(api_get_user_id(), strtotime($ann_date), 'd-m-Y') . '</div>';
        echo '		<div class="announcement_sender" style="padding:5px;"><a href="javascript:void(0);" class="user_info" id="user_id_' . $insert_user_id . '">' . $firstname . ' ' . $lastname . '</a></div>';
        echo '	</div>';
        ...
        if (api_is_allowed_to_edit() || (api_get_course_setting('allow_user_edit_announcement') && api_get_user_id() == $insert_user_id)) {
            ...
            echo '<a href="index.php?' . api_get_cidreq() . '&amp;action=edit&amp;id=' . $announcement_id . '">' . Display::return_icon('pixel.gif', get_lang('EditAnnouncement'), array('align' => 'middle', 'class' => 'actionplaceholdericon actionedit')) . ' ' . get_lang('EditAnnouncement') . '</a>';
            echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
            $link  = api_get_self() . '?' . api_get_cidreq() . '&amp;action=delete&amp;id=' . $announcement_id;
