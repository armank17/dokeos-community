...
$htmlHeadXtra[] = '
    ...
    function checkEnter(ev)
    {
        ...
        if (ev.keyCode == 13)
        {
            document.getElementById("kwdswere_string").value = document.getElementById("kwds_string").value;
...
if (!$query_string)
    $query_string = trim($mdsc);
$ckwcdt = file_exists($ckwc = KEYWORDS_CACHE . 'c') ?
        date('Y/m/d H:i:s', filemtime($ckwc)) : '?';
$pkwc = '';
...
if ($result_message == phpdigMsg('noresults'))
    $result_message .= ' ' . phpdigMsg('on') . ' "' . htmlspecialchars($query_string, ENT_QUOTES) . '"';

echo $result_message, '<br><br><table border="1">', "\n";

if ($phpdigSearchResults['results'])
    foreach ($phpdigSearchResults['results'] as $searchResult) {
...
if ($result_message && ($hits > 10))
    echo "Results page ", str_replace('?template_demo=', '?kwdswere_string=' . urlencode($kwdswere_string), $pages_bar), '<br><br>';
...
function tdhtm($xc) {
    ...
    if (($eov = strpos($xc, '>')) == FALSE)
        ...
        $value = substr($xc, 0, $eov);
        $label = substr($xc, $eov + 1);

        if (($eot = strpos($label, '<')) !== FALSE) {
            $eol = substr($label, $eot);
            $label = substr($label, 0, $eot);
        }
    }
...
<div onMouseUp="if ((kw = pU_clicked(event)))
            pU_select(kw);
        else
            pU_hide();">

    <input type="text" id="kwds_string" class="kwl" onKeyUp="takeTypeIn(this, 150, -100, '60%');
        checkEnter(event);
        return true;"/>
...
if (api_is_allowed_to_edit())
    ...
    echo '<textarea name="mdsc" id="mdsc" class="dvc"> </textarea>';
?>
                </td>
                <td>
                    <input type="hidden" id="kwdswere_string" name="kwdswere_string"/>
                    <input type="submit" id="form_submit" onClick="document.getElementById('kwdswere_string').value = document.getElementById('kwds_string').value;
        return prepSearch(event);" value="<?php echo phpdigMsg('go') ?>"/>
                </td>
            </tr>
        </table>
...
<?php if ($keywordscache == '') { ?> &#xa0; <?php } else { ?>
            ...
            <input type="button" class="btn" value="+" onClick="if (this.value == '+')
                deselectAll(event, this);
            openOrClose(this);"/>
