...
<div id="dataDiv">
<a class="pull-right" id="report_back" href="index.php?user_id=<?php echo $user_id; ?>&course_code=<?php echo $_GET['course_code']?>&rank=<?php echo $_GET['rank']?>&status=<?php echo $_GET['status']?>&trainer_id=<?php echo $_GET['trainer_id']?>&course_search=<?php echo $course_search; ?>&search=<?php echo $_GET['search']?>"><?php echo api_convert_encoding(get_lang("BackToList"),"UTF-8",api_get_system_encoding()); ?></a>
<h5><?php echo get_lang("IndividualReporting"); ?> : <?php echo api_convert_encoding(Database::escape_string($user_info['lastname']),'UTF-8',api_get_system_encoding()).' '.api_convert_encoding(Database::escape_string($user_info['firstname']),'UTF-8',api_get_system_encoding()); ?><h5>
<table>
	<thead>
		<tr>
			<th><?php echo api_convert_encoding(get_lang("OverallInformation"),"UTF-8",api_get_system_encoding()); ?></th>
			<th><?php echo api_convert_encoding(get_lang("AccessDetails"),"UTF-8",api_get_system_encoding()); ?></th>
			<th><?php 
	...
	<tbody>
		<tr>
			...
			<p><?php echo $quiz_score; ?></p>-->
				<table class="responsive large-only table-striped" >
					<tr><td><?php echo api_convert_encoding(get_lang("Status"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo $status; ?></td></tr>
					<tr><td><?php echo api_convert_encoding(get_lang("Email"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo wordwrap($user_info['mail'],5,"<br/>\n"); ?></td></tr>
					<tr><td><?php echo api_convert_encoding(get_lang("FirstConnection"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo api_convert_encoding($first_connection_date,'UTF-8',api_get_system_encoding()); ?></td></tr>
					<tr><td><?php echo api_convert_encoding(get_lang("LatestConnection"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo api_convert_encoding($last_connection_date,'UTF-8',api_get_system_encoding()); ?></td></tr>
					<tr><td><?php echo api_convert_encoding(get_lang("TimeSpentInModules"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo $time_spent; ?></td></tr>
					<tr><td><?php echo api_convert_encoding(get_lang("ScoreInModules"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo $score; ?></td></tr>
					<tr><td><?php echo api_convert_encoding(get_lang("ProgressInModules"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo $progress; ?></td></tr>
					<tr><td><?php echo api_convert_encoding(get_lang("QuizzesScore"),"UTF-8",api_get_system_encoding()); ?> :</td><td><?php echo $quiz_score; ?></td></tr>
				</table>
			</td>
		</tr>
	</tbody>
...
foreach($course_codes as $code) {
	...
	if(sizeof($modules) > 0 || sizeof($quizzes) > 0) {
		...
		if(!empty($course_search)){
			...
			echo '<b>'.api_convert_encoding(Database::escape_string($course_info['name']),'UTF-8',api_get_system_encoding()).'</b></h5>';
		}
		else {
		?>
		<b><?php echo api_convert_encoding(Database::escape_string($course_info['name']),'UTF-8',api_get_system_encoding()); ?></b></h5>
		<?php
		}
	if(!empty($session_id)) {
	...
	$cnt_i = 0;
	$largest_row = 0;
	if(api_get_setting('enable_course_scenario') == 'true' && $num_scenario > 0){
		echo '<h5>'.api_convert_encoding(get_lang("ScenarioOverview"),'UTF-8',api_get_system_encoding()).'</h5>';
		...
		for( $i = 0; $i < $row; $i++ )
		{
			...
			for( $j = 0; $j < $col; $j++ ) {  
					if($i == 0){
						echo '<td><b>'.api_convert_encoding($foo[$j][$i],'UTF-8',api_get_system_encoding()).'</b></td>';        
					}
					else {
					...
					if($activity_status == 'completed'){
						...
						if($activity_type == 'face2face' || $activity_type == 'quiz' || $activity_type == 'module'){
							echo '<td style="background:#579D1C;color:#FFF;">'.api_convert_encoding($activity_name,'UTF-8',api_get_system_encoding());
							...
							echo '<td style="background:#579D1C;color:#FFF;">'.api_convert_encoding($activity_name,'UTF-8',api_get_system_encoding()).'</td>'; 
						}
					
					}
					else {		
						
						echo '<td>'.api_convert_encoding($activity_name,'UTF-8',api_get_system_encoding()).'</td>'; 						
					}
					}
			}
			echo '</tr>';
	...
	<table class="responsive large-only table-striped" id="ind_modules">
		<thead>
			<tr>
				<th><?php echo api_convert_encoding(get_lang("Module"),"UTF-8",api_get_system_encoding()); ?></th>
				...
				<th><?php echo api_convert_encoding(get_lang("Screens"),"UTF-8",api_get_system_encoding()); ?></th>							
			</tr>
		</thead>
		<tbody>
	<?php
		foreach($modules as $module) {
			...
			if(!empty($course_search)){
				...
				echo "<td>".api_convert_encoding($module['name'],'UTF-8',api_get_system_encoding())."</td>";
			}
			else {
				echo "<td>".api_convert_encoding($module['name'],'UTF-8',api_get_system_encoding())."</td>";
			...
			echo "<td align='center'>".display_time_format($module_time)."</td>
					...
					echo "<td align='center'><a id='module_result' href='module_result.php?action=stats&course_code=".$code."&course=".$code."&user_id=".$user_id."&student_id=".$user_id."&lp_id=".$module["id"]."&sessionId=".$session_id."'><img src='$pathStyleSheets/images/action/reporting32.png'></a></td>";
					}
					else {
	...
	if(sizeof($quizzes) > 0) {
		...
		<thead>
			<tr>
				<th><?php echo api_convert_encoding(get_lang("StandaloneQuiz"),"UTF-8",api_get_system_encoding()); ?></th>
	...
	<?php
		...
		foreach($quizzes as $quiz) {
			...
			if(!empty($course_search)){
				...
				echo "<td>".api_convert_encoding($quiz['title'],'UTF-8',api_get_system_encoding())."</td>";
			}
			else {				
				echo "<td>".api_convert_encoding($quiz['title'],'UTF-8',api_get_system_encoding())."</td>";
			}
			echo "	<td align='center'>".$attempts."</td>
					...
					echo "<td align='center'><a id='individual_result' href='quiz_result.php?action=qualify&quiz_id=".$quiz["id"]."&course_code=".$code."&exe_id=".$exe_id."&user_id=".$user_id."&session_id=".$session_id."&from=individual'><img src='$pathStyleSheets/images/action/reporting32.png'></a></td>";
					}
				  echo "</tr>";	
		}
		echo "</tbody>
