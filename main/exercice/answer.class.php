...
/**
	 ...
	 */
	function Answer($questionId, $db_name = '')
	{
	...
	{
		global $_course;
		$TBL_ANSWER = Database::get_course_table(TABLE_QUIZ_ANSWER, $this->db_name);
		...
		if($_GET['fromTpl'] <> 0){
			...
			while($object=mysql_fetch_object($result))
			{
				...
				if ($object->correct==1) {
					$this->new_nbrCorrects++;
                                ...
                                //$this->comment[$i] = ($this->comment[$i]!='')?$this->comment[$i]:'';
				if((str_word_count($object->comment) <= 3)&&(str_word_count($object->comment) > 0)){		
			...
			while($object=mysql_fetch_object($result))
			{
				$this->id[$i]=$object->id;
				$this->answer[$i]=$object->answer;
                                $this->answer[$i]=str_replace("<p>&nbsp;</p>","",$this->answer[$i]);                                
				$this->correct[$i]=$object->correct;
				if ($object->correct==1) {
					$this->new_nbrCorrects++;
				}
