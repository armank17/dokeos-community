...
require_once(api_get_path(SYS_CODE_PATH).'exercice/hotspot.class.php');
require_once(api_get_path(SYS_CODE_PATH).'exercice/reasoning.class.php');
...
class ScormQuestion extends Question
{
    ...
    function getQuestionHTML()
    {
		...
		$cols = 2;
		$s='<tr>' .
			'<td colspan="'.$cols.'" id="question_'.$this->id.'_title" valign="middle" style="background-color:#ffffff;padding:10px;background-color:#00AFFA;color:#FFFFFF;">' . "\n" .
		   	api_parse_tex($title).
		   	'</td>' . "\n" .
		   	'</tr>' . "\n" .
		   	'<tr>' . "\n" .
		   	'<td valign="top" colspan="'.$cols.'">' . "\n" .
		   	'<i><div style="border-spacing:10px;border:1px solid #000;color:#4B4B4B;background-color:#ffffff;">'.api_parse_tex($description).'</div></i>' . "\n" .
		   	'</td>' . "\n" .
		   	'</tr>' . "\n";
		return $s;
    }
...
class ScormAnswerMultipleChoice extends Answer
{
	...
	function export()
	{
		$html = '';
    	$js   = '';
    	$html = '<tr><td colspan="2"><table width="100%" style="border-collapse:separate;border-spacing:10px;border:1px solid #000;color:#4B4B4B;background-color:#CDC8C1;">' . "\n";
...
class ScormAnswerHotspot extends Answer
{
	...
	function get_js_header()
	{
		if($this->standalone)
		{
			...
			//because this header closes so many times the <script type="text/javascript"> tag, we have to reopen our own
			$header .= '<script type="text/javascript" language="javascript">'."\n";
