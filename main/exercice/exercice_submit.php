...
$htmlHeadXtra[] = '<script type="text/javascript" src="' . api_get_path(WEB_LIBRARY_PATH) . 'javascript/responsive/js/jquery.ui.touch-punch.min.js" language="javascript"></script>';
//$htmlHeadXtra[] = '<script type="text/javascript" src="' . api_get_path(WEB_CODE_PATH) . 'exercice/exam_mode/js/exam_mode.js" language="javascript"></script>';
$htmlHeadXtra[] = '<script type="text/javascript">        
                                ...
                                for ($i = 0; $i < count($real_correct_tags); $i++) {
                                    ...
                                    if (!$switchable_answer_set) {
                                        ...
                                        elseif (!empty($user_tags[$i])) {
                                            ...
                                            $answer .= '<font color="red"><s>'.(mb_detect_encoding($user_tags[$i]) != 'UTF-8'?($user_tags[$i]):$user_tags[$i]).'</s></font>';
                                        } else {
...
//                    echo '<script language="javascript" type="text/javascript">' . $href . '</script>' . "\n";
                ...
                if($tool == 'scenario') {
                                            unset($_SESSION['expired_time'][$course_code][intval($_SESSION['id_session'])][$exerciseId][$safe_lp_id]);
                                            unset($_SESSION['end_expired_time'][$exerciseId][$safe_lp_id]);
                                            header("Location: ".api_get_path(WEB_CODE_PATH)."exercice/exercise_show.php?".api_get_cidreq()."&id=$exe_id&exerciseType=$exerciseType&quizpopup=1&origin=$origin&learnpath_id=$learnpath_id&learnpath_item_id=$learnpath_item_id&exerciseid=$exerciseId&step=$step_id&activity_id=$activity_id&tool=scenario");
				}
				else {
                                            unset($_SESSION['expired_time'][$course_code][intval($_SESSION['id_session'])][$exerciseId][$safe_lp_id]);
                                            unset($_SESSION['end_expired_time'][$exerciseId][$safe_lp_id]);
...
//|| api_is_course_admin() == 1 || api_is_course_coach() == 1 || api_is_course_tutor() == 1
if(api_is_coach() == 1){
    unset($_SESSION['expired_time']);
...
// Get real question ID
    ...
    if (!isset($_REQUEST['action'])) {
        ...
        foreach ($questionList as $questionId) {
            ...
            showQuestion($questionId, false, $origin, $i, $nbrQuestions, $quiz_context,$simplifyQuestionsAuthoring);
            if ($exerciseType == 1) {
                echo '</td></tr></table>';
            }
            $i++;
        ...
        showFeedback($questionId, false, $origin, $i, $nbrQuestions, $exe_id, $quiz_context,$simplifyQuestionsAuthoring);
    }
...
if ($arr_user['status'] != 1) {
    ...
    if(empty($_REQUEST["notime"])){
        $link = api_get_path(WEB_COURSE_PATH) . $_GET['cidReq'] . '/index.php';
        echo '<script>
            function load_page(){
    ...
    echo '<div style="display:none;" id="expiredTimeMsg">
        ...
        <center>
            <a id="redirect_to_homequiz" href="javascript:void(0)"; onclick="load_page();">' . get_lang('TimeOfQuizOverMsg') . '</a>
                <br/>
        </center>
        </div>';
    if ($origin == 'learnpath') {
