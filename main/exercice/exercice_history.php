	...
	if(isset($_GET['message'])) {
		if (in_array($_GET['message'], array('ExerciseEdited'))) {
			...
			//Display::display_confirmation_message(get_lang($my_message_history));
                        $_SESSION["display_confirmation_message"]=get_lang($my_message_history);
		}
	}
...
while($row = Database::fetch_array($query)){
	...
	if(!empty($row['teacher_comment'])){
		...
		echo '<td>'.get_lang('WithoutComment').'</td>';
            ...
            echo '<td>'.date('d-m-Y h:i:s a',strtotime($row['insert_date'])).'</td>';
	echo '<td>'.(empty($row['full_name'])?'<i>'.get_lang('OriginalValue').'</i>':$row['full_name']).'</td>';
	echo '</tr>';
            $i++;
}
}else{
    echo '<tr><td>' .  get_lang("langNoResult") . '</td></tr>';
}
echo '</table>';
echo '</div>';
// Actions bar
