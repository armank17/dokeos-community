    ...
    for ($i = 1; $i <= $nbrAnswers; $i++) {
        ...
        while ($row = Database::fetch_array($result)) {
			...
			if(str_word_count($row['comment']) <= 3){
                            $qn_comment[$i]=($row['comment']!='')?get_lang($row['comment']):$row['comment'];                           
			}
			else {
...
//	Question :: uploadPicture($baseName,$fileName);
        ...
        echo '<script type="text/javascript">window.location.href = "' . api_get_self() . '?' . api_get_cidReq() . '&newQuestion=yes&hotspotadmin=' . $insert_id . '&exerciseId=' . $exerciseId . '&answerType='.$type_hotspost_delineation.'&lp_id=' . $lp_id . '&next_order='.$next_order.'"</script>';
...
if ($editQuestion != '') {
    ...
    $action = api_get_self() . '?' . 'newQuestion=yes&' . api_get_cidreq() . '&answerType='.$type_hotspost_delineation.'&exerciseId=' . $exerciseId . '&hotspotadmin=' . $modifyAnswers . '&insert_id=' . $insert_id . '&next_order=' . $next_order;
}
?>
    ...
    if ($_REQUEST['hotspotadmin'] == '' && intval($editQuestion) == 0) {
        ...
        ?>
                ...
                <td colspan="2" valign="top"><table class="data_table" style="width:95%;float:right;">
                        <tr>
                            <th width="">&nbsp;</th>
                            <th width=""><?php echo get_lang('Zones'); ?>*</th>
                    ...
                    for ($i = 1; $i <= $nbrAnswers; $i++) {
                        ...
                        ?>
                            <tr  class="<?php echo $class; ?> ">
                                ...
                                <td height="30" align="left" valign="top"><input type="text" name="reponse[<?php echo $i; ?>]" value="<?php echo api_convert_encoding($reponse[$i], ENT_QUOTES, api_get_system_encoding()); ?>" size="15" style="font-size:16px;" /></td>
                                <!--<td align="left"><?php echo $return; ?></td>-->
                                <td height="30" valign="top"><input type="text" name="weighting[<?php echo $i; ?>]" size="5" style="font-size:16px;" value="<?php echo (isset($weighting[$i]) ? float_format($weighting[$i], 1) : 10); ?> " />
    ...
    ?>
            ...
            <tr>
                <td colspan="4" valign="top" >
                    <div class="coast2coast" style="text-align:left;">
...
//                            $comment[2]     = ($comment[2]=='[==]')?'':$comment[2];
                    ...
                    <div class="coast2coast" style="text-align:left;">
                        ...
                        <?php
                            api_disp_html_area('comment[2]', $comment[2], '', '', '', $editor_config);
                        ?>
                    </div>
                </td>
            </tr>
...
if ($navigator_info['name'] == 'Internet Explorer' && $navigator_info['version'] == '6') {
    ?>
                        ...
                        <input type="submit" class="save" name="submitAnswers" style="float: right ! important; margin-top: 20px;" value="<?php echo get_lang('Ok'); ?>" />
    <?php
} else {
    ?>
                            ...
                            <button type="submit" class="save" name="submitAnswers" style="float: right ! important; margin-top: 20px;" value="<?php echo get_lang('Ok'); ?>" ><?php echo get_lang('Validate'); ?></button>
                                    <?php
                                }
                                ?>
                    </div></td>
