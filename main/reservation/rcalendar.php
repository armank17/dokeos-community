...
class rCalendar {
	...
	function get_week_view($day, $month, $year, $itemid, $day_scale = 180,$cat) {
		// 0) get all the information of the item
		$item = Rsys :: get_item($itemid);
       ...
       {
			$subscriptions = Rsys :: get_item_always_available_subscriptions($fromdate, Rsys :: timestamp_to_datetime($tilldate), $itemid);
			for ($i = 1; $i <= 7; $i ++)
			{
		       	$info = array(
		       				'id'=>'',
		       				...
		       				'auto_accept'=>1,
		       				'max_users'=>1,
		       				'start_at'=>Rsys :: timestamp_to_datetime($day_start_dates[$i]),
		       				'end_at'=>Rsys :: timestamp_to_datetime($day_start_dates[$i]+60*60*24),
		       				'subscribe_from' => '0000-00-00 00:00:00',
							'subscribe_until' => '0000-00-00 00:00:00',
							'subscribers' => 0,
							'notes' => '',
							'timepicker' => 1,
							'timepicker_min' => 0,
	       		...
	       		$arr['reservations'][] =  array('info'=>$info,'subscriptions'=>$subscriptions[date('j',$day_start_dates[$i])]);
			}
       }
       /*
		echo '<pre>';
	...
	*/
	// 6) Doorloop $arr met reservatie periodes en vul de $days array op met balkjes (bars) voor elke dag
		...
		echo '</td></tr>';
		for ($i = 1; $i <= 7; $i ++) {
			// the first column with the days
