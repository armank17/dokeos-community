...
class WSUser extends WS {
	 ...
	 * @param string User id field name. Use "dokeos_user_id" as the field name if you want to use the internal user_id
	 * @param string User id value
	 */
	 ...
	 */
	protected function createUserHelper($firstname, $lastname, $status, $login, $password, $encrypt_method, $user_id_field_name, $user_id_value, $visibility, $email, $language, $phone, $expiration_date, $extras, $admin) {
		global $api_failureList;
		// Add the original user id field name and value to the extra fields if needed
		$extras_associative = array();
		if($user_id_field_name != "dokeos_user_id") {
			$extras_associative[$user_id_field_name] = $user_id_value;
		}
		foreach($extras as $extra) {
			$extras_associative[$extra['field_name']] = $extra['field_value'];
		}
		$result = UserManager::create_user($firstname, $lastname, $status, $email, $login, $password, '', $language, $phone, '', PLATFORM_AUTH_SOURCE, $expiration_date, $visibility, 0, $extras_associative, '', '', $admin);
		if (!$result) {
	 ...
	 */
	public function CreateUser($secret_key, $firstname, $lastname, $status, $login, $password, $encrypt_method, $user_id_field_name, $user_id_value, $visibility = 1, $email = '', $language = 'english', $phone = '', $expiration_date = '0000-00-00 00:00:00', $extras = array(), $admin = false) {
		// First, verify the secret key
		if ($secret_key == 'ignore') {
			$result = $this->createUserHelper($firstname, $lastname, $status, $login, $password, $encrypt_method, $user_id_field_name, $user_id_value, $visibility, $email, $language, $phone, $expiration_date, $extras, $admin);
			if ($result instanceof WSError) {
				$this->handleError($result);
			} else {
				return $result;
	 ...
	 *
	 * @param string User id field name. Use "dokeos_user_id" in order to use internal system id
	 * @param string User id value
	...
	protected function editUserHelper($user_id_field_name, $user_id_value, $firstname, $lastname, $status, $loginname, $password, $encrypt_method, $email, $language, $phone, $expiration_date, $extras) {
		...
		if($user_id instanceof WSError) {
			...
			// We update the platform language if it is required
			if (isset($extras_associative['update_platform_language']) && $extras_associative['update_platform_language'] === 'true') {
				api_set_setting('platformLanguage', $language);
			}
			
			// We add manager email for the portal in setting
			if (isset($extras_associative['set_portal_upgrade_email_contact'])) {
				if (api_check_if_setting_exist('set_portal_upgrade_email_contact', '')) {
					api_set_setting('set_portal_upgrade_email_contact', $extras_associative['set_portal_upgrade_email_contact']);
				}
				else {
					api_add_setting($extras_associative['set_portal_upgrade_email_contact'], 'set_portal_upgrade_email_contact');
				}
				unset($extras_associative['set_portal_upgrade_email_contact']);
			}
			
			$result = UserManager::update_user($user_id, $firstname, $lastname, $loginname, $password, PLATFORM_AUTH_SOURCE, $email, $status, '', $phone, $user_info['picture_uri'], $expiration_date, $user_info['active'], null, $user_info['hr_dept_id'], $extras_associative, $language);
			if (!$result) {
				$failure = $api_failureList[0];
	...
	/**
	 * Check user in dokeos
	 */
	public function ExistUsername($secret_key, $username) {
		$verifKey = $this->verifyKey($secret_key);
		if($verifKey instanceof WSError) {
			$this->handleError($verifKey);
		} else {
			$exist = 0;
			$check = (int)UserManager::is_username_available($username);
			if (!$check) {
				$user_info = api_get_user_info_from_username($username);
				$exist = $user_info['user_id'];
			}
			return (int)$exist;
		}
	}
