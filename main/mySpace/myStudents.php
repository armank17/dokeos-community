...
<?php
//$htmlHeadXtra[] = '<style type="text/css">.ui-dialog { left: 50% !important; margin-left: -470px !important; margin-top: -200px !important; position: fixed !important; top: 50% !important; }</style>';
...
if (!empty ($_GET['student'])) {
	...
	echo '<div class="actions">';
        if (isset($_GET['origin']) && $_GET['origin'] == 'tracking_course') {
               ...
               echo '<a href="'.api_get_path(WEB_CODE_PATH).'user/user.php">'.Display::return_icon('pixel.gif',get_lang('UserList'), array('class' => 'toolactionplaceholdericon toolactionadminusers')).get_lang("Back").'</a>';
        }
	...
	if(!empty($course_id)) {
		...
		foreach($courses as $course_code) {
			$avg_student_progress = Tracking :: get_avg_student_progress($info_user['user_id'], $course_code);
			//the score inside the Reporting table
			$avg_student_score = Tracking :: get_average_test_scorm_and_lp($info_user['user_id'], $course_code);

			$sum_progress = $sum_progress + $avg_student_progress;
			$sum_score = $sum_score + $avg_student_score;
		}
		$avg_student_progress = $sum_progress/$course_count;
		$avg_student_score	  = $sum_score/$course_count;
	}
	...
	if(empty($course_id)) {
		//$courses = Tracking :: get_courses_followed_by_coach($_user['user_id']);
		$sum_time_spent_on_course = 0;
		foreach ($courses as $course_code) {			
			$time_spent_on_each_course = Tracking :: get_time_spent_on_the_course($info_user['user_id'], $course_code);
			if(empty($time_spent_on_each_course)){
				$time_spent_on_each_course = 0;
			}
			$sum_time_spent_on_course = $sum_time_spent_on_course + $time_spent_on_each_course;
			$time_spent_on_the_course = api_time_to_hms($sum_time_spent_on_course);
		}
	}
	else {
...
?>
				...
				<table width="100%" class="data_table">
						...
						<td align="right">
						<?php
                        echo get_lang('progressInModule').' : ';
                        Display :: display_icon('info3.png', get_lang('ScormAndLPProgressTotalAverage'), array (
                            'align' => 'middle',
                            'hspace' => '0px'
                        ));
                        ?>
	...
	echo get_lang('scoreInModule').' : ';
	Display :: display_icon('info3.png', get_lang('ScormAndLPTestTotalAverage'), array (
		'align' => 'middle',
		'hspace' => '0px'
	));
?>
...
<?php
	...
	if (!empty ($_GET['details'])) {
		...
		$info_course = CourseManager :: get_course_information($course_code_info);
                ...
                $sessions = SessionManager::get_sessions_by_course($_GET['course']);
		...
		$nb_login = Tracking :: count_login_per_student($info_user['user_id'], $_GET['course']);
                        $table_title = $info_course['title'] . '&nbsp;|&nbsp;' . get_lang('CountToolAccess') . ' : ' . $nb_login . '&nbsp; | &nbsp;' . get_lang('Tutor') . ' : ' . stripslashes($info_course['tutor_name']) . ((!empty ($session_info['name'])) ? ' | ' . get_lang('Session') . ' : ' . $session_info['name'] : '');
...
<?php
		$headerLearnpath = array (get_lang('Learnpath'), get_lang('Time'), get_lang('Progress'), get_lang('LastConnexion'));
		...
		$sql_learnpath = "	SELECT lp.name,lp.id FROM $t_lp AS lp WHERE session_id = $session_id ORDER BY lp.name ASC";                
		...
		if (Database :: num_rows($result_learnpath) > 0) {
			...
			while ($learnpath = Database :: fetch_array($result_learnpath)) {
				...
				// calculates time
				$sql = 'SELECT SUM(total_time) FROM ' . $t_lpiv . ' AS item_view INNER JOIN ' . $t_lpv . ' AS view
				...
				if ($any_result === true) {
					...
					if ($from_myspace) {
						$from ='&from=myspace';
                                        ...
                                        ?>
                                                   <a id="runTabs"  class="dialog" title="<?php echo $learnpath['id']; ?>">
	...
	<!-- line about exercises -->
			<table class="data_table">
				<tr>
					...
					<th>
						<?php echo get_lang('Score');/*Display :: return_icon('info3.png', get_lang('LastScoreTest'), array('align' => 'middle', 'hspace' => '3px'))*/ ?>
					</th>
					<th>
		...
		//if (Database :: result($result_visibility_quizz, 0, 'visibility') == 1) {
			...
			$sql_exercices = "SELECT quiz.title,id
												FROM " . $t_quiz . " AS quiz
                                        WHERE active='1' AND session_id = $session_id ORDER BY quiz.title ASC
												";
			...
			if (Database :: num_rows($result_exercices) > 0) {
				while ($exercices = Database :: fetch_array($result_exercices)) {
					...
					$sql_last_attempt = 'SELECT exe_id FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . $exercices['id'] . '" AND exe_user_id="' . Security :: remove_XSS($_GET['student']) . '" AND exe_cours_id="' . $info_course['code'] . '" AND orig_lp_id = 0 AND orig_lp_item_id = 0 AND status <> "incomplete" AND status <> "left_incomplete" AND session_id = '.$session_id.' ORDER BY exe_date DESC LIMIT 1';
...
<div id="dialog" title="<?php echo sprintf(get_lang('CompareUsersOnLearnpath'),$learnpath['name']) ?>" >
...
<script type="text/javascript">
   $(document).ready(function() {
           $(".dialog").click(function(event) {
                             var LpId = $(this).attr('title');   
                             $("#dialog").load('charts/learnpath.ajax.php?width=900&height=500&cidReq=<?php echo $course_code_info; ?>&course=<?php echo $course_code_info ?>&lp_id='+LpId+'&user_id=<?php echo intval($_GET['student']) ?>');                            
           });
   });
</script>
