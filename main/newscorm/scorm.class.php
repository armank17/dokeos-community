 ...
 */
require_once dirname(__FILE__).'/scormItem.class.php';
require_once dirname(__FILE__).'/scormMetadata.class.php';
require_once dirname(__FILE__).'/scormOrganization.class.php';
require_once dirname(__FILE__).'/scormResource.class.php';
require_once dirname(__FILE__).'/learnpath.class.php';
		...
		foreach($this->organizations as $id => $dummy)
		{
	    	...
	    	if(Database::num_rows($res_max)>0){
	     	...
	     	$myname = str_replace("'",' ', $myname);
                ...
                $myname = str_replace($search_characters, $replace_characters, $myname);
                $myname = trim($myname);
			...
			foreach($list as $item){
				...
				if (!empty($_POST['index_document']) && api_get_setting('search_enabled') == 'true' && extension_loaded('xapian') ) {
	 ...
	 function set_proximity($proxy=''){
		if($this->debug>0){error_log('In scorm::set_proximity('.$proxy.') method',0);}
	 	...
	 	if($lp!=0){
	 		...
	 		$sql = "UPDATE $tbl_lp SET content_local = '".Database::escape_string($proxy)."' WHERE id = ".$lp;
	 ...
	 */
	 function set_maker($maker=''){
		if($this->debug>0){error_log('In scorm::set_maker method('.$maker.')',0);}
	 	...
	 	if($lp!=0){
	 		$tbl_lp = Database::get_course_table(TABLE_LP_MAIN);
	 		$sql = "UPDATE $tbl_lp SET content_maker = '".Database::escape_string($maker)."' WHERE id = ".$lp;
	 		$res = Database::query($sql);
	 		return $res;
	 	}else{
	 		return false;
