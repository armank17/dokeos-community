...
$debug = 0;
if ($debug>0)
    error_log('New LP -+- Entered lp_controller.php -+-', 0);

// name of the language file that needs to be included
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'export') { //only needed on export
...
// When import Scrom use popup in edit
if(!empty($_REQUEST['action']) && $_REQUEST['action'] !='view'){
    $htmlHeadXtra[] = '<script src="'.api_get_path(WEB_CODE_PATH).'application/author/assets/js/scriptController.js" type="text/javascript"></script>';
}
...
$htmlHeadXtra[] = '<link rel="stylesheet" href="' . api_get_path(WEB_CODE_PATH) . 'css/jquery.jscrollpane.css" />';
...
switch ($action) {
    case 'add_item':
        ...
        break;
    case 'add_lp':
        if(!$is_allowed_to_edit || api_get_setting('enable_author_tool') === 'false'){
            api_not_allowed(true);
        }
    ...
    case 'update_lp':
            ...
            if(!$lp_found){ error_log('New LP - No learnpath given for edit',0); require('lp_list.php'); }
                    ...
                    $lp_description = array();
                    if (isset($_REQUEST['lp_description'])) {
                        $lp_description['description'] = $_REQUEST['lp_description'];
                    } else {
                        $lp_description['description'] = '';
                    }
                    $lp_description['start_module_tool_type'] = $_REQUEST['start_module_tool_type'];
                    $lp_description = serialize($lp_description);
                    $_SESSION['oLP']->set_description(api_convert_encoding($lp_description, api_get_system_encoding(), $_SESSION['oLP']->encoding));
                   ...
                   //$_SESSION['oLP']->set_author($author_fixed);
                    ...
                    $debug_value = intval($_REQUEST['enable_debug']);
                    $_SESSION['oLP']->update_scorm_debug($debug_value);
                    if (api_get_setting('enable_certificate') === 'true') {
                        ...
                        if ($_REQUEST['certif_evaluation_type'] == 1) {
                            ...
                            $certificate_template = intval($_REQUEST['certificate_template']);
                            if ($certificate_template > 0) {
                                $_SESSION['oLP']->save_certificate_item($_REQUEST['certificate_template'], get_lang('Certificate'), '');
                            }
                            else {
                                $_SESSION['oLP']->delete_certificate_item();
                            }
                    }
    ...
    default:
        ...
        if (api_get_setting('enable_author_tool') === 'true') {
            if (api_is_allowed_to_edit()) {
                header('Location: '.api_get_path(WEB_CODE_PATH).'index.php?module=author&cmd=Authoring&func=index&'.api_get_cidreq());
                exit;
            }
            else {
                require('lp_list_course.php');
