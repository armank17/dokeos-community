						...
						foreach($msg_array[$key] as $subkey => $value){
							switch(strtolower($subkey)){
								...
								case 'score':
									...
									if(count($scores) != 3){
										$oItem->set_score(intval($value));
									}
									else { //aicc format Core.Score.Raw, Core.Score.Max, Core.Score.Min.
										$oItem->set_score(intval($scores[0]));
										$oItem->set_max_score(intval($scores[1]));	
									}
									break;
								case 'time':
									//error_log('Setting lesson_time to '.$value,0);
									$oItem->set_time($value);
									$oItem->force_scorm_time = true;
									break;
							}
						}
						break;
