...
class learnpath {
     ...
     */
    function learnpath($course, $lp_id, $user_id, $session_id = null) {
        //check params
        //check course code
        ...
        if (empty($lp_id)) {
            ...
            if (Database::num_rows($res) > 0) {
                ...
                $this->origin_tool = $row['origin_tool'];
        ...
        $sql = "SELECT * FROM $lp_table WHERE lp_id = '$lp_id' AND user_id = '$user_id' AND session_id = '$session_id' ORDER BY view_count DESC";
        if ($this->debug > 2) {
        ...
        $res = Database::query($sql, __FILE__, __LINE__);
        if (isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export') {
            // Layout template in first page
           require_once ('learnpathItem.class.php');
           $oItem = new learnpathItem(0, $user_id);
           if (is_object($oItem)) {
    ...
    /**
     * Delete certificate item in lp when it is disabled
     */
    function delete_certificate_item() {
        ...
        if (Database::num_rows($check) > 0) {
            Database::query("DELETE FROM $tbl_lp_item WHERE item_type='certificate' AND lp_id='".intval($this->lp_id)."'");
        }
    }
     ...
     */
    function get_db_progress($lp_id, $user_id, $mode = '%', $course_db = '', $sincere = false, $session_id = null) {
        //if($this->debug>0){error_log('New LP - In learnpath::get_db_progress()',0);}
        if (!isset($session_id)) {
            $session_id = api_get_session_id();
        }
        $table = Database :: get_course_table(TABLE_LP_VIEW, $course_db);
        $sql = "SELECT * FROM $table WHERE lp_id = $lp_id AND user_id = $user_id AND session_id = $session_id";
        $res = Database::query($sql, __FILE__, __LINE__);
        $view_id = 0;
        if (Database :: num_rows($res) > 0) {
            $row = Database :: fetch_array($res);
    ...
    function get_mediaplayer($autostart='true') {
        ...
        $sql = "SELECT lp.id, audio, lp_view.status  FROM " . $tbl_lp_item . " as lp inner join " . $tbl_lp_item_view . " as lp_view on lp.id = lp_view.lp_item_id " .
        ...
        if (!empty($row['audio'])) {
            ...
            $audioDirPath = api_get_path(SYS_COURSE_PATH).api_get_course_path().'/upload/learning_path/items/'.$row['id'].'/';
            if (!empty($audioName) && file_exists($audioDirPath.$audioName)) {
                $audioWebPath = api_get_path(WEB_COURSE_PATH).api_get_course_path().'/upload/learning_path/items/'.$row['id'].'/'.$audioName;
                // the mp3 player
                $visibility = !api_is_allowed_to_edit() && $autoplay?'hidden':'visible';
                
                $playerid = uniqid();
                $output .='<div id="playeraudio'.$playerid.'" class="thePlayer"><script type="text/javascript" src="http://my.dokeospro.com/main/inc/lib/ckeditor/plugins/audio/jwplayer/jwplayer.js"></script>
			<div id="playeraudio'.$playerid.'-parent2_wrapper" style="position: relative; width: 400px; height: 24px;"><object width="100%" height="100%" type="application/x-shockwave-flash" data="http://my.dokeospro.com/main/inc/lib/jwplayer/jwplayer.flash.swf" bgcolor="#000000" id="playeraudio'.$playerid.'-parent2" name="playeraudio'.$playerid.'-parent2" tabindex="0"><param name="allowfullscreen" value="true"><param name="allowscriptaccess" value="always"><param name="seamlesstabbing" value="true"><param name="wmode" value="transparent"></object><div id="playeraudio'.$playerid.'-parent2_jwpsrv" style="position: absolute; top: 0px; z-index: 10;"></div></div>
			<script type="text/javascript">
                        jwplayer("playeraudio'.$playerid.'-parent2").setup({file: "'.$audioWebPath.'",height: 24,autostart: '.($autoplay?"true":"false").',repeat: false,width: 400, primary: "flash"});</script></div>';
            }
        }
        return $output;
    }
     ...
     * @return	mixed	Ordered list of item IDs or false on error
     */
    function get_report_flat_ordered_items_list($lp, $code, $parent = 0) {
        //if($this->debug>0){error_log('New LP - In learnpath::get_flat_ordered_items_list('.$lp.','.$parent.')',0);}
        $list = array();
        if (empty($lp)) {
            return false;
        }
        $tbl_lp_item = Database :: get_course_table(TABLE_LP_ITEM, $code);
        $sql = "SELECT * FROM $tbl_lp_item WHERE lp_id = $lp AND parent_item_id = $parent ORDER BY display_order";
        $res = Database::query($sql, __FILE__, __LINE__);
        $certif_item = array();
        while ($row = Database :: fetch_array($res)) {
            ...
            $sublist = self::get_report_flat_ordered_items_list($lp, $code, $row['id']);
            $list[] = $row['id'];
            foreach ($sublist as $item) {
                $list[] = $item;
            }
    ...
    function get_view($attempt_num = 0) {
        ...
        if (Database :: num_rows($res) > 0) {
            ...
            $sql = "INSERT INTO $lp_view_table(lp_id,user_id,view_count,session_id)" .
                    "VALUES (" . $this->get_id() . "," . $this->get_user_id() . ",1,".api_get_session_id().")";
            $res = Database::query($sql, __FILE__, __LINE__);
            $id = Database :: insert_id();
            $this->lp_view_id = $id;
        }
    ...
    function restart() {
        ...
        $lp_view_table = Database :: get_course_table(TABLE_LP_VIEW);
        $sql = "INSERT INTO $lp_view_table (lp_id, user_id, view_count, session_id) " .
                "VALUES (" . $this->lp_id . "," . $this->get_user_id() . "," . ($this->attempt + 1) . ", ".api_get_session_id().")";
        if ($this->debug > 2) {
    ...
    function save_last() {
        ...
        if (isset($this->current)) {
            ...
            $sql = "UPDATE $table SET last_item = " . Database::escape_string($this->get_current_item_id()) . " " .
                    "WHERE lp_id = " . $this->get_id() . " AND user_id = " . $this->get_user_id()." AND session_id = ".api_get_session_id();
    ...
    function set_jslib($lib = '') {
        ...
        if ($lp != 0) {
            $tbl_lp = Database :: get_course_table(TABLE_LP_MAIN);
            $sql = "UPDATE $tbl_lp SET js_lib = '".Database::escape_string($lib)."' WHERE id = " . $lp;
            $res = Database::query($sql, __FILE__, __LINE__);
            return $res;
        } else {
            return false;
    ...
    /**
     * Sets the description of the LP (local/remote) (and save)
     * @author  Ricardo Garcia Rodriguez
     * @param	string	Optional string giving the new serialized description of this learnpath
     * @return  bool returns true if description is not empty
     */
    function set_description($description = '') {
        if ($this->debug > 0) {
            error_log('New LP - In learnpath::set_description()', 0);
        }
        ...
        $lp_table = Database :: get_course_table(TABLE_LP_MAIN);
        $lp_id = $this->get_id();
        $sql = "UPDATE $lp_table SET description = '" . $this->description . "' WHERE id = '$lp_id'";
        if ($this->debug > 2) {
            error_log('New LP - lp updated with new description : ' . $this->description, 0);
        }
        $res = Database::query($sql, __FILE__, __LINE__);
        return true;
    }
...
//set_interface
    ...
    function overview() {
        ...
        if (api_is_allowed_to_edit()) {
            ...
            $gradebook = Security :: remove_XSS($_GET['gradebook']);
            $author_lang_var = api_convert_encoding(get_lang('MyModule'), $charset, api_get_system_encoding());

            $return .= '<div class="actions">';
            $return .= '<a href="' . api_get_path(WEB_CODE_PATH) . 'newscorm/lp_controller.php?' . api_get_cidreq() . '&action=add_item&type=step&lp_id=' . $this->lp_id . '" title="' . $author_lang_var . '">' . Display::return_icon('pixel.gif', $author_lang_var, array('class' => 'toolactionplaceholdericon toolactionauthorcontent')) . ' ' . $author_lang_var . '</a>';
            $return .= '</div>';
        }
        ...
        $return .= '<div id="content_with_secondary_actions"><div class="rounded" style="width: 100%; padding: 1px;"><table  class="gallery" style="width: 100%;"><tr><td><div id="GalleryContainer">';
        ...
        for ($i = 0; $i < count($arrLP); $i++) {
            ...
            $div_style = 'style="background-image:url(' . $img_type . ');background-repeat:no-repeat; height:100px;background-position: center center;"';
...
<?php
    ...
    function build_action_menu() {
		...
		echo '<a href="' . api_get_self() . '?cidReq=' . Security::remove_XSS($_GET['cidReq']) . '&amp;action=add_item&amp;type=step&amp;lp_id=' . $_SESSION['oLP']->lp_id . '">' . Display::return_icon('pixel.gif', get_lang('MyModule'),array('class'=>'toolactionplaceholdericon toolactionauthorcontent')) . get_lang('MyModule') . '</a>';
		echo $return . '</div>';
        }
    }
    ...
    function create_document($_course) {
        ...
        if (!file_exists($filepath . $filename)) {
            if ($fp = @ fopen($filepath . $filename, 'w')) {
                ...
                $document_id = add_document($_course, $save_file_path, 'file', $file_size, $tmp_title);

                if ($document_id) {
                    api_item_property_update($_course, TOOL_DOCUMENT, $document_id, 'DocumentAddedFromLearnpath', api_get_user_id(), null, null, null, null, api_get_session_id());
    ...
    function edit_document($_course) {
        ...
        if (isset($_POST['path']) && !empty($_POST['path'])) {
            ...
            $res = Database::query($sql, __FILE__, __LINE__);
            $row = Database :: fetch_array($res);
            $content = stripslashes($_POST['document_content']);
            ...
            if (!empty($_POST['title'])) {				
				$sql = "UPDATE $table_doc SET title = '".Database::escape_string($_POST['title'])."' WHERE id = '".intval($_POST['path'])."'";
                Database::query($sql);
            }
        }
    }
        ...
        if (is_numeric($item_id)) {
            ...
            switch ($row['item_type']) {
                ...
                case TOOL_DOCUMENT :
                    ...
                    //	$return .= $this->display_manipulate($item_id, $row['item_type']);
                    $return .= "<a href='" . $preview_link . "'><img src='../img/navigation/doc_48.png' style='float:right;' alt='" . get_lang('preview') . "' title='" . get_lang('preview') . "'></a>".  str_repeat('<br />', 4);
    ...
    function display_resources() {
        ...
        /*}
            ...
            $return .= '<a href="'.api_get_path(WEB_CODE_PATH).'newscorm/lp_controller.php?'.api_get_cidreq().'&action=add_item&type=document&lp_id='.$_SESSION['oLP']->lp_id.'&tplid=0" class="' . $commonCssClasses . ' create_page_button">' . $add_page_lang_var . '</a>';
        }*/
    ...
    function display_document($id, $show_title = true, $iframe = true, $edit_link = false) {
        ...
        if ($iframe) {
            ...
            $texte = file_get_contents(api_get_path(SYS_COURSE_PATH) . $_course['path'] . '/document' . $row_doc['path']);
			
			$count = substr_count($texte,'<style type="text/css">');
			if($count > 1){
				$style_start = strrpos(trim($texte),'<style type="text/css">');			
				if($style_start !== false){
					$style_end = strrpos(trim($texte),'</body>');
					$sub = substr($texte,$style_start,$style_end);
					if($style_end > $style_start){
					$texte = str_replace(substr($texte,$style_start,$style_end), '', $texte);
					}
				}
			}
    ...
    function display_quiz_form($action = 'add', $id = 0, $extra_info = '') {
        ...
        $return .= '<div class="clear"></div><div class="sectioncomment">';
    ...
    function display_item_form($item_type, $title = '', $action = 'add', $id = 0, $extra_info = 'new') {
        ...
        if (($item_type == 'asset' || $item_type == 'sco') && ($extension == 'html' || $extension == 'htm')) {
            ...
            $renderer->setElementTemplate('<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{label}<br />{element}', 'document_content');
            $form->addElement('html_editor', 'document_content', '', null, array('ToolbarSet' => 'LearningPathDocuments', 'Width' => '100%', 'Height' => '900', 'FullPage' => true));
            $defaults["document_content"] = file_get_contents($item_path);
        }
...
//                            $js .= '<script language="javascript" src="' . api_get_path(WEB_LIBRARY_PATH) . 'fckeditor/editor/plugins/glossary/fck_glossary_manual.js"></script>';
      ...
      AND prop.visibility = 1 AND doc.id = '" . Database::escape_string(Security::remove_XSS($_GET['tplid'])) . "'";
                  ...
                  <tr>
                    ...
                    $valcontent = '<table cellspacing="2" cellpadding="10" border="0" style="width: 95%; height: 500px;">
                     ...
                     if (in_array($resource_info['extension'], array('flv', 'mp4', 'mov'))) {
                              $valcontent .= '
                                <script src="'.api_get_path(WEB_CODE_PATH).'inc/lib/fckeditor/editor/plugins/videoPlayer/jwplayer.js" type="text/javascript"></script>
            ...
            if (!$no_display_add) {
                if (($extra_info == 'new' || $extra_info['item_type'] == TOOL_DOCUMENT || $_GET['edit'] == 'true')) {
                    ...
                    if (!$no_display_edit_textarea) {
                        ...
                        $editor_config = array('ToolbarSet' => (api_is_allowed_to_edit() ? 'Documents' : 'DocumentsStudent'), 'Width' => '940', 'Height' => '600', 'FullPage' => true,
                        ...
                        $form->addElement('html', '</div>');
                        $renderer = $form->defaultRenderer();
                        $renderer->setElementTemplate('{label}{element}', 'document_content');

                        //$form->addElement('html', '<div');
                        //$form->addElement('html_editor', 'document_content', '', null, $editor_config);
                        $form->add_html_editor('document_content', '', false, true, $editor_config);
                        //$form->addElement('html', '</div>');
                        $defaults["document_content"] = $content;
                    }
                } elseif (is_numeric($extra_info)) {
        ...
        if ($action == 'add' && (isset($_GET['tplid']) && $_GET['tplid'] >= 0)) {
            $defaults["document_content"] = $valcontent;
    ...
    function get_exercises() {
        ...
        $sql_quiz = "
					SELECT *
					FROM " . $tbl_quiz . "
					WHERE active<>'-1' $condition_session AND quiz_type <> 2
					ORDER BY title ASC";
    ...
    function get_student_publication() {
        ...
        $return = '<a href="javascript:void(0)" id="work_tool_id" class="big_button four_buttons rounded grey_border assignement_button">' . $assignments_lang_var . '</a>';
    ...
    function scorm_export() {
        ...
        $zip_files = $zip_files_abs = $zip_files_dist = $nav_items = $nav_items_title = array();
        ...
        //format
        if (isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export') {
            $dokeos_intro = 'dokeos_intro';
            if (isset($_GET['navigation']) && $_GET['navigation'] == 'top') {
                $dokeos_intro = 'dokeos_introtop';
            }
        }
        $link_updates = array();
        foreach ($this->items as $index => $item) {
            if (!in_array($item->type, array(TOOL_QUIZ, TOOL_FORUM, TOOL_THREAD, TOOL_LINK, TOOL_STUDENTPUBLICATION))) {
                ...
                if ($item->type == 'sco' && !empty($item->db_id) && isset($_GET['export_mode']) && $_GET['export_mode'] == 'default_export') {
                ...
                } else if ($item->type == 'sco' && empty($item->db_id) && isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export') {
                    // Copy introduction file
                    $file_course_sys_path = api_get_path(SYS_COURSE_PATH).api_get_course_path().'/document/'.$dokeos_intro.'.html';
                    $source_intro_path = api_get_path(SYS_CODE_PATH).'newscorm/layout/'.$dokeos_intro.'.html';
                    @copy($source_intro_path,$file_course_sys_path);
                    // Copy script.js inside course
                    $current_script_path = api_get_path(SYS_CODE_PATH).'newscorm/layout/script.js';
                    $dest_script_path =  api_get_path(SYS_COURSE_PATH).api_get_course_path().'/document/script.js';
                    @copy($current_script_path,$dest_script_path);
                     
                     //Copy style.js
                    $current_style_path = api_get_path(SYS_CODE_PATH).'newscorm/layout/styles.css';
                    $current_style_content = file_get_contents($current_style_path);
                    //$current_style_content = str_replace('{IMAGE_PATH}', api_get_course_path().'/document/', $current_style_content);
                    $dest_style_path =  api_get_path(SYS_COURSE_PATH).api_get_course_path().'/document/styles.css';
                    if (file_exists($dest_style_path)) {
                        @unlink($dest_style_path);
                    }
                    file_put_contents($dest_style_path, $current_style_content);
                     
                    // Replace libraries in document header
                    $intro_string = file_get_contents($source_intro_path);
                    
                    // Add wrapper file
                    $api_wrapper_lib = api_get_path(WEB_CODE_PATH).'newscorm/js/api_wrapper.js';
                    $api_wrapper_string = file_get_contents($api_wrapper_lib);

                    $dest_wrapper_file = api_get_path(SYS_COURSE_PATH).api_get_course_path().'/document/api_wrapper.js';
                    file_put_contents($dest_wrapper_file, $api_wrapper_string);
                    $intro_string = str_replace("<head>", '<head><script type="text/javascript" src="'.api_get_path(WEB_COURSE_PATH). api_get_course_path().'/document/api_wrapper.js'.'" language="javascript" ></script>', $intro_string); 

                    //$intro_string = str_replace('{COURSE_CSS}', api_get_path(WEB_COURSE_PATH). api_get_course_path().'/document/css/template.css', $intro_string);
                    $intro_string = str_replace('{STYLE_CSS}', api_get_path(WEB_COURSE_PATH). api_get_course_path().'/document/styles.css', $intro_string);
                    $intro_string = str_replace('{JQUERY_LIB}', api_get_path(WEB_LIBRARY_PATH).'javascript/jquery-1.4.2.min.js', $intro_string);
                    $intro_string = str_replace('{JWPLAYER_LIB}', api_get_path(WEB_LIBRARY_PATH).'jwplayer/jwplayer.js', $intro_string);
                    $intro_string = str_replace('{SCRIPT_JS}', api_get_path(WEB_COURSE_PATH). api_get_course_path().'/document/script.js', $intro_string);
                    $intro_string = str_replace('{IMAGE_PATH}', api_get_path(WEB_COURSE_PATH). api_get_course_path().'/document/', $intro_string);                    
                    $intro_string = str_replace('{MODULE_NAME}', api_htmlentities($this->get_name(), ENT_QUOTES, $charset), $intro_string);
                    // Save document
                    file_put_contents($file_course_sys_path, $intro_string);
                    $inc_docs = $item->get_resources_from_source('sco', $file_course_sys_path);
                } else {
                ...
                if ($item->type == 'sco' && empty($item->db_id) && isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export') {
                    $my_file_path = 'document/'.$dokeos_intro.'.html';
                }
                ...
                // Course info
                $real_course_info = api_get_course_info(api_get_course_id());
                ...
                foreach ($inc_docs as $doc_info) {
                    ...
                    // clean urls
                    $orig_doc_info = $doc_info[0];
                    $doc_info[0] = str_replace('+', ' ', $doc_info[0]);
		    $doc_info[0] = str_replace($convert_enc, $convert_dec, $doc_info[0]);
                    ...
                    if ($doc_info[1] == 'remote') { //remote file. Save url as is
                        ...
                        $my_dep->setAttribute('xml:base', '');
                         ...
                         //error_log('file path start switch : '.$my_file_path);
                        switch ($doc_info[2]) {
                            case 'url':
                                ...
                                if (strstr($file_path, $main_path) !== false) {
                                    ...
                                    $link_updates[$my_file_path][] = array('orig' => $orig_doc_info, 'dest' => $file_path);
                                    $my_dep_file->setAttribute('href', $file_path);
                                    $my_dep->setAttribute('xml:base', '');
                            ...
                            case 'abs': //absolute path from DocumentRoot. Save file and leave path as is in the zip
                                ...
                                $cur_path = substr($current_course_path, -1) == '/' ? $current_course_path : $current_course_path . '/';
                                 // Check if folder is not in the document root folder
                                 $real_file_path = dirname($cur_path . $my_file_path);
                                 $known_dokeos_document_folders = array("animations","audio","chat_files","css",
                                     "images","mascot","mindmaps","photos","podcasts","screencasts","shared_folder",
                                     "themes","video");
                                 $real_course_path = $real_course_info['path'];
                                 $real_course_document_path = api_get_path(SYS_COURSE_PATH).$real_course_path.'/document/';
                                $real_path_info = explode($real_course_document_path, $real_file_path);
                                // Count paths after /document/...
                                $count_real_path_info = count($real_path_info);
                                $count_last_item_real_path_info = $count_real_path_info - 1;
                                $real_current_folder = $real_path_info[$count_last_item_real_path_info];
                                // If we have a path like /document/Isaac files/dokeos.png we need make a exception in the zip archive
                                $is_normal_document = true;
                                if (!in_array($real_current_folder, $known_dokeos_document_folders)) {
                                    $is_normal_document = false;
                                }
                                //check if the current document is in that path and check if the folder inside document is in the document folders list
                                if (strstr($file_path, $cur_path) !== false && in_array($real_current_folder, $known_dokeos_document_folders)) {
                                    ...
                                    $link_updates[$my_file_path][] = array('orig' => $orig_doc_info, 'dest' => $relative_path);
                                    $my_dep_file->setAttribute('href', $file_path);
                                    $my_dep->setAttribute('xml:base', '');
                                    ...
                                    //error_log('Reduced path: '.$file_path,0);
                                     //error_log('file path start switch 3 : '.$my_file_path);
                                    $zip_files_abs[] = $file_path;
                                    $link_updates[$my_file_path][] = array('orig' => $orig_doc_info, 'dest' => $file_path);
                ...
                $ext = substr(strrchr($my_file_path,'.'),1);                
                if ($ext == 'html' && !(strpos($my_file_path, '/'.$dokeos_intro.'.html') !== FALSE && isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export')) {
                    $nav_items[] = '"'.str_replace('document/', '', $my_file_path).'"';
                    $nav_items_title[] = '"'.api_htmlentities($item->get_title(), ENT_QUOTES, $charset).'"';
                }                
                if (($ext == 'html' && (strpos($my_resource, 'quiz_') !== FALSE)) && isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export' && !(strpos($my_file_path, '/'.$dokeos_intro.'.html') !== FALSE)) {
                    $path_to_document_in_course = api_get_path(SYS_COURSE_PATH).api_get_course_path().'/'.$my_file_path;
                    $file_course_document_content = file_get_contents($path_to_document_in_course);
                    $file_course_document_content = str_replace("<head>", '<head><script type="text/javascript" src="courses/'.api_get_course_path().'/document/api_wrapper.js'.'" language="javascript" ></script>', $file_course_document_content);
                    file_put_contents($path_to_document_in_course, $file_course_document_content);
                }                
                $zip_files[] = $my_file_path;
                $processing_files[] = $my_file_path;
                ...
                if ($item->type == TOOL_LINK) {
                    ...
                    $rs = Database::query($sql, __FILE__, __LINE__);
                    while ($link = Database :: fetch_array($rs)) {
                        $url = $link['url'];
                                ...
                                case 'rel': //path relative to the current document. Save xml:base as current document's directory and save file in zip as subdir.file_path
                                    if (substr($doc_info[0], 0, 2) == '..') { //relative path going up
                                        ...
                                        if (strstr($file_path, $main_path) !== false) {//the calculated real path is really inside the dokeos root path
                                            ...
                                            if ($info_file_path[0] == 'courses' || $info_file_path[0] == 'main') {//add character "/" in file path
                                                $file_path_dest = '/' . $file_path;
                                            }
                                            $zip_files_abs[] = $file_path;
                    ...
                    if (($ext == 'html' && (strpos($my_resource, 'quiz_') !== FALSE)) && isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export') {
                        $nav_items[] = '"../'.$my_file_path.'"';
                        $nav_items_title[] = '"'.api_htmlentities($item->get_title(), ENT_QUOTES, $charset).'"';
                    }
        ...
        foreach ($processing_files as $my_file_path) {
          if (!$is_normal_document) {
              $my_file_path_temp = str_replace(array("document/"), array(""), $my_file_path);
              $my_file_temp_info =  explode("/", $my_file_path_temp);
              array_pop($my_file_temp_info);
               $count_temp_folders = count($my_file_temp_info);
               if ($count_temp_folders == 1) {
                   $my_file_temp_path = $my_file_temp_info[0];
               } else {
                   $my_file_temp_path = implode("/",$my_file_temp_info);
               }
              $real_doc_path = $my_file_temp_path;
          } else {
             $real_doc_path = "";
          }
        $scorm_filey_list = array();
        foreach ($zip_files as $file_path) {
            ...
            $scorm_filey_list[] = $dest_file;
            @copy($sys_course_path . $_course['path'] . '/' . $file_path, $dest_file);
            //error_log(print_r($scorm_filey_list,true));
        ...
        //error_log('zip files : ');
        //error_log(print_r($zip_files_abs,true));
        foreach ($zip_files_abs as $file_path) {
            ...
            //error_log($main_path . $file_path);
            //error_log(__LINE__.'getting document from '.$main_path.$file_path.' removing '.api_get_path('SYS_COURSE_PATH').$_course['path'].'/',0);
            //error_log('dest file : '.$dest_file);
            //error_log('current path : '.$real_doc_path);
            $new_tem_file = substr($file_path, 0,7);
            if ($new_tem_file == 'courses') {
                $file_path = '/'.$file_path;
            }
            $dest_file = $archive_path . $temp_dir_short . '/document/' .$real_doc_path. $file_path;
            $this->create_path($dest_file);
            ...
            if (in_array($file_path, array_keys($link_updates))) {
                ...
                foreach ($link_updates[$file_path] as $old_new) {
                    ...
                    //$string = str_replace(array(api_get_path(WEB_PATH)), array("/"), $string);
                    $string = str_replace($old_new['orig'], $old_new['dest'], $string);
                    $string = str_replace(str_replace('https', 'http', $old_new['orig']), $old_new['dest'], $string);
                }
                file_put_contents($dest_file, $string);
...
EOD;
        ...
        // add items arrays to script.js
        if (file_exists($archive_path.$temp_dir_short.'/document/courses/'.api_get_course_path().'/document/script.js') && isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export') {
            $myscript_file = $archive_path.$temp_dir_short.'/document/courses/'.api_get_course_path().'/document/script.js';
            $myscript_string = file_get_contents($myscript_file);
            
            // array of pages
            $array_js  = '[';
            if (!empty($nav_items)) {
                $nav_items = array_unique($nav_items);
                $array_js .= implode(',', $nav_items);
            }
            $array_js .= ']';    
            
            // array of items titles
            $array_tit_js  = '[';
            if (!empty($nav_items_title)) {
                $nav_items_title = array_unique($nav_items_title);
                $array_tit_js .= implode(',', $nav_items_title);
            }
            $array_tit_js .= ']';
            
            $myscript_string = str_replace('{DOKEOS_ITEMS}', $array_js, $myscript_string);
            $myscript_string = str_replace('{DOKEOS_ITEMS_TITLES}', $array_tit_js, $myscript_string);
            file_put_contents($myscript_file, $myscript_string);
        }
