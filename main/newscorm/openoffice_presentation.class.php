 ...
 */
require_once dirname(__FILE__).'/openoffice_document.class.php';
    ...
    function make_lp($files=array()) {
		...
		foreach($files as $file){
			...
			if($this->take_slide_name === true)
			{
				...
				$slide_name = api_ucfirst($slide_name,api_get_setting('platform_charset'));
			}
			else
			{
...
'<html>
	...
	<body>
		<img src="'.$slide_src.'" class="oogie-image" />
	</body>
			...
			if ($document_id){
				...
				$previous = learnpath::add_item(0, $previous, 'document', $document_id, $slide_name, '');
                                createThumbnail(api_get_path(SYS_PATH).$slide_src, $this->lp_id, $previous);
