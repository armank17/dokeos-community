...
if (!empty($_GET['fs']) && strcmp($_GET['fs'], 'true') == 0) {
	$output .= '<table width="100%" align="center" class="responsive large-only table-striped">';
} else {
	$output .= '<table width="100%" class="responsive large-only table-striped">';
}
...
$extend_all = 0;
if ($origin == 'tracking') {
	$url_suffix = '&amp;cidReq='.Security::remove_XSS($_GET['course']).'&course=' . Security::remove_XSS($_GET['course']) .'&course_code=' . Security::remove_XSS($_GET['course']). '&student_id=' . $student_id . '&user_id='.$student_id.'&lp_id=' . Security::remove_XSS($_GET['lp_id']) . '&amp;origin=' . Security::remove_XSS($_GET['origin']).$from_link;
} else {
	$url_suffix = '';
}

if (!empty ($_GET['extend_all'])) {
	$extend_all_link = '<a id="statslink" href="' . api_get_self() . '?action=stats' . $url_suffix . '"><img src="../img/view_less_stats.png" alt="fold_view" border="0" title="'.api_convert_encoding(get_lang('HideAllAttempts'), $lp_charset, $dokeos_charset).'"></a>';
	$extend_all = 1;
} else {
	$extend_all_link = '<a id="statslink" href="' . api_get_self() . '?action=stats&extend_all=1' . $url_suffix . '"><img src="../img/add_22.png" alt="extend_view" border="0" title="'.api_convert_encoding(get_lang('ShowAllAttempts'), $lp_charset, $dokeos_charset).'"></a>';
}
...
$output .= "<table border='0' class='responsive large-only table-striped'><tr>\n" . '<td width="16">&nbsp;</td>' . '<td colspan="4" class="title"><div class="mystatusfirstrow">' . get_lang('ScormLessonTitle') . "</div></td>\n" . '<td colspan="2" class="title"><div class="mystatusfirstrow">' . api_convert_encoding(get_lang('ScormStatus'), $lp_charset, $dokeos_charset) . "</div></td>\n" . '<td colspan="2" class="title"><div class="mystatusfirstrow">' . api_convert_encoding(get_lang('ScormScore'), $lp_charset, $dokeos_charset) . "</div></td>\n" . '<td colspan="2" class="title"><div class="mystatusfirstrow">' . api_convert_encoding(get_lang('ScormTime'), $lp_charset, $dokeos_charset) . "</div></td><td class='title'><div class='mystatusfirstrow'>" . api_convert_encoding(get_lang('Actions'), $lp_charset, $dokeos_charset) . "</div></td></tr>\n";
}
else {
$output .= "<table border='0' class='responsive large-only table-striped'><tr>\n" . '<td width="16">' . $extend_all_link . '</td>' . '<td colspan="4" class="title"><div class="mystatusfirstrow">' . api_convert_encoding(get_lang('ScormLessonTitle'), $lp_charset, $dokeos_charset) . "</div></td>\n" . '<td colspan="2" class="title"><div class="mystatusfirstrow">' . api_convert_encoding(get_lang('ScormStatus'), $lp_charset, $dokeos_charset) . "</div></td>\n" . '<td colspan="2" class="title"><div class="mystatusfirstrow">' . api_convert_encoding(get_lang('ScormScore'), $lp_charset, $dokeos_charset) . "</div></td>\n" . '<td colspan="2" class="title"><div class="mystatusfirstrow">' . api_convert_encoding(get_lang('ScormTime'), $lp_charset, $dokeos_charset) . "</div></td><td class='title'><div class='mystatusfirstrow'>" . api_convert_encoding(get_lang('Actions'), $lp_charset, $dokeos_charset) . "</div></td></tr>\n";
}
...
"WHERE lp_id = $lp_id AND user_id = '" . $user_id . "' AND session_id = ".api_get_session_id();
$res = Database::query($sql, __FILE__, __LINE__);
$view = '';
$num = 0;
if (Database :: num_rows($res) > 0) {
...
if (isset($_GET['lp_id']) && isset($_GET['my_lp_id'])) {
	...
	if (Database::num_rows($res_path) > 0 ){
		if ($origin != 'tracking') {
			$sql_attempts = 'SELECT * FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . (int)$row_path['path'] . '" AND exe_user_id="' . (int)api_get_user_id() . '" AND orig_lp_id = "'.(int)$clean_lp_id.'" AND orig_lp_item_id = "'.(int)$clean_lp_item_id.'" AND exe_cours_id="' . $clean_course_code. '" AND status <> "incomplete" AND status <> "left_incomplete" ORDER BY exe_date';
		} else {
			$sql_attempts = 'SELECT * FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . (int)$row_path['path'] . '" AND exe_user_id="' . $student_id . '" AND orig_lp_id = "'.(int)$clean_lp_id.'" AND orig_lp_item_id = "'.(int)$clean_lp_item_id.'" AND exe_cours_id="' . $clean_course_code. '" AND status <> "incomplete" AND status <> "left_incomplete" ORDER BY exe_date';
		}
	}
...
if (is_array($list) && count($list) > 0){
	foreach ($list as $my_item_id) {
		...
		if (($extend_this || $extend_all) && $num > 0) {
	        ...
	        $extend_link='';
			if (!empty($inter_num)) {
				$extend_link = '<a id="statslink" href="' . api_get_self() . '?action=stats&fold_id=' . $my_item_id . $url_suffix . '"><img src="../img/visible.gif" alt="'.api_convert_encoding(get_lang('HideAttemptView'), $lp_charset, $dokeos_charset).'" title="'.api_convert_encoding(get_lang('HideAttemptView'), $lp_charset, $dokeos_charset).'"  border="0"></a>' . "\n";
			}
			...
			if ($row['item_type'] != 'dokeos_chapter') {
				...
				$title = api_convert_encoding(Security::remove_XSS($title),'UTF-8',api_get_system_encoding());
			...
			do {
				...
				if (!empty($export_csv) || !empty($export_print)) {				
		...
		elseif ($num > 0) {
			...
			if (($inter_num > 0 || $objec_num > 0) && !$extend_all) {
				if (!empty ($_GET['extend_attempt_id']) && $_GET['extend_attempt_id'] == $row['iv_id']) {
					//the extend button for this attempt has been clicked
					$extend_this_attempt = 1;
					$extend_attempt_link = '<a id="statslink" href="' . api_get_self() . '?action=stats&extend_id=' . $my_item_id . '&fold_attempt_id=' . $row['iv_id'] . $url_suffix . '"><img src="../img/visible.gif" alt="'.api_convert_encoding(get_lang('HideAttemptView'),$lp_charset,$charset).'" title="'.api_convert_encoding(get_lang('HideAttemptView'),$lp_charset,$charset).'" border="0"></a>' . "\n";
				} else { //same case if fold_attempt_id is set, so not implemented explicitly
					//the extend button for this attempt has not been clicked
			...
			$extend_link = '';
			if ($inter_num > 1) {
				$extend_link = '<a id="statslink" href="' . api_get_self() . '?action=stats&extend_id=' . $my_item_id . '&extend_attempt_id=' . $row['iv_id'] . $url_suffix . '"><img src="../img/invisible.gif" alt="'.api_convert_encoding(get_lang('ExtendAttemptView'),$lp_charset,$charset).'" title="'.api_convert_encoding(get_lang('ExtendAttemptView'),$lp_charset,$charset).'"  border="0"></a>';
			}
			if (($counter % 2) == 0) {
				$oddclass = "row_odd";
			} else {
			...
			if ($origin != 'tracking') {
				$sql_last_attempt = 'SELECT * FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . $row['path'] . '" AND exe_user_id="' . api_get_user_id() . '" AND orig_lp_id = "'.$lp_id.'" AND orig_lp_item_id = "'.$row['myid'].'" AND exe_cours_id="' . $course_code . '" AND status <> "incomplete" AND status <> "left_incomplete" ORDER BY exe_date DESC limit 1';
			} else {
				$sql_last_attempt = 'SELECT * FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . $row['path'] . '" AND exe_user_id="' . $student_id . '" AND orig_lp_id = "'.$lp_id.'" AND orig_lp_item_id = "'.$row['myid'].'" AND exe_cours_id="' . $course_code . '" AND status <> "incomplete" AND status <> "left_incomplete" ORDER BY exe_date DESC limit 1';
			}
			...
			if ($row['item_type'] != 'dokeos_chapter') {
				...
				if ($row['item_type'] == 'quiz') {
					...
					if ($origin != 'tracking' && $origin != 'tracking_course') {
						$my_url_suffix = '&course=' . Security::remove_XSS($_GET['course']).'&course_code=' . Security::remove_XSS($_GET['course']) . '&student_id=' . api_get_user_id() . '&user_id='.api_get_user_id().'&lp_id=' . Security::remove_XSS($row['mylpid']).'&sessionId='.$_GET['sessionId'];
						$sql_last_attempt = 'SELECT * FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . $row['path'] . '" AND exe_user_id="' . api_get_user_id() . '" AND orig_lp_id = "'.$lp_id.'" AND orig_lp_item_id = "'.$row['myid'].'" AND exe_cours_id="' . $course_code . '" AND status <> "incomplete" AND status <> "left_incomplete" ORDER BY exe_date DESC ';
					} else {
						$my_url_suffix = '&course=' . Security::remove_XSS($_GET['course']).'&course_code=' . Security::remove_XSS($_GET['course']). '&student_id=' . $student_id . '&user_id='.$student_id.'&lp_id=' . Security::remove_XSS($row['mylpid']).'&sessionId='.$_GET['sessionId'].'&amp;origin=' . Security::remove_XSS($_GET['origin'].$from_link);
						$sql_last_attempt = 'SELECT * FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . $row['path'] . '" AND exe_user_id="' . $student_id . '" AND orig_lp_id = "'.$lp_id.'" AND orig_lp_item_id = "'.$row['myid'].'" AND exe_cours_id="' . Database :: escape_string($_GET['course']) . '" AND status <> "incomplete" AND status <> "left_incomplete" ORDER BY exe_date DESC ';
					}

					$resultLastAttempt = Database::query($sql_last_attempt, __FILE__, __LINE__);
					$num = Database :: num_rows($resultLastAttempt);
					if ($num > 0 && empty($export_print) && empty($export_csv)) {
						if (isset($_GET['extend_attempt']) && $_GET['extend_attempt'] == 1 && (isset($_GET['lp_id']) && $_GET['lp_id'] == $my_lp_id) && (isset($_GET['my_lp_id']) && $_GET['my_lp_id'] == $my_id)  ) {
							$correct_test_link = '<a id="statslink" href="' . api_get_self() . '?action=stats' . $my_url_suffix . '&my_ext_lp_id='.$my_id.'"><img src="../img/view_less_stats.png" alt="fold_view" border="0" title="'.api_convert_encoding(get_lang('HideAllAttempts'),$lp_charset,$charset).'"></a>';
							$extend_attempt = 1;
						} else {
							$correct_test_link = '<a id="statslink" href="' . api_get_self() . '?action=stats&extend_attempt=1'.$my_url_suffix.'&my_lp_id='.$my_id.'"><img src="../img/add_22.png" alt="extend_view" border="0" title="'.api_convert_encoding(get_lang('ShowAllAttemptsByExercise'),$lp_charset,$charset).'"></a>';
						}
					} else {
						$correct_test_link = '-';
					}
				...
				if ( (isset($_GET['lp_id']) && $_GET['lp_id'] == $my_lp_id ) && (isset($_GET['my_lp_id']) && $_GET['my_lp_id'] == $my_id)) {
					$output .= "<tr class='$oddclass' >\n" . "<td>$extend_link</td>\n" . '<td colspan="4"><div class="mystatus">' .api_convert_encoding($title,'UTF-8',api_get_system_encoding()). '</div></td>' . "\n";
					...
					if (!empty($export_csv) || !empty($export_print)) {	
						...
						$title = api_convert_encoding($title,'UTF-8',api_get_system_encoding());
					}
								...
								$output .= '<tr class="'.$oddclass.'" ><td>&nbsp;</td><td>'.$extend_attempt_link.'</td><td colspan="3">' . api_convert_encoding(get_lang('Attempt'), $lp_charset, $dokeos_charset) . ' ' . $n . '</td>'
							 	...
							 	if ($origin != 'tracking') {
							 		if (!$is_allowed_to_edit && $result_disabled_ext_all) {
										...
										$output .= '<td><a id="module_quiz_result" href="../exercice/quiz_result.php?origin=student_progress&myid='.$my_orig_lp.'&my_lp_id='.$my_orig_lp_item.'&amp;id=' . $my_exe_id . '&quiz_id='.$my_exo_exe_id.'&cidReq=' . $course_code . '&course_code='.$course_code.'&student=' . $student_id .'&from=module'.$from_link. '&sessionId='.$_GET['sessionId'].'" target="_parent"><img src="' . api_get_path(WEB_IMG_PATH) . 'quiz.gif" alt="'.api_convert_encoding(get_lang('ShowAttempt'), $lp_charset, $dokeos_charset).'" title="'.api_convert_encoding(get_lang('ShowAttempt'), $lp_charset, $dokeos_charset).'"></a></td>';
									}
								} else {
									if (!$is_allowed_to_edit && $result_disabled_ext_all) {
										...
										$output .= '<td><a id="module_quiz_result" href="../exercice/quiz_result.php?origin=tracking_course&myid='.$my_orig_lp.'&my_lp_id='.$my_orig_lp_item.'&amp;id=' . $my_exe_id . '&quiz_id='.$my_exo_exe_id.'&cidReq=' . $course_code . '&course_code='.$course_code.'&user_id='.$student_id.'&student=' . $student_id . '&total_time='.$mytime.'&exe_id='.$my_exe_id.'&my_exe_exo_id='.$my_exo_exe_id.'&from=module'.$from_link. '&sessionId='.$_GET['sessionId'].' " target="_parent"><img src="' . api_get_path(WEB_IMG_PATH) . 'quiz.gif" alt="'.api_convert_encoding(get_lang('ShowAndQualifyAttempt'), $lp_charset, $dokeos_charset).'" title="'.api_convert_encoding(get_lang('ShowAndQualifyAttempt'), $lp_charset, $dokeos_charset).'"></a></td>';
									}
...
if (!empty($a_my_id)) {
	...
	if ($origin == 'tracking') {
        ...
        if (!empty($my_studen_id) && !empty($my_course_id)) {
            ...
            $total_score = Tracking::get_avg_student_score($my_studen_id, $my_course_id, array(intval($_GET['lp_id'])), null, false, false);            
        } else {
            $total_score = 0;
        } 
} else {
...
if (empty($extend_all)) {
	...
	$output .=  "<tr class='$oddclass'>\n" . "<td></td>\n" . '<td colspan="4"><div class="mystatus"><i>' . api_convert_encoding(get_lang('AccomplishedStepsTotal') , 'UTF-8', api_get_system_encoding()) . "</i></div></td>\n".'<td colspan="2"></td>' . "\n" . '<td colspan="2"><div class="mystatus" align="center">' . $final_score . "</div></td>\n" . '<td colspan="2"><div class="mystatus">' . $total_time . '</div></td><td></td>' . "\n" . "</tr>\n";
	}
}
