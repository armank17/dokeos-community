 ...
 var $audio; //the path to an audio file (stored in document/audio/)
    ...
    function learnpathItem($db_id, $user_id) {
        ...
        if ($db_id != 0) {
            ...
            $this->db_id = $id;
        } else if ($db_id == 0 && (isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export')) {
            $this->lp_id	 = 0;
            $this->max_score = 100;
            $this->min_score = 0;
            ...
            $this->prereq_string = 0;
            $this->max_time_allowed = 100;
            $this->save_on_close = true;
            $this->db_id = $id; 
        }
        // get search_did
    		...
    		case 'sco':
    			...
    			if(is_file($abs_path))
    			{
					...
					switch(strtolower($ext))
					{
						...
						case 'shtml':
						case 'css':
					 		$wanted_attributes = array('src','url','@import','href','value','fileUrl');
...
//	            '(@import([ \n\t\r]+)?("[^"]+"|\'[^\']+\'|[^ \n\t\r]+)))?/', -> takes a lot (like 100's of thousands of empty possibilities)
               ...
               // Add videos and mp3 files to scorm export
                $pattern = '/\s*fileUrl(.+?)\s/';
                preg_match_all($pattern, $attrString, $video_matches);
                if (is_array($video_matches) && !empty($video_matches)) {
                    //error_log(print_r($video_matches, true));
                    $all_video_matches = $video_matches[0];
                    $all_video_paths = array();
                    foreach ($all_video_matches as $videos) {
                      if (!in_array($videos, $all_video_paths)) {
                        $all_video_paths[] = $videos;
                        $video_path = trim(str_replace(array("fileUrl=","fileUrlHtml="),array(""," "),$videos));
                        $all_video_path[] = $video_path;
                        $video_attributes['fileUrl'] = $all_video_path;
                      }
                    }
                    // Add flash to scorm package
                    $attributes['src'][] = api_get_path(WEB_LIBRARY_PATH).'jwplayer/jwplayer.flash.swf';
                    // Add images to course document folder
                    if (isset($_GET['export_mode']) && $_GET['export_mode'] != 'default_export') {
                        $scan_dir_path = api_get_path(SYS_CODE_PATH).'newscorm/layout/images/';
                        $scan_layout_images = scandir($scan_dir_path);

                        foreach ($scan_layout_images as $image_item) {
                            if ($image_item == '.' || $image_item == '..') {
                                continue;
                            } else {
                               if (!is_dir(api_get_path(SYS_COURSE_PATH).api_get_course_path(). '/document/module_images')) {
                                 mkdir(api_get_path(SYS_COURSE_PATH).api_get_course_path(). '/document/module_images');
                               }
                               @copy($scan_dir_path.$image_item,api_get_path(SYS_COURSE_PATH).api_get_course_path(). '/document/module_images/'.$image_item);
                               $attributes['src'][] = api_get_path(WEB_COURSE_PATH).api_get_course_path(). '/document/module_images/'.$image_item; 
                            }
                        }
                    }
                 }
                //error_log(print_r($regs[1],true));                     
		} catch (Exception $e) {
	    ...
	    if ($res) {
            ...
            if (is_array($video_attributes)) {
              $attributes = array_merge_recursive($attributes,$video_attributes);
            }
            //error_log(print_r($attributes,true));
	    return $attributes;
	}
		...
		$row_verified=Database::fetch_array($rs_verified);
   		...
   		if (in_array($sql_verified['status'],$my_case_completed)) {
   			...
   			$save=true;
                ...
                // This code line MUST be fixed before release 3.0 
   		if (($save===false && $this->type == 'sco')) // This depend of multiple attempt : ||(($this->type == 'sco') && ($credit == 'no-credit' OR $mode == 'review' OR $mode == 'browse'))
   		{
