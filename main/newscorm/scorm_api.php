...
olms.lms_first_item = '<?php echo $oLP->get_first_item_id(); ?>';
olms.lms_last_item = '<?php echo $oLP->get_last(); ?>';
olms.lms_current_item = olms.lms_item_id;
...
*/
	...
	switch(next_item){
        ...
        olms.lms_current_item = next_item;
 ...
 // The cidReq is now very need
	...
	<?php } else { ?>
                        ...
                        //location.reload();
                        if (olms.lms_lp_type == 1) {
                            location.href = "<?php echo api_get_path(WEB_CODE_PATH); ?>newscorm/lp_controller.php?cidReq=<?php echo $oLP->cidReq; ?>&action=view&lp_id="+olms.lms_lp_id+"&switch=item";
                        }
                    }
	<?php } ?>
   ...
   // (5) switch audio recorder
     ...
     // (6)
        $.ajax({
		type: "GET",
		url: "lp_ajax_current_item_title.php?lp_item_id="+next_item,
		success: function(tmp_data) {
...
/**
* Disable next or prev button in top navigation
*
*/
function hideTopButtonNavigation() {
    var isFirstItem = olms.lms_first_item == olms.lms_current_item;
    var isLastItem = olms.lms_last_item == olms.lms_current_item;
    
    if (isFirstItem) {  
        $(".prev_button").hide();
        $("#coursepreviousbutton_na").show();
    } else {
        $(".prev_button").show();
        $("#coursepreviousbutton_na").hide();
    }
    
    if (isLastItem) {       
        $(".next_button").hide();
        $("#coursenextbutton_na").show();
    } else {
        $(".next_button").show();
        $("#coursenextbutton_na").hide();
    }
...
function attach_glossary_into_scorm() {
    ...
    $.ajax({
        ...
        for (i=0;i < data_terms.length;i++) {
            ...
            $frame_content.highlight(real_term, false, real_code);
        }
        //mouse over event
          ...
          $("iframe").contents().find('body').find("div#main_div_show_id").center();
            ...
            $("iframe").contents().find('body').find("div#glossary-close").click(function(){$("iframe").contents().find('body').find("div#main_div_show_id").remove();});
                ...
                $.ajax({
                    ...
                    success: function(datos) {
                        var btn_close = "<div class='btn-glossary-close' style='text-align:right;background-color: #E8E8E8;height: 20px;margin:-5px -10px;padding:5px;margin-bottom:5px;padding-top:0px;overflow:hidden;'><table width='100%'><tr><td align='left' valign='top' style='font-weight:bold;'>"+text_box+"</td><td align='right' valign='top' width='50px;'><a href='javascript:void(0)' onclick='window.parent.closeGlossaryPopup(\"main_div_show_id\");'><img src='<?php echo api_get_path(WEB_IMG_PATH).'close.gif'; ?>' /></a></td></tr></table></div>";
