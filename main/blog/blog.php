...
$task_id = intval(isset($_GET['task_id']));
...
if (!empty($_POST['new_task_submit']) && $safe_task_name != '') {
...
if (!empty($return_message)) {
    if ($return_message['type'] == 'confirmation') {
        Display::display_confirmation_message($return_message['message'], false,true);
    }
    if ($return_message['type'] == 'error') {
        Display::display_error_message($return_message['message'], false, true);
    }
}
...
if (!empty($current_page) && !isset($_GET['month'])) { //&& !isset($_GET['task_id']) && isset($_GET['filter']
    echo '<div class="quiz_content_actions blog_margintop" style="overflow:hidden;width: 700px;">';
}


if ($error)
    Display :: display_error_message($message, false, true);
    ...
    case 'new_post' :
        if (api_is_allowed('BLOG_' . $blog_id, 'article_add', $user_task ? $task_id : 0)) {
            ...
            if (!$_POST OR (!empty($_POST) AND empty($_POST['post_title']))) {
                ...
                if ($_POST) {
                    Display::display_error_message(get_lang('FormHasErrorsPleaseComplete'), false, true);
                }
    ...
    $(document).ready(function() {
        if ($("#add_post").length > 0) {
            $("#add_post").validate({
                ...
                messages: {
                    task_name: {
                        required: "<img src='<?php echo api_get_path(WEB_IMG_PATH); ?>exclamation.png' title='<?php echo get_lang('Required'); ?>' />"
                    }
                }
            });
        }
