	...
	*/
	...
	if (!empty($_POST['new_blog_submit']) AND !empty($_POST['blog_name'])) {
		if (strlen(trim($_POST['blog_name']))>0)  {
			...
			Display::display_confirmation_message(get_lang('BlogStored'), false, true);
		}
	...
	if (!empty($_POST['edit_blog_submit']) AND !empty($_POST['blog_name'])) {
		if (strlen(trim($_POST['blog_name']))>0) {
			...
			Display::display_confirmation_message(get_lang('BlogEdited'), false, true);
		}
	}
	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete') {
		...
		Display::display_confirmation_message(get_lang('BlogDeleted'), false, true);
	}
