...
if (isset ($_GET['exportpdf']))	{
    ...
    $export_pdf_form = new DataForm(DataForm::TYPE_EXPORT_PDF, 'export_pdf_form', null, api_get_self() . '?exportpdf=&amp;offset=' . Security::remove_XSS($_GET['offset']) . '&amp;selectcat=' . Security::remove_XSS($_GET['selectcat']), '_blank', $attributes);
    if (!$export_pdf_form->validate()) {
        Display::display_tool_header(get_lang('ExportPDF'));
    }
    if ($export_pdf_form->validate()) {
...
$offset = (isset($_GET['offset']) ? Security::remove_XSS($_GET['offset']) : '0');
...
function get_printable_data($users,$alleval, $alllinks) {
    ...
    $count = (($offset + 10) > $datagen->get_total_items_count()) ?
                ($datagen->get_total_items_count()-$offset) : 
                10;
