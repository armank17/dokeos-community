...
class EvalForm extends FormValidator
{
	 ...
	 * @param action
	 */
	function EvalForm($form_type, $evaluation_object, $result_object, $form_name, $method= 'post', $action= null, $extra1 = null, $extra2 = null, $target = '', $attributes = null)
	{
...
//		));
	...
	protected function build_result_add_form() {
		...
		$renderer = $this->defaultRenderer();                                
                if ($this->evaluation_object->get_type() == 'presence') {
                    $elementTemplateTwoLabel = '<div class="row">
			<div class="label">
			<!-- BEGIN required --><span class="form_required">*</span> <!-- END required -->{label}
			</div>
		...
		foreach ($tblusers as $user) {
                        ...
                        if ($this->evaluation_object->get_type() == 'presence') {
                            $this->addElement('checkbox', 'score['.$user[0].']', $this->build_stud_label($user[0], $user[1], $user[2]));
                        }
                        else {
                            $this->add_textfield('score['.$user[0].']', $this->build_stud_label($user[0], $user[1], $user[2]), false, array ('size' => 4, 'maxlength' => 4));
                        }
	...
	protected function build_result_edit_form() {
		...
		$this->addElement('static', null, null, api_get_person_name($userinfo['lastName'], $userinfo['firstName']));
                ...
                if ($this->evaluation_object->get_type() == 'presence') {
                    $this->addElement('checkbox', 'score', $this->build_stud_label($user[0], $user[1], $user[2]));
                }
                else {
	...
	protected function build_add_form() {
		...
		//	$this->addElement('checkbox', 'adduser', get_lang('AddUserToEval'));
			$this->addElement('hidden','adduser');
		} else {
		//	$this->addElement('checkbox', 'addresult', get_lang('AddResult'));
			$this->addElement('hidden','addresult');
		}
	...
	/*	if ($edit) {
		...
		$this->addElement('hidden','description');
		$this->addElement('hidden','visible',1);
		//$this->addElement('hidden','max',0);
