	...
	function display_header_result($evalobj, $selectcat, $shownavbar) {
		...
		if ($shownavbar == '1' && $status==1) {
			...
			if ($evalobj->get_course_code() == null) {
				$header .= '<a href="gradebook_add_user.php?selecteval=' . $evalobj->get_id() . '&'.  api_get_cidreq().'"><img src="../img/add_user_big.gif" alt="' . get_lang('AddStudent') . '" align="middle" /> ' . get_lang('AddStudent') . '</a>';
			}
...
//								'<img src="../tick.png" alt="' . get_lang('AddResult') . '" align="middle"/> ' . get_lang('AddResult') .
...
//						'<img src="../img/print32.png" alt="' . get_lang('Print') . '" /> ' . get_lang('Print') .
	...
	*/
	function display_header_reduce_flatview($catobj, $showeval, $showlink,$simple_search_form) {
		...
		$header .= '<a class="quiz_export_link" href="#" onclick="javascript: document.form1b.submit();">'.Display::return_icon('pixel.gif', get_lang('ExportAsXLS'),array('class'=>'toolactionplaceholdericon toolactionexportcourse')).' '.get_lang('ExportAsXLS').'</a>';
//		$header .= '<td style="vertical-align: top;"><a href="' . api_get_self() . '?exportpdf=&amp;offset='.Security::remove_XSS($_GET['offset']).'&amp;search=' . Security::remove_XSS($_GET['search']).'&amp;selectcat=' . $catobj->get_id() . '"><img src=../img/file_pdf.gif alt=' . get_lang('ExportPDF') . '/> ' . get_lang('ExportPDF') . '</a>';
		...
		$header .= '<a href="' . api_get_self() . '?exportpdf=&amp;selectcat=' . $catobj->get_id() . '" >'.Display::return_icon('pixel.gif', get_lang('ExportToPDF'),array('class'=>'toolactionplaceholdericon toolaction32x32file_pdf')).' ' . get_lang('ExportToPDF') . '</a>';
                $header .= '<a href="' . api_get_path(WEB_CODE_PATH) . 'exercice/exercice.php?'.api_get_cidreq().'">'.Display::return_icon('pixel.gif', get_lang('Quiz'), array('class' => 'toolactionplaceholdericon toolactionquiz')).get_lang("Quiz").'</a> ';
		//exportpdf
		//<div class="clear">
		$header .= '</div>';
		if (!$catobj->get_id() == '0') {
		...
		if (($status_user==1 && $is_course_admin && !isset ($_GET['search'])) || api_is_platform_admin()) {
			if ($selectcat == '0') {
                ...
                if ($show_add_qualification === true && $message_resource===false) {
    				//$header .= '<a href="gradebook_add_cat.php?'.api_get_cidreq().'&amp;selectcat=' . $catobj->get_id() . '" ><img src="../img/folder_new.gif" alt="' . get_lang('NewSubCategory') . '" align="middle" /> ' . get_lang('NewSubCategory') . '</a></td>';
                }
               $my_category=$catobj->shows_all_information_an_category($catobj->get_id());
                ...
                if ($show_add_link === true && $message_resource==false) {
                    ...
                    if ($cats[0]->get_course_code() != null && $message_resource===false) {
                        //$header .= '<td><a href="gradebook_add_link.php?'.api_get_cidreq().'&amp;selectcat=' . $catobj->get_id() . '"><img src="../img/link.gif" alt="' . get_lang('MakeLink') . '" align="middle" /> ' . get_lang('MakeLink') . '</a>';
			...
			$header_actions .= '<b>'.get_lang('SearchResults').' :</b>';
                ...
                $header_actions .= '<a href="' . api_get_path(WEB_CODE_PATH) . 'exercice/exercice.php?'.api_get_cidreq().'">'.Display::return_icon('pixel.gif', get_lang('Quiz'), array('class' => 'toolactionplaceholdericon toolactionquiz')).get_lang("Quiz").'</a> ';
		$header_actions .= '</div>';
