...
if (isset($_GET['forum']) && $_GET['forum'] != '') {
    $return_back = '<a href="' . api_get_path(WEB_CODE_PATH) . '/forum/viewforum.php?'.api_get_cidreq() . '&forum=' . $_GET['forum'] . '">'.Display::return_icon('pixel.gif',get_lang('BackUser'),array('class'=>'toolactionplaceholdericon toolactionback')).get_lang('Back').'</a>';
} else {
    if(isset($_GET['origin']) AND $_GET['origin']=='forum' ){
        $return_back = '<a href="../forum/index.php?'.  api_get_cidreq().'">'.Display::return_icon('pixel.gif',get_lang('BackUser'),array('class'=>'toolactionplaceholdericon toolactionback')).get_lang('Back').'</a>';
    }
    else if(isset($_GET['origin']) AND $_GET['origin']=='wiki' ){
        $return_back = '<a href="../wiki/index.php?'.api_get_cidreq().'&action=mactiveusers&group_id=">'.Display::return_icon('pixel.gif',get_lang('BackUser'),array('class'=>'toolactionplaceholdericon toolactionback')).get_lang('Back').'</a>';
    }
    else{
        $return_back = '<a href="../user/user.php?'.  api_get_cidreq().'">'.Display::return_icon('pixel.gif',get_lang('BackUser'),array('class'=>'toolactionplaceholdericon toolactionback')).get_lang('Back').'</a>';
    }
...
if (api_is_allowed_to_edit()) {
		...
		echo '<a href="userInfo.php?'.api_get_cidreq().'&origin='.$origin.'&uInfo='.$userIdViewed.'">'.Display::return_icon('pixel.gif',get_lang('ViewUser'),array('class'=>'toolactionplaceholdericon toolactionuser')).get_lang('ViewUser').'</a>';
	}
}
echo '</div>';
	...
	if ($mainUserInfo)
	{
		...
		echo	"<table width=\"80%\" border=\"0\">",
				...
				//DISPLAY TABLE CONTENT
                                ...
                                echo "<td>".$status[$mainUserInfo['status']]."</td>\n";
				// deprecated feature
