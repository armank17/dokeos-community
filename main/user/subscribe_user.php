        ...
        for ($j = 0; $j < $counter; $j++) {
            if ($user_id_temp[$j] == $_GET['user_id']) {
                if ($result_simple_sub)	{
                    Display::display_confirmation_message($_SESSION['session_user_name'][$j] . ' ' . get_lang('langAddedToCourse'), false, true);
                } else {
                    Display::display_confirmation_message($_SESSION['session_user_name'][$j] . ' ' . get_lang('langNotAddedToCourse'), false, true);
                }
            }
        }
    ...
    switch ($_POST['action']) {
        case 'subscribe' :
            if (is_array($_POST['user'])) {
                $_SESSION["display_confirmation_message"] = get_lang('TheUsersHaveBeenSubscribedToCourse');
                foreach ($_POST['user'] as $user_id) {
                    ...
                    if(isset($_REQUEST['type']) && $_REQUEST['type'] == 'teacher') {
                        ...
                        //                        
                        subscrib_user_to_payment($user_id);
                        //
            ...
            if (!empty($list_register_user)) {
                if ($$is_suscribe_counter == 1) {
                    ...
                    Display::display_confirmation_message($register_user_message, false, true);
                } else {
            ...
            if (!empty($list_not_register_user)) {
                ...
                Display::display_confirmation_message($not_register_user_message, false, true);
            }
            break;
    }
}
 ...
 * 
 */
function subscrib_user_to_payment($user_id){
    $table_payment_course_rel_user = Database::get_main_table(TABLE_MAIN_PAYMENT_COURSE_REL_USER);
    $query = "SELECT id FROM $table_payment_course_rel_user WHERE user_id=".$user_id;
    $res = Database::query($query);
    $resp = Database::fetch_array($res);

    $query2 = "DELETE FROM $table_payment_course_rel_user WHERE id=".$resp[0];
...
function get_user_data($from, $number_of_items, $column, $direction) {
	...
	if (isset($_REQUEST['type']) && $_REQUEST['type']=='teacher') {
		...
		if (!empty($_SESSION["id_session"])) {
			$sql = "SELECT
					...
					hr_dept_id AS col7
					FROM $user_table u
					LEFT JOIN $Roletbl_session_rel_course_user cu on u.user_id = cu.id_user and course_code='".$_SESSION['_course']['id']."'";
	...
	if (isset ($_REQUEST['keyword'])) {
		$keyword = Database::escape_string($_REQUEST['keyword']);
		$sql .= " AND (firstname LIKE '%".$keyword."%' OR lastname LIKE '%".$keyword."%'   OR email LIKE '%".$keyword."%'  OR username LIKE '%".$keyword."%'  OR official_code LIKE '%".$keyword."%') AND u.lastname<>'Anonymous'";
	...
	while ($user = Database::fetch_row($res)) {
		...
		if ($image_array['file']!='') {                   
                       ...
                       //var_dump($id_image);                        
			$photo = '<center><a id="" class="" onClick="runDialog('."'".$id_image."'".')" class="" href="#" title="'.get_lang('Info').'"  >
						<img id="" src="'.$user_profile['file'].'" style="width:22px" alt="'.api_get_person_name($o_course_user['firstname'], $o_course_user['lastname']).'"  title="'.api_get_person_name($o_course_user['firstname'], $o_course_user['lastname']).'" />
					  </a></center>';             
                       //var_dump($image_array['file']);                       
                      
                       echo  '<div id="'.$id_image.'" style="display: none;text-align:center;" title="'.get_lang('Photo').'">
                              <img src="'.$image_array['dir'].$image_array['file'].'">
                        </div>';
			...
			$photo = '<center>'.Display::return_icon('pixel.gif','',array('class' => 'actionplaceholdericon actionunknown')).'</center>';
                ...
                $user_rel_course = UserManager::get_user_in_course($user[0], $_SESSION['_course']['id']);
		...
		$users[] = array($user[1], $user[0], $user[2], $user[3], $user_rel_course, $session_name, $status, UserManager::get_user_manager_name($user[7]));
...
$actions .= '<a href="user.php?'.api_get_cidreq().'">'.Display::return_icon('pixel.gif', get_lang('Back'), array('class' => 'toolactionplaceholdericon toolactionback')).get_lang('Back').'</a>';
if ($_GET['keyword'] AND !empty($_GET['keyword']))
{
	$actions .= '<a href="subscribe_user.php?'.  api_get_cidreq().'&amp;type='.Security::remove_XSS($_GET['type']).'">'.Display::return_icon('pixel.gif',get_lang('ClearSearchResults'),array('class' => 'actionplaceholdericon actionclean')).get_lang('ClearSearchResults').'</a>';
}
if ($_GET['subscribe_user_filter_value'] AND !empty($_GET['subscribe_user_filter_value']))
{
	$actions .= '<a href="subscribe_user.php?'.  api_get_cidreq().'&amp;type='.Security::remove_XSS($_GET['type']).'">'.Display::return_icon('pixel.gif',get_lang('ClearSearchResults'),array('class' => 'actionplaceholdericon actionclean')).get_lang('ClearFilterResults').'</a>';
}
if (api_get_setting('ProfilingFilterAddingUsers') == 'true') {
	display_extra_profile_fields_filter();
}

$form = new FormValidator('search_user', 'GET',api_get_self().'?'. api_get_cidreq().'&amp;type='.  Security::remove_XSS($_REQUEST['type']),'',null,false);
$renderer = & $form->defaultRenderer();
$renderer->setElementTemplate('<span>{element}</span> ');

$form->addElement('static','additionalactions',null, '<div style="width:520px;float:left;">'.$actions.'</div>');
$form->addElement('static', 'div_float_right', null, '<div style="width:400px;float:left;">');
...
$table->set_header($col ++, get_lang('Course'));
$table->set_header($col ++, get_lang('Session'));
