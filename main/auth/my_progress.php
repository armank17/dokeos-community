...
$courses = CourseManager::get_courses_list_by_user_id($_user['user_id'], true);
// Actions
//echo '<div class="actions" >';
////.Display::return_icon('pixel.gif',get_lang('MyProgress'),array('class' => 'toolactionplaceholdericon toolactionstatistics')) . ' ' . get_lang('MyProgress') .
//echo '</div>';
$now = date('Y-m-d');
...
<table class="data_table" width="100%">
    ...
    <tr>
        ...
        <th><?php
        echo get_lang('Score');
        Display :: display_icon('info3.png', get_lang('ScormAndLPTestTotalAverage'), array ('align' => 'middle', 'hspace' => '3'));
        ?></th>
...
?>
    <tr class='<?php echo $i?'row_odd':'row_even'; ?>'>
        ...
        <td align='center'>
            <?php echo (is_numeric($percentage_score)?$progress:0).'%'; ?>
        </td>
        <td align='center'>
            <?php
            if (is_numeric($percentage_score)) {
                if ($percentage_score > 100) { $percentage_score = 100; }
                if ($percentage_score < 0) { $percentage_score = 0; }
...
if (isset($_GET['course'])) {
    ...
    $sessions = SessionManager::get_sessions_by_course($course);
    ...
    if (!empty($sessions)) {
        foreach ($sessions as $session_id) {
            ...
            $tableTitle = $course_info['title'].' | '.get_lang('Coach').' : '.$course_info['tutor_name'].((!empty($session_info['name']))?' | '.get_lang('Session').' : '.$session_info['name']:'');
?>
<table class="data_table" width="100%">
    <tr class="tableName">
        <td colspan="4">
    ...
    <?php
            $sql_learnpath = "SELECT lp.name,lp.id FROM ".$course_info['db_name'].".".$tbl_course_lp." AS lp WHERE session_id=".$session_id;
            ...
            if (Database::num_rows($result_learnpath) > 0) {
                while($learnpath = Database::fetch_array($result_learnpath)) {
                    ...
                    $lp_score = Tracking :: get_avg_student_score($_user['user_id'], $course_info['code'], array($learnpath['id']));
                    if ($lp_score > 100) { $lp_score = 100; }
                    if ($lp_score < 0) { $lp_score = 0; }
                    echo "	</td>
                        <td align='center'>".(!empty($lp_score)?round($lp_score,2).'%':'0%')."</td>
    ...
    <tr>
        ...
        <th class="head" style="color:#000" colspan="2"><?php echo get_lang('Details'); ?></th>
    </tr>
    ...
    <?php
            ...
            if (Database::result($result_visibility_tests, 0, 'visibility') == 1) {
                $sql_exercices = "	SELECT quiz.title,id, results_disabled
                                    ...
                                    WHERE active='1' AND session_id = $session_id";
                ...
                if (Database::num_rows($result_exercices) > 0) {
                    ...
                    echo '<tr><td colspan="5">'.get_lang('NoEx').'</td></tr>';
                }
            } else {
                echo '<tr><td colspan="5">'.get_lang('NoEx').'</td></tr>';
            }
