...
if (Database::num_rows($result) > 1)
{
	...
	else
	{
		...
		echo '<div class="pull-bottom"><button type="submit" name="Submit" class="next">'.get_lang('Ok').'</button></div>';
		echo '</form>';
  // Close content
  echo '</div>';
...
if (isset($_POST['finish_survey']))
{
	...
	echo ' 
             <div id="survey_title" class="">'. get_lang('SurveyFinished') .'</div> 
             <div style="padding:10px">'.$survey_data['survey_thanks'].'</div>
            </div>
            ';
...
$sql = "SELECT * FROM $table_survey_question WHERE survey_id = '".Database::escape_string($survey_invitation['survey_id'])."' AND	type <> '".Database::escape_string('pagebreak')."' ORDER BY sort ASC";
$result = Database::query($sql, __FILE__, __LINE__);
...
if(is_array($questions) && count($questions)>0)
{
        ...
        {	                       
                ...
                echo '<style>.survey_title_cus{display:none}.quiz_content_actions{margin-top:0px;}#survey_title{border-top:1px solid #c4c4c4; margin-top:-1px}</style>';
                }else{   
                echo '<div class="quiz_content_actions" style="">';
                echo '<style>.survey_title_cus{display:none}.quiz_content_actions{margin-top:0px;}#survey_title{border-top:1px solid #c4c4c4; margin-top:-1px}</style>';
                }
