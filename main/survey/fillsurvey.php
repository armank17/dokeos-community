...
{
	Display :: display_error_message(get_lang('SurveyParametersMissingUseCopyPaste'), false, true);
...
{
	Display :: display_error_message(get_lang('WrongInvitationCode'), false, true);
	Display :: display_footer();
	exit;
}
...
{
	Display :: display_error_message(get_lang('YouAlreadyFilledThisSurvey'), false, true);
	Display :: display_footer();
	exit();
}
		...
		{				
			...
			if (strstr($key, 'question'))
			{
				...
				else
				{
					...
					{
                                                $sqlOpt = "SELECT * FROM $table_survey_question_option WHERE survey_id='".$survey_invitation['survey_code']."'AND option_text= ".$value;

                                                $resultOpt = Database::query($sqlOpt, __FILE__, __LINE__);
                                                $rowOpt = Database::fetch_array($resultOpt,'ASSOC');
                                                $value = $rowOpt['question_option_id'];
                                                $option_value = $rowOpt['option_text'];
...
//    echo '<div id="survey_title" class="survey_content"><strong>'.get_lang('SurveyFinished').'</strong> <br />'.$survey_data['survey_thanks'].'<br/><br/> </div>
    echo '<div class="pull-bottom"><button class="cancel" type="button" name="survey_send" id="survey_send" >'.get_lang('CourseHome').'</button> </div>   
';
    ...
    if ($origin == 'learnpath') {
        ...
        $(document).ready(function(){
        $("button#survey_send").click(function(){
        parent.location = "../../courses/'.api_get_course_id().'/index.php?id_session='.$session.'";
        });
        });
        </script>';
    } else {
...
if (isset($_GET['show']))
	...
	echo '<form id="question" name="question" method="post" action="' . api_get_self() .'?'.$add_parameters. 'course=' . $g_c . '&invitationcode=' . $g_ic . '&show=' . $show . '&'.  api_get_cidreq() .$origin_string. $param_scenario.'&count='.$brecount.'">';
...
echo '<div style="text-align: right; padding-left: 10px; margin-right: 20px;">';  

if (($show < $numrows))
	{
		//echo '<a href="'.api_get_self().'?survey_id='.$survey_id.'&show='.$limit.'">NEXT</a>';
		  
    echo '<div class="pull-bottom"><button type="submit" name="next_survey_page" class="next">'.get_lang('Validate').'</div></button>';
	}
	if ($show >= $numrows)
	{
		echo '<div class="pull-bottom">
                    <button type="submit" name="finish_survey" class="next">'.get_lang('FinishSurvey').'
                        </div>
                        </button>';
	}
        echo '</div>';
...
function check_time_availability($surv_data){
    ...
    if ($cur_date < $start_date) {
        Display :: display_warning_message(get_lang('SurveyNotAvailableYet'), false, true);
        Display :: display_footer();
        exit;
    }
    if ($cur_date > $end_date)
    {
        Display :: display_warning_message(get_lang('SurveyNotAvailableAnymore'), false, true);
        Display :: display_footer();
        exit;
    }
}
