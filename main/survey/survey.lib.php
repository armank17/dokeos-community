...
*/
$htmlHeadXtra[] = '<style>.custom-actions{margin-bottom: 30px; padding-top: 0;}.survey_question_options{padding-top:10px; padding-bottom:5px}</style>';
	...
	function get_survey($survey_id,$shared=0,$course_code='')
	{
		...
		if (Database::num_rows($result)> 0) {
			...
			$return['end_date'] 			= $return['avail_till'];
                        $return['question_per_page'] 		= $return['question_per_page'];
	...
	function store_survey($values)
	{
		...
		/*	$sql = 'SELECT 1 FROM '.$table_survey.' WHERE code="'.Database::escape_string($values['survey_code']).'" AND lang="'.Database::escape_string($values['survey_language']).'"';
			...
			if($values['survey_type']==1)
			{
				...
				if(!empty($values['parent_id']))
				{
					...
					else
					{
                        ...
                        /*IF DELETE ALL SURVERY RESET CODE = 1*/
                        $query="SELECT * FROM $table_survey";
                        $resul = Database::query($query);
                        $count = Database::num_rows($resul);
                        if($count==0){
                            $table_survey_invitation = Database :: get_course_table(TABLE_SURVEY_INVITATION);
                            $q="DELETE FROM $table_survey_invitation";
                            Database::query($q);
                            $query="ALTER TABLE $table_survey AUTO_INCREMENT=1";
			...
			//End
                        ...
                        $start = (date("Y-m-d", strtotime($values['start_date'])));
                        $end = (date("Y-m-d", strtotime($values['end_date'])));
                        
                        if (strtotime($start) < strtotime(date('Y-m-d'))) {
                                $_SESSION['display_error_message']=get_lang('InvalidDate');
                                header('Location:create_new_survey.php?'.  api_get_cidreq().'&action=add');
				exit();
                        }
				...
				exit();
                        ...
                        if ($start > $end) {
                                $_SESSION['display_error_message']=get_lang('StartDateShouldBeBeforeEndDate');
                                header('Location:create_new_survey.php?'.  api_get_cidreq().'&action=add');
				exit();
                        }
			$sql = "INSERT INTO $table_survey (title, subtitle, author, lang, avail_from, avail_till, is_shared, template, intro, surveythanks, creation_date, anonymous".$additional['columns'].",question_per_page, session_id) VALUES (						
						...
						'".Database::escape_string($start)."',
						'".Database::escape_string($end)."',
			...
			if ($survey_id > 0) {
				...
				$sql_upd = "UPDATE $table_survey SET code = '".Database::escape_string($survey_id)."' WHERE survey_id = ".$survey_id;
		...
		else
		{
			...
			else
			{
				...
				$additionalsets .= ", form_fields = '' ";
                        ...
                        $start = (date("Y-m-d", strtotime($values['start_date'])));
                        //$start = TimeZone::ConvertTimeFromUserToServer($user_id, $start, "Y-m-d");
                        $end   = (date("Y-m-d", strtotime($values['end_date'])));
                        //$end   = TimeZone::ConvertTimeFromUserToServer($user_id, $end, "Y-m-d");
                        
                        print_r($values['start_date'].' - '.$values['end_date']);
                         if ($start > $end) {
                                $_SESSION['display_error_message']=get_lang('StartDateShouldBeBeforeEndDate');
                                header('Location:create_new_survey.php?'.  api_get_cidreq().'&action=edit&survey_id='.Database::escape_string($values['survey_id']));
				exit();
                         } 
			$sql = "UPDATE $table_survey SET
							...
							avail_from 		= '".Database::escape_string($start)."',
							avail_till		= '".Database::escape_string($end)."',
							...
							Anonymous	= '".Database::escape_string($values['anonymous'])."'".$additionalsets.",
                                                        question_per_page		= '".Database::escape_string($values['question_per_page'])."'    
  ...
  if (isset($_GET['lp_id']) && $_GET['lp_id'] > 0 && isset($_SESSION['oLP']) && $survey_id > 0) {
	...
	function update_survey_answered($survey_id, $user, $survey_code)
	{
		...
		$table_survey_invitation 	= Database :: get_course_table(TABLE_SURVEY_INVITATION, $_course['db_name']);
                ...
                $sql = "UPDATE $table_survey_invitation SET answered='1' $session_condition AND user='".Database::escape_string($user)."' AND survey_code='".Database::escape_string($survey_code)."'";		
	...
	function delete_survey_question($survey_id, $question_id, $shared=false)
	{
		...
		survey_manager::delete_survey_question_option($survey_id, $question_id, $shared);
                $message = get_lang('SurveyDeleted');
                return $message;
	}
...
class question
{
	...
	function create_form($form_content)
	{
		...
		if ($_GET['type'] == 'yesno') {
			...
			$tool_name .= ': '.get_lang('UniqueSelect');
                    ...
                    $tool_name = '';
                    $tool_name = ($_GET['action'] == 'add') ? get_lang('AddAComment') : get_lang('EditAComment');
                }else {
		...
		$this->html .= '<form id="question_form" name="question_form" method="post" action="'.api_get_self().'?'.api_get_cidreq().'&action='.Security::remove_XSS($_GET['action']).'&type='.Security::remove_XSS($_GET['type']).'&survey_id='.Security::remove_XSS($_GET['survey_id']).(isset($_GET['question_id'])? '&question_id='.Security::remove_XSS($_GET['question_id']):'').'">';
		...
		$this->html .= '		<div class="formw">';
                $this->html .=          api_return_html_area('question',$form_content['question'], '', '', null, array('ToolbarSet' => 'Survey', 'Width' => '700px', 'Height' => '60px'));
		$this->html .= '		</div>';
		$this->html .= '	</div>';
	...
	function render_form()
	{
		...
		$this->html .= '			<div class="pull-bottom"><button class="'.$class.'"type="submit" name="save_question">'.$text.'</button></div>';
		$this->html .= '		</div>';
		$this->html .= '	</div>';
	...
	function handle_action($form_content)
	{
		...
		// saving a question
			...
			$message = survey_manager::save_question($form_content);                        
                        $_SESSION['display_confirmation_message'] = get_lang($message);
			if ($message == 'QuestionAdded' || $message == 'QuestionUpdated' ) {
				...
				if ($config['survey']['debug']) {
					...
					Display :: display_confirmation_message($message.'<br />'.get_lang('ReturnTo').' <a href="survey.php?survey_id='.Security::remove_XSS($_GET['survey_id']).'">'.get_lang('Survey').'</a>', false, true);
				} else {
				...
				if ($message == 'PleaseEnterAQuestion' || $message=='PleasFillAllAnswer'|| $message=='PleaseChooseACondition'|| $message=='ChooseDifferentCategories') {
					...
					$_SESSION['temp_values']=$form_content['values'];
                                        $_SESSION['error']=  '* '.get_lang('AddResponses');
	...
	function add_remove_buttons($form_content)
	{
        ...
        if (strtolower($currentEditor) == 'ckeditor') {
            $return .= '<div class="row" style="padding-left:850px;">
                <div name="remove_answer" id="remove_answer" class="button_less" onclick="removeAnswer()" style="float:left;"></div>
                <div name="add_answer" id="add_answer" class="button_more" onclick="addNewAnswer()" style="float:left;"></div>
            </div>';
        }   
        else {
            $return .= '<div class="row" style="padding-left:850px;">
...
//		$this->html .= '		<textarea name="answers[0]" id="answers[0]" style="width: 600px; height: 50px;">'.stripslashes($form_content['answers'][0]).'</textarea>';
                $this->html .=          api_return_html_area('answers[0]',$form_content['answers'][0], '', '', null, array('ToolbarSet' => 'Survey', 'Width' => '700px', 'Height' => '60px'));
		$this->html .= '		</div></div>';
                $this->html .= '                            <div class="row">';
		$this->html .= '		<div class="label">';
...
//		$this->html .= '		<textarea name="answers[1]" id="answers[1]" style="width: 600px; height: 50px;">'.stripslashes($form_content['answers'][1]).'</textarea>';
                $this->html .=          api_return_html_area('answers[1]',$form_content['answers'][1], '', '', null, array('ToolbarSet' => 'Survey', 'Width' => '700px', 'Height' => '60px'));
		$this->html .= '		</div>';
		$this->html .= '	</div>';
	}
	...
	{        
                    $disabled = (isset($answers[0]))? 'disabled':'';
                    
                    
		foreach ($form_content['options'] as $key=>$value)
                {       if($disabled<>''){//if I'm on reporting
                            $checked = in_array($key, $answers)?'checked="checked"':'';
                        }
                        
                        if ($form_content['display'] == 'horizontal') { 
                        $this->html .= '<div id="div_radio-'.$key.'" style="float:left;">';
                        }else{
                        $this->html .= '<div id="div_radio-'.$key.'">'; 
                        }
                        $this->html .= '<input id="radio-'.$key.'" '.$disabled.' name="question'.$form_content['question_id'].'" type="radio" '.$disabled .' '. $checked.' value="'.$key.'"'; 
                        
                        $value = str_replace("<p>","",$value);
                        $value = str_replace("</p>","",$value);
                        $this->html .= '/><label for="radio-'.$key.'">'.$value.'</label></div>';
//                    if (is_array($answers))
//                    {	                                              
...
//				$this->html .= '/><label for="radio-'.$key.'">'.substr($value,3,(strlen($value)-7)).'</label></div>';
...
//                    }
                ...
                echo '<div id="survey_title">'.$form_content['survey_question'].'</div>';
...
class multiplechoice extends question
{
	...
	function create_form($form_content)
	{
		...
		$this->html .= '		</div>';
                //$this->html .= 'Error';
                //$this->html .= '<span class="form_error" id="form_error">'.$form_content['error'].'</span>';
		...
		$this->html .= '		<div class="formw">';
                 ...
                 * $_SESSION['temp_values']=$form_content['values'];
                 */
                if($form_content['answers'][$key]==''){
                    //$this->html .= 'answers: '.'<label>'.$form_content['answers'][$key].'</label>';
                    $this->html .= '<span class="form_error" id="form_error">'.$form_content['error'].'</span>';
                }   
                //$this->html .= '<textarea name="answers['.$key.']" id="answers['.$key.']" style="width: 600px; height: 50px;">'.api_html_entity_decode(stripslashes($form_content['answers'][$key]), ENT_QUOTES, $charset).'</textarea>';
                $this->html .=          api_return_html_area('answers['.$key.']',$value, '', '', null, array('ToolbarSet' => 'Survey', 'Width' => '700', 'Height' => '60'));
...
//        <textarea name="answers['.$key.']" id="answers['.$key.']" style="width: 600px; height: 50px;">'.api_html_entity_decode(stripslashes($form_content['answers'][$key]), ENT_QUOTES, $charset).'</textarea>';
                $this->html .=          api_return_html_area('answers['.$key.']',$value, '', '', null, array('ToolbarSet' => 'Survey', 'Width' => '700px', 'Height' => '60px'));
		if ($total_number_of_answers> 2) {
				//$this->html .= '			<input type="image" src="../img/delete_link.png"  value="delete_answer['.$key.']" name="delete_answer['.$key.']"/>';
			}
		$this->html .= '</div></div>';
	...
	function render_question($form_content, $answers=array())
	{
		...
		foreach ($form_content['options'] as $key=>$value)
		{
                        ...
                        $this->html .= '<input id="check-'.$key.'" name="question'.$form_content['question_id'].'[]" type="checkbox"  '.$disabled.' value="'.$key.'" style="vertical-align:middle" ';
			...
			if (is_array($answers))
			{
				if (in_array($key,$answers))
				{
					$this->html .= 'checked="checked"';
                        ...
                        $value = str_replace("<p>","",$value);
                        $value = str_replace("</p>","",$value);
                        $this->html .= '/><label for="check-'.$key.'">'.$value.'</label></div>';
		}
//		foreach ($form_content['options'] as $key=>$value)
//		{
//                        if ($form_content['display'] == 'horizontal') {
...
//                        }
//                        $this->html .= '<input id="check-'.$key.'" name="question'.$form_content['question_id'].'[]" type="checkbox"  '.$disabled.' value="'.$key.'" style="vertical-align:middle" ';
//                        //$temp='<input id="check-'.$key.'" name="question'.$form_content['question_id'].'[]" type="checkbox"  '.$disabled.' value="'.$key.'" style="vertical-align:middle" ';
//			if (is_array($answers))
//			{
//				if (in_array($key,$answers))
//				{
...
class dropdown extends question
{
	...
	function render_question($form_content, $answers=array())
	{
		...
		echo '<div id="survey_title">'.$form_content['survey_question'].'</div>';
		echo '<div class="survey_question_options">';
		echo '<select name="question'.$form_content['question_id'].'" id="select" '.$disabled.'>';
		echo $this->html;
		echo '</select>';
		echo '</div>';
		echo '</div>';
...
class open extends question
{
	...
	function create_form($form_content)
	{
		...
		$this->html .= '	<div class="row">';		
		$this->html .= '		<div class="formw">';
                $this->html .= '		<input type="hidden" name="answers[0]" id="answers[0]" value="'.stripslashes($form_content['answers'][0]).'" />';
...
//          <textarea name="question'.$form_content['question_id'].'" id="textarea" style="width: 800px; height: 130px;">'.$content.'</textarea>';
        echo api_return_html_area('question'.$form_content['question_id'],$content, '', '', $disabled, array('ToolbarSet' => 'Messages', 'Width' => '700px', 'Height' => '60px'));
		echo '</div>';
		echo '</div>';
	}
}
	...
	{?>
            <script>
                $(document).ready(function(){
                   $("input[name^='question']").focusout(function(){
                       var perce = document.getElementById("question<?php echo $form_content['question_id'] ?>");
                       var perceVal= perce.value;
                       if (perceVal > 100){
                           alert("<?php echo get_lang('valueMustBeLessThanOrEqualThanHundred'); ?>");
                           perce.value= '';
		...
		if(isset($answers[0])){
                    ...
                    $sql =  'SELECT option_text FROM'. Database :: get_course_table(TABLE_SURVEY_QUESTION_OPTION).'
                         WHERE question_option_id =' .$answers[0];
                    $result = Database::query($sql, __FILE__, __LINE__);
                    $row = Database::fetch_array($result,'ASSOC');
                    $percent_val = $row['option_text'];
                   
                    
                }else{
                    $percent_val = 0;
                    $disabled = ''; 
                    $disabled_slider = 'disabled: false,'; 
				}
                ...
                echo '<script type="text/javascript">
                         $(document).ready( function() {
                            ...
                            $("#question'.$form_content['question_id'].'").keyup(function() { 
                                 var value1 = $( "#question'.$form_content['question_id'].'" ).val();
                        ...
                        $(function() {
                            $( "#slider-percent" ).slider({
                                range: "min", // If set to true, the slider will detect if you have two handles and create a stylable range element between these two. 
                                value:'.$percent_val.', // Determines the value of the slider, if there is only one handle.
                                min: 0, // The minimum value of the slider.
                                max: 100, // The maximum value of the slider.
                                step: 1, // Determines the size or amount of each interval or step the slider takes between min and max. 
                                orientation: "horizontal", // Initialize a slider with the orientation option specified. 
                                '.$disabled_slider.'
                                slide: function( event, ui ) {
                                    $( "#question'.$form_content['question_id'].'" ).val( ui.value);
			}
                            });
                            $( "#question'.$form_content['question_id'].'" ).val( $( "#slider-percent" ).slider( "value" ));
                        });
                     </script>';
		echo '<div class="survey_question_wrapper">';
		echo '<div id="survey_title">'.$form_content['survey_question'].'</div>';
		echo '<div class="survey_question_options">';
                echo '<p><label for="Percent">'.get_lang('Percentage').':</label>';
                echo '<input type="text" id="question'.$form_content['question_id'].'" name="question'.$form_content['question_id'].'" '.$disabled.' style="background-color: #F6F6F6; border: 0 none; padding: 1px 3px; text-align: right; width: 40px;" />';
                echo '<label>%</label></p>';
                echo '<div id="slider-percent" style="margin: 0 25px 0 5px;" ></div>'; 
		echo '</div>';
		echo '</div>';
	}
}
...
class score extends question
{
	function create_form($form_content)
	{
		...
		$this->html .= '		</div>';
		$this->html .= '		<div class="formw">';
		$this->html .= '			<input type="text" name="maximum_score" id="maximum_score" style="width: 50px;" value="'.$form_content['maximum_score'].'" />';
		$this->html .= '		</div>';
		$this->html .= '	</div>';
...
//        <textarea name="answers['.$key.']" id="answers['.$key.']" style="width: 600px; height: 50px;">'.api_html_entity_decode(stripslashes($form_content['answers'][$key]), ENT_QUOTES, $charset).'</textarea>';
	...
	function render_question($form_content, $answers=array())
	{
		...
		echo '</pre></div>';
		*/
                $disabled = (!empty($answers))? 'disabled':'';
		...
		foreach ($form_content['options'] as $key=>$value)
		{
			$this->html .= '<tr>
								<td>'.$value.'</td>';
			$this->html .= '	<td>';
			$this->html .= '<select name="question'.$form_content['question_id'].'['.$key.']" '.$disabled.'>';
...
class SurveyUtil {
	...
	function check_first_last_question($survey_id, $continue=true)
	{
		...
		while ($row = Database::fetch_array($result,'ASSOC'))
		{
			...
			{
				Display::display_error_message(get_lang('PagebreakNotFirst'), false, true);
				$error = true;
			}
			if ($counter == $total AND $row['type'] == 'pagebreak')
			{
				Display::display_error_message(get_lang('PagebreakNotLast'), false, true);
				$error = true;
			}
			$counter++;
		}
	...
	//	echo '<a href="reporting.php?survey_id='.Security::remove_XSS($_GET['survey_id']).'">'.Display::return_icon('back.png',get_lang('BackTo').' '.get_lang('ReportingOverview')).' '.get_lang('BackTo').' '.get_lang('ReportingOverview').'</a>';
		...
		//	echo '<a href="javascript: void(0);" onclick="document.form1b.submit();">'.Display::return_icon('excel.gif', get_lang('ExportAsXLS')).' '.get_lang('ExportAsXLS').'</a> ';
			...
			echo '</form>';
			echo '<form id="form2" name="form2"  style="padding-bottom:25px;" method="post" action="'.api_get_self().'?action='.Security::remove_XSS($_GET['action']).'&survey_id='.Security::remove_XSS($_GET['survey_id']).'">';
		}
		...
		echo '<option value="reporting.php?'.api_get_cidreq().'&action='.Security::remove_XSS($_GET['action']).'&survey_id='.Security::remove_XSS($_GET['survey_id']).'">'.get_lang('SelectUser').'</option>';
              $count_anonymous = 0;
		foreach ($people_filled as $key=>$person)
		{
			...
			echo '<option value="reporting.php?'.api_get_cidreq().'&action='.Security::remove_XSS($_GET['action']).'&survey_id='.Security::remove_XSS($_GET['survey_id']).'&user='.Security::remove_XSS($id).'" ';
			if ($_GET['user'] == $id)
			{
		...
		if (isset($_GET['user']))
		{
			//Display::display_normal_message(get_lang('AllQuestionsOnOnePage'), false, true);
	...
	//	echo '<a href="reporting.php?survey_id='.Security::remove_XSS($_GET['survey_id']).'">'.Display::return_icon('back.png',get_lang('BackTo').' '.get_lang('ReportingOverview')).' '.get_lang('BackTo').' '.get_lang('ReportingOverview').'</a>';
		...
		else
		{
			...
			echo '		<th>'.get_lang('Answer').'</th>';
			echo '		<th>'.get_lang('AbsoluteTotal').'</th>';
			echo '		<th>'.get_lang('PercentageOfUser').'</th>';
			echo '		<th align="center">'.get_lang('VisualRepresentation').'</th>';
			echo '	<tr>';
			...
			if(is_array($options)){
				foreach ($options as $key=>$value)
				{
					...
					echo '		<td align="center">'.round($answers_number, 2).' %</td>';
					echo '		<td align="right">';
					$size = $answers_number*2;
					if ($size > 0)
					{
						echo '<div class="survey_progress_color" style="width:'.$size.'px">&nbsp;</div>';
					}
					echo '		</td>';
					echo '	</tr>';
	...
	/*	echo '<div class="actions">';
		echo '<a href="reporting.php?survey_id='.Security::remove_XSS($_GET['survey_id']).'">'.Display::return_icon('back.png',get_lang('BackTo').' '.get_lang('ReportingOverview')).' '.get_lang('BackTo').' '.get_lang('ReportingOverview').'</a>';
		echo '</div>';*/		
		echo '<div style="border-bottom: 1px solid #CCCCCC;font-weight: bold;margin-bottom: 15px;padding: 10px 10px 20px;"><img style="display: block;float: left; margin-right: 5px; margin-top: -6px;" src="'.  api_get_path('WEB_CSS_PATH').api_get_setting('stylesheets').'/images/action/filter.png"><a id="displayText" href="javascript:toggle();"> '.get_lang('ShowFilterAttribute').'</a></div>';
		// the form
		echo '<div id="toggleText" style="display: none; margin-bottom:10px;" class="actions custom-actions" >';
		...
		echo '<form id="form1b" name="form1b" method="post" action="'.api_get_self().'?action='.Security::remove_XSS($_GET['action']).'&survey_id='.Security::remove_XSS($_GET['survey_id']).'">';
                echo '<input type="hidden" name="questions_filter[]" value=1>';
                echo '<input type="hidden" name="export_report" value="export_report">';
		echo '<input type="hidden" name="export_format" value="xls">';
		echo '</form>';
		...
		echo '			<div class="pull-bottom"><button class="save" type="submit" name="submit_question_filter" value="'.get_lang('SubmitQuestionFilter').'">'.get_lang('SubmitQuestionFilter').'</button></div>';
		echo '</td></tr>';
		echo '</table><br/>';

		echo '</div>'; //close the toggle div
                
		if (!($_POST['export_report']) OR !empty($_POST['fields_filter']) OR !empty($_POST['submit_question_filter']))
		{ 
                    echo '<div class="actions custom-actions">';
                    echo '<div style="margin-top:20px; text-transform:uppercase; font-weight:bolder; margin-bottom:10px;">'.get_lang('UserFields').'</div>';	
		...
		$result = Database::query($sql, __FILE__, __LINE__);
			while ($row = Database::fetch_array($result)) {		
				...
				if (!($_POST['submit_question_filter']) OR (is_array($_POST['questions_filter']) && in_array($row['question_id'], $_POST['questions_filter']))) {
					...
					else if ($row['type'] <> 'comment' AND $row['type'] <> 'pagebreak' AND $row['type'] <> '')
					{						
						echo '			<th>';
			...
			if(intval($user)!==0)
			{
				...
				else
				{
					$user_displayed = '-';
                                ...
                                $style = (count($possible_options[1]) == 100) ? 'style="width:280px;height:35px; line-height:30px"' : '';
        
				echo '		<td '.$style.'><a href="'.api_get_self().'?action=userreport&survey_id='.Security::remove_XSS($_GET['survey_id']).'&user='.$user.'">'.$user_displayed.'</a></td>'; // the user column
			}
			else
			{
	 ...
	 * @version February 2007
	 */
	function export_complete_report_xls($filename, $user_id=0, $arr_question_id=null)
	{
		global $charset;
		...
		$table_survey_answer 			= Database :: get_course_table(TABLE_SURVEY_ANSWER);
              ...
              $condition_id=(!isset($arr_question_id))?'':' AND questions.question_id IN ('.implode(',',$arr_question_id).')';
		...
		$sql = "SELECT  questions.question_id, questions.type, questions.survey_question, count(options.question_option_id) as number_of_options
				...
				ON questions.question_id = options.question_id 
				WHERE questions.survey_id = '".Database::escape_string($_GET['survey_id'])."' $condition_id
				GROUP BY questions.question_id 
				ORDER BY questions.sort ASC";
		...
		if ($display_extra_user_fields)
		{
			...
			foreach($extra_user_fields as $field)
			{
                ...
                $condition_id=(!isset($arr_question_id))?'':' AND survey_question.question_id IN ('.implode(',',$arr_question_id).')';
		...
		$sql = "SELECT 	survey_question.question_id, survey_question.survey_id, survey_question.survey_question, survey_question.display, survey_question.sort, survey_question.type,
				...
				LEFT JOIN $table_survey_question_option survey_question_option
				ON survey_question.question_id = survey_question_option.question_id
				WHERE survey_question.survey_id = '".Database::escape_string($_GET['survey_id'])."' $condition_id
				ORDER BY survey_question.sort ASC, survey_question_option.sort ASC";
	...
	function display_comparative_report()
	{
		...
		// actions bar
		echo '<div class="actions2" style="padding:10px;">';
	//	echo '<a href="reporting.php?survey_id='.Security::remove_XSS($_GET['survey_id']).'">'.Display::return_icon('back.png',get_lang('BackTo').' '.get_lang('ReportingOverview')).' '.get_lang('BackTo').' '.get_lang('ReportingOverview').'</a>';
	//	echo '</div>';
		...
		echo '</select><br /><br />';
		echo '<div style="float: right; margin-right: 120px; margin-top: -80px;"><button class="save_survey" type="submit" name="Submit" value="Submit">'.get_lang('CompareQuestions').'</button></div>';
//                echo '<style>#content{padding-bottom:80px;}</style>';
		...
		echo '<br/><table width="100%"><tr style="background-color: #9D9D9D; color: #000000; height:40px;"><th width="50%">' . get_lang("XAxisQuestion") . '</th><th width="50%">' . get_lang("YAxisQuestion") . '</th></tr><tr><td valign="top">'.$question_x['question'].'</td><td valign="top">'.$question_y['question'].'</td></tr></table><br/>';
		...
		if (isset($_GET['xaxis']) AND is_numeric($_GET['xaxis']) AND isset($_GET['yaxis']) AND is_numeric($_GET['yaxis']))
		{
			...
			for ($ii=0; $ii<=count($question_x['answers']); $ii++)
			{
				if ($ii == 0)
				{
					echo '		<th>X/Y ' . get_lang("Options") . '</th>';
				}
				else
				{
            ...
            foreach ($users_array as $key=>$value) {
                ...
                if ((is_numeric($value) AND !in_array($value,$my_alredy_invited)) OR
                    ...
                    if (!array_key_exists($value,$survey_invitations)) {
                        $sql = "INSERT INTO $table_survey_invitation (user, survey_code, invitation_code, invitation_date,session_id) VALUES
                                 ('".Database::escape_string($value)."','".Database::escape_string($survey_data['code'])."','".Database::escape_string($invitation_code)."','".Database::escape_string(date('Y-m-d H:i:s'))."',".$session_id.")";
                        $result = Database::query($sql, __FILE__, __LINE__);
                    }
                }
                // send the email if checkboxed
                if (($newUser==true || $reminder==1)) {
	 ...
	 */
        function send_invitation_mail($invitedUser, $invitation_code, $invitation_title, $invitation_text) {
            ...
            $survey_data = survey_manager::get_survey($_GET['survey_id']);            
            $page_qpp = ( $survey_data['question_per_page'] == '0' ) ? 'fillsurvey.php' : 'fillsurveyall.php';
            
            // replacing the **link** part with a valid link for the user
            $survey_link = $portal_url.$_configuration['code_append'].'survey/'.$page_qpp.'?course='.$_course['sysCode'].'&cidReq='.$_course['sysCode'].'&invitationcode='.$invitation_code;
...
//				</div>';
	...
	function display_survey_list()
	{
		...
		$table = new SortableTable('surveys', 'get_number_of_surveys', 'get_survey_data',1,20,'ASC');
		...
		$table->set_header(6, get_lang('AvailableTill'));
		$table->set_header(7, get_lang('Invite'));
		//$table->set_header(9, get_lang('Anonymous'));		
		$table->set_header(8, get_lang("Actions"),false,'colspan="4"');
	/*	$table->set_header(10, '');
		$table->set_header(11, '');
		$table->set_header(12, '');
		$table->set_header(13, '');*/
	...
	function display_survey_list_for_coach()
	{
		...
		// Create a sortable table with survey-data
		$table = new SortableTable('surveys', 'get_number_of_surveys_for_coach', 'get_survey_data_for_coach',3);
		$table->set_additional_parameters($parameters);
		$table->set_header(0, '', false);
	...
	function modify_filter($survey_id)
	{
		...
		{
			$return .= '<div align="center"><a href="survey.php?'.api_get_cidreq().'&action=edit&survey_id='.$survey_id.'">'.Display::return_icon('pixel.gif', get_lang('Edit'),array("class" => "actionplaceholdericon toolactionedit_link")).'</a></div>';
	...
	function get_survey_data($from, $number_of_items, $column, $direction)
	{
		...
		//	$array[5] = '<div style="float:left;">'.$survey[4].'</div>';
                        $array[4] = '<div style="float:left;">'.sprintf('<a id="user_id_%s" class="user_info" href="javascript:void(0);">'.$survey[4].'</a>',$user_id_a).'</div>';
                        
		//	$array[5] = $survey[5];
			$array[5] = date('d-m-Y', strtotime($survey[6]));
			$array[6] = date('d-m-Y', strtotime($survey[7]));
			$array[7] = $survey[8];
		//	$array[9] = $survey[9];
		...
		/*	$return = '<div align="center"><a href="survey_list.php?'.api_get_cidreq().'&action=delete&survey_id='.$surveyid.'">'.Display::return_icon('delete_link.png', get_lang('Delete')).'</a></div>';
			...
			$array[8] = $return;
			$return = '<div align="center"><a href="preview.php?'       .api_get_cidreq().'&survey_id='.$surveyid.'&origin='.$origin.'">'.Display::return_icon('pixel.gif', get_lang('Preview'),array("class" => "actionplaceholdericon toolactionpreview_22")).'</a></div>';
			$array[9] = $return;
			...
			$array[11] = $return;
		//	$return = '<div align="center"><a href="survey_list.php?'.api_get_cidreq().'&action=delete&survey_id='.$surveyid.'" onclick="javascript:if(!confirm(\''.addslashes(api_htmlentities(get_lang("DeleteSurvey").'?')).'\')) return false;">'.Display::return_icon('pixel.gif', get_lang('DeleteSurvey'),array("class" => "actionplaceholdericon actiondelete")).'</a></div>';
		//	$array[13] = $return;
	...
	function survey_list_user($user_id)
	{
		...
		while ($row = Database::fetch_array($result,'ASSOC')) {
			...
			echo '<tr>';                        
			echo '<td align="center"><img src="' .api_get_path(WEB_CSS_PATH).api_get_setting('stylesheets').'/images/tool/home/survey.png" style ="width:50px"></td>';
                        $allinone = ($row['question_per_page'] != 0) ? 'all' : '';
			if ($row['answered'] == 0)
			{
				echo '<td><a href="fillsurvey'. $allinone .'.php?course='.$_course['sysCode'].'&session_id='.$session_id.'&invitationcode='.$row['invitation_code'].'&cidReq='.$_course['sysCode'].'">'.$row['title'].'</a></td>';
			}
			else
			{
