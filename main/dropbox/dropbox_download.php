...
if (isset($_GET['cat_id']) AND is_numeric($_GET['cat_id']) AND $_GET['action'] == 'downloadcategory' AND isset($_GET['sent_received'])) {
    ...
    $DropboxModel->zip_download($files_to_download);
    exit;
}
...
// the user is allowed to download the file
    ...
    $file = html_entity_decode($work->title);
    require_once(api_get_path(LIBRARY_PATH) . '/document.lib.php');
    $mimetype = DocumentManager::file_get_mime_type(TRUE);
//    $fileparts = explode('.', $file);
//    $filepartscount = count($fileparts);

//    if (preg_match('/\.(.+)?$/', $file, $matches) && isset($mimetype[$matches[1]])) { //( $filepartscount > 1) && isset( $mimetype[$fileparts [$filepartscount - 1]]) && $_GET['action']<>'download')
        //{
        // give hint to browser about filetype
        //header( "Content-type: " . $mimetype[$fileparts [$filepartscount - 1]] . "\n");
//        header("Content-type: " . $mimetype[$matches[1]] . "\n");
        //}
//    } else {
...
//    }
    ...
    header("Content-Description: " . trim($file) . "\n");
...
if ($_GET['mailing'])  // RH: Mailing detail window call
    $DropboxModel->getUserOwningThisMailing($_GET['mailing'], $_user['user_id'], '500');  // RH or die
 ...
 */
if (!isset($_GET['id']) || !is_numeric($_GET['id']))
    die(dropbox_lang("generalError") . " (code 501)");
...
//if (preg_match('/\.(.+)?$/', $file, $matches) && isset($mimetype[$matches[1]])) {
    // give hint to browser about filetype
//    header("Content-type: " . $mimetype[$matches[1]] . "\n");
//} else {
    //no information about filetype: force a download dialog window in browser
