...
if (isset($_POST['submitWork'])) {
    ...
    if ($check) {
        ...
        $_SESSION['display_confirmation_message'] = store_add_dropbox();
...
if (isset($_POST['StoreCategory'])) {
    ...
    if ($return_information['type'] == 'confirmation') {
        ...
        $_SESSION['display_confirmation_message'] = $return_information['message'];
    }
    if ($return_information['type'] == 'error') {
        ...
        $_SESSION['display_error_message'] = get_lang('FormHasErrorsPleaseComplete') . '<br />' . $return_information['message'];
...
if ($_POST['do_move']) {
    ...
    $_SESSION['display_confirmation_message'] = store_move($_POST['id'], $_POST['move_target'], $_POST['part']);
}
...
if (!isset($_POST['feedback']) && (strstr($_POST['action'], 'move_received') OR
    ...
    $_SESSION['display_normal_message'] = get_lang($display_message);
}
...
if ($_POST['feedback']) {
    ...
    //Display :: display_normal_message($display_message);
    $_SESSION['display_normal_message'] = get_lang($_SESSION['display_normal_message']);
}
...
if (isset($_GET['error']) AND !empty($_GET['error'])) {
    ...
    $_SESSION['display_normal_message'] = get_lang($_GET['error']);
}
...
// ACTIONS
    ...
    if (!$_GET['view'] OR $_GET['view'] == 'sent' OR $dropbox_cnf['sent_received_tabs'] == false) {
        ...
        $column_header[] = array(get_lang('Size'), TRUE, 'style="width:30px"');
        $column_header[] = array(get_lang('SentTo'), TRUE, 'style="width:250px"');
        $column_header[] = array(get_lang('LastResent'), TRUE, 'style="width:120px"');

        if (api_get_session_id() == 0)
            $column_header[] = array(get_lang('Modify'), FALSE, '', 'nowrap style="text-align: center; width:173px"');
        elseif (api_is_allowed_to_session_edit(false, true)) {
            $column_header[] = array(get_lang('Modify'), FALSE, '', 'nowrap style="text-align: center; width:173px"');
        }
        ...
        foreach ($dropbox_person->sentWork as $dropbox_file) {
            ...
            if ($view_dropbox_category_sent == $dropbox_file->category) {
                $dropbox_file_data[] = $dropbox_file->id;
                $dropbox_file_data[] = build_document_icon_tag('file', $dropbox_file->title);
                $dropbox_file_data[] = '<a href="dropbox_download.php?' . api_get_cidreq() . '&id=' . $dropbox_file->id . '&action=download">' . Display::return_icon('pixel.gif', get_lang('Save'), array('style' => 'float:right;', 'class' => 'actionplaceholdericon forcedownload')) . '</a><a href="dropbox_download.php?' . api_get_cidreq() . '&id=' . $dropbox_file->id . '&action=download" target="_blank">' . $dropbox_file->title . '</a><br>' . $dropbox_file->description;
                $dropbox_file_data[] = ceil(($dropbox_file->filesize) / 1024) . ' ' . get_lang('kB');
                ...
                //$dropbox_file_data[]=$dropbox_file->author;
                $receivers_celldata = '';
                $action_icons = check_number_feedback($dropbox_file->id, $number_feedback) . '<div class="edit-sent-file"> <span>' . get_lang('Feedback') . '</span>
				<span><a href="' . api_get_self() . '?' . api_get_cidreq() . '&view_received_category=' . Security::remove_XSS($_GET['view_received_category']) . '&view_sent_category=' . Security::remove_XSS($_GET['view_sent_category']) . '&view=' . Security::remove_XSS($_GET['view']) . '&action=viewfeedback&id=' . $dropbox_file->id . '">' . Display::return_icon('comment_bubble.gif', get_lang('Comment')) . '</a></span>
				<span><a href="' . api_get_self() . '?' . api_get_cidreq() . '&view_received_category=' . Security::remove_XSS($_GET['view_received_category']) . '&view_sent_category=' . Security::remove_XSS($_GET['view_sent_category']) . '&view=' . Security::remove_XSS($_GET['view']) . '&action=movesent&move_id=' . $dropbox_file->id . '">' . Display::return_icon('pixel.gif', get_lang('Move'), array('class' => 'actionplaceholdericon actionmove')) . '</a></span>
				<span><a href="' . api_get_self() . '?' . api_get_cidreq() . '&view_received_category=' . Security::remove_XSS($_GET['view_received_category']) . '&view_sent_category=' . Security::remove_XSS($_GET['view_sent_category']) . '&view=' . Security::remove_XSS($_GET['view']) . '&action=deletesentfile&id=' . $dropbox_file->id . '" onclick="return confirmation(\'' . $dropbox_file->title . '\');">' . Display::return_icon('pixel.gif', get_lang('Delete'), array('class' => 'actionplaceholdericon actiondelete')) . '</a></span><div>';
                // this is a hack to have an additional row in a sortable table
                if ($_GET['action'] == 'viewfeedback' AND isset($_GET['id']) and is_numeric($_GET['id']) AND $dropbox_file->id == $_GET['id']) {
                    $action_icons.="</td></tr>\n"; // ending the normal row of the sortable table
