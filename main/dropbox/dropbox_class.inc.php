...
class Dropbox_Work {
    ...
    function _createNewWork($uploader_id, $title, $description, $author, $filename, $filesize) {
        ...
        $this->uploaderName = $dropboxModel->getUserNameFromId($this->uploader_id);
        ...
        $sql = "SELECT id, upload_date
				FROM " . $dropboxModel->dropbox_cnf("tbl_file") . "
        ...
        if ($this->isOldWork) {
            ...
            $sql = "UPDATE " . $dropboxModel->dropbox_cnf("tbl_file") . "
            ...
            $sql = "INSERT INTO " . $dropboxModel->dropbox_cnf("tbl_file") . "
        ...
        $sql = "INSERT INTO " . $dropboxModel->dropbox_cnf("tbl_person") . "
				(file_id, user_id)
    ...
    function _createExistingWork($id) {
        ...
        $uploaderName = $dropboxModel->getUserNameFromId($uploader_id);
        ...
        if ($_GET['action'] == 'viewfeedback' AND $this->id == $_GET['id']) {
            $feedback2 = array();
            $sql_feedback = "SELECT * FROM " . $dropboxModel->dropbox_cnf("tbl_feedback") . " WHERE file_id='" . $id . "' ORDER BY feedback_id ASC";            
            $result = Database::query($sql_feedback, __FILE__, __LINE__);
            while ($row_feedback = Database::fetch_array($result)) {
                $row_feedback['feedback_date'] = TimeZone::ConvertTimeFromServerToUser(api_get_user_id(), $row_feedback['feedback_date']);
...
class Dropbox_SentWork extends Dropbox_Work {
    ...
    function _createNewSentWork($uploader_id, $title, $description, $author, $filename, $filesize, $recipient_ids) {
        ...
        foreach ($recipient_ids as $rec) {
            if (empty($rec))
                die(dropbox_lang("generalError") . " (code 210)");
            ...
            $this->recipients[] = array("id" => $rec, "name" => $dropboxModel->getUserNameFromId($rec));
        }

        // insert data in dropbox_post and dropbox_person table for each recipient
        foreach ($this->recipients as $rec) {
            $sql = "INSERT INTO " . $dropboxModel->dropbox_cnf("tbl_post") . "
            ...
            if (($ownerid = $this->uploader_id) > $dropbox_cnf["mailingIdBase"]) {
                $ownerid = $dropboxModel->getUserOwningThisMailing($ownerid);
            }
    ...
    function _createExistingSentWork($id) {
        ...
        $sql = "SELECT dest_user_id, feedback_date, feedback
				FROM " . $dropboxModel->dropbox_cnf("tbl_post") . "
				WHERE file_id='" . addslashes($id) . "'";
        $result = Database::query($sql, __FILE__, __LINE__);
        while ($res = Database::fetch_array($result)) {
            // check for deleted users
            $dest_user_id = $res["dest_user_id"];
            $recipientName = $dropboxModel->getUserNameFromId($dest_user_id);
            //$this->category=$res['cat_id'];
...
class Dropbox_Person {
    ...
    function Dropbox_Person($userId, $isCourseAdmin, $isCourseTutor) {
        ...
        $or_session = ((!api_is_allowed_to_edit())&&(api_get_session_id()!=0))? ' OR r.session_id=0 ':'';
        
        // find all entries where this person is the recipient
        $sql = "SELECT r.file_id, r.cat_id
				FROM $post_tbl r, $person_tbl p , $file_tbl f WHERE " 
                                        . "( r.session_id =" . api_get_session_id() .$or_session. " ) AND r.dest_user_id = '" . addslashes($this->userId) . "'
...
//$this->sql_ = $sql;
    ...
    function deleteAllReceivedWork() {
        ...
        foreach ($this->receivedWork as $w) {
            Database::query("DELETE FROM " . $dropboxModel->dropbox_cnf("tbl_person") . " WHERE user_id='" . $this->userId . "' AND file_id='" . $w->id . "'", __FILE__, __LINE__);
        }
        $dropboxModel->removeUnusedFiles(); //check for unused files
    }
    ...
    function deleteReceivedWorkFolder($id) {
        ...
        $sql = "DELETE FROM " . $dropboxModel->dropbox_cnf("tbl_file") . " where cat_id = '" . $id . "' ";
        if (!Database::query($sql))
            return false;
        $sql = "DELETE FROM " . $dropboxModel->dropbox_cnf("tbl_category") . " where cat_id = '" . $id . "' ";
        if (!Database::query($sql))
            return false;
        $sql = "DELETE FROM " . $dropboxModel->dropbox_cnf("tbl_post") . " where cat_id = '" . $id . "' ";
        if (!Database::query($sql))
            return false;
        return true;
    }
    ...
    function deleteReceivedWork($id) {
        ...
        //delete entries in person table concerning received works
        Database::query("DELETE FROM " . $dropboxModel->dropbox_cnf("tbl_person") . " WHERE user_id='" . $this->userId . "' AND file_id='" . $id . "'", __FILE__, __LINE__);
        $dropboxModel->removeUnusedFiles(); //check for unused files
    }
    ...
    function deleteAllSentWork() {
        ...
        foreach ($this->sentWork as $w) {
            Database::query("DELETE FROM " . $dropboxModel->dropbox_cnf("tbl_person") . " WHERE user_id='" . $this->userId . "' AND file_id='" . $w->id . "'", __FILE__, __LINE__);
            $dropboxModel->removeMoreIfMailing($w->id);  // RH: Mailing: see init1
        }
        $dropboxModel->removeUnusedFiles(); //check for unused files
    }
    ...
    function deleteSentWork($id) {
        ...
        //delete entries in person table concerning sent works
        Database::query("DELETE FROM " . $dropboxModel->dropbox_cnf("tbl_person") . " WHERE user_id='" . $this->userId . "' AND file_id='" . $id . "'", __FILE__, __LINE__);
        $dropboxModel->removeMoreIfMailing($id);  // RH: Mailing: see init1
        $dropboxModel->removeUnusedFiles(); //check for unused files
    }
